...
6. When WIRING_PHYSICALITY is material, each material edge must name producer, transported contract, consumer, enforcement point, failure behavior, exact obligation IDs, binding state, and evidence. EXISTING_AUTHORITY or EXISTING_REUSABLE_CAPABILITY proves that a component exists; it does not prove that the real execution path consumes it. A CLOSED wiring obligation requires an OBSERVED_WIRED existing binding with current external evidence. PROPOSED_WIRING / PROPOSED_DELIVERABLE is valid future design but leaves the current obligation open.
6a. When CONTEXT_TRANSPORT is material, closure additionally requires a context-transport proof for selection/admission, transported contract, actual consumer, enforcement point, total budget guard, hydration mode, JIT resolver when applicable, failure behavior, physical wiring edge(s), and current readback evidence. Describing a compact capsule, JIT capability, budget, Card, resolver, or policy without proving consumption on the execution path is not closure.
