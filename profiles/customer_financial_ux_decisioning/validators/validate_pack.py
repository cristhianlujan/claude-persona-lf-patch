...
REQUIRED = [
    ...
    "contracts/input_governance_binding.json",
...
outputs = set(schema["properties"]["output_type"]["enum"])
...
if not set(REQUIRED).issubset(required_files):
...
case_types = {case.get("type") for case in evals.get("cases", [])}
...
if bad.get("self_verdict") != "READY_FOR_REVIEW":
    fail("NEGATIVE_FIXTURE_NO_LONGER_ADVERSARIAL")
if bad.get("options", [{}])[0].get("authority_refs") != []:
