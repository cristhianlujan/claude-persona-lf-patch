...
A normal decision spec must include:
- `decision_id` and `decision_question`;
- `options[]` with stable IDs;
- for each option: authoritative terms, customer consequence, timing, monetary basis, key tradeoff, irreversible/reversible effect when known;
- `comparison_basis` proving alternatives are comparable;
- `must_understand[]` for material facts before action;
- `uncertainties[]` with exact unresolved refs;
- `claim_guardrails[]`;
- `recommended_presentation_priority` limited to semantic priority, not layout;
- `handoff_to_next` preserving exact option IDs and authority refs;
- `self_verdict`.
...
- Missing nonmaterial copy/detail may remain `PROPOSED_NOT_CANONICAL`; missing material truth routes fail-closed.
...
## Proof boundary
Deterministic validators and fixtures prove contract consistency only. Behavioral PASS requires RAW profile output plus an execution receipt bound to exact source/input/output and applicable semantic review. Candidate creation does not authorize runtime, production, VALIDATED or VIGENTE.
