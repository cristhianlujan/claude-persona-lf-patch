...
with tempfile.TemporaryDirectory() as td:
    ...
    artifact={}
    artifact_sha=canonical_sha(artifact)
    for name in ['artifact_payload.json','upstream_existing_screen_review_contract.md','quality_gate_contract.md','lf_quality_controls.md','quality_pack_score_rubric.md','quality_pack_mini_judge.md','source_visual.png','composer_payload_boundary_v1.md']:
        (b/name).write_text(json.dumps(artifact) if name=='artifact_payload.json' else ('{}' if name.endswith('.json') else 'fixture'),encoding='utf-8')
    case={'review_case_id':'CASE-001','artifact_ref':'bundle://artifact_payload.json','artifact_canonical_sha256':artifact_sha,'upstream_worker_contract_ref':'bundle://upstream_existing_screen_review_contract.md'}
    (b/'review_case.json').write_text(json.dumps(case),encoding='utf-8')
    review={
      'review_id':'CASE-001-TEST','reviewed_artifact':'bundle://artifact_payload.json#sha256='+artifact_sha,
    ...
    (b/'artifact_payload.json').write_text(json.dumps({'mutated':True}),encoding='utf-8')
    run('artifact_mutated',receipt,False)
    (b/'artifact_payload.json').write_text(json.dumps(artifact),encoding='utf-8')
    ...
    print('INDEPENDENT_REVIEW_BUNDLE_BINDING_REGRESSIONS_PASS=8/8')
