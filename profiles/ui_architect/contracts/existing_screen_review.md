...
4. **Missing information would materially change interaction semantics, business meaning, safety, primary action, route or a protected constraint** -> return the missing input to the orchestrator under `contracts/missing_input_policy.md`; do not silently invent it and do not ask the end user directly from the worker.
