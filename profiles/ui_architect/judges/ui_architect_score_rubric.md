...
## Visual output note
If the next artifact is an image prompt or rendered UI mockup, Handoff quality can score 5 only when `prompt_constraints` include enough acceptance criteria to preserve layout, hierarchy, legibility and state fidelity without inventing structure.
