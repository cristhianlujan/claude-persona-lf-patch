...
def fail(code): raise SystemExit(f"CUSTOMER_IDENTITY_CONSENT_PRIVACY_FAIL:{code}")
for rel in REQUIRED:
...
if set(schema["properties"]["output_type"]["enum"])!=expected_modes: fail("OUTPUT_ENUM_MISMATCH")
...
if set(binding.get("allowed_triggers",[]))!=TRIGGERS: fail("INPUT_GOVERNANCE_TRIGGER_SET_MISMATCH")
...
if manifest.get("profile_code")!="CUSTOMER_IDENTITY_CONSENT_PRIVACY": fail("PROFILE_IDENTITY_MISMATCH")
...
if bad["consent_items"][0]["affirmative_action"]!="prechecked checkbox": fail("NEGATIVE_FIXTURE_CONSENT_DEFECT_REMOVED")
if bad["identity_assurance"].get("authority_refs")!=[]: fail("NEGATIVE_FIXTURE_AUTHORITY_DEFECT_REMOVED")
...
print("CUSTOMER_IDENTITY_CONSENT_PRIVACY_CHAMPION_EVIDENCE_PASS")
