...
def evaluate(case):
    if not isinstance(case,dict):
        return {"status":"BLOCK_PIPELINE","blocking_codes":["MALFORMED_CASE"],"readback_codes":[]}
    b=[]; r=[]; refs=set(); receipts=set(); independent=False
    ...
    for i,s in enumerate(sources):
        ...
        if s.get("authority") is True:
            ...
            elif s.get("read") is True and s.get("relevance")=="MATERIAL": independent=True
    ...
    if sources and not independent: b.append("INDEPENDENT_AUTHORITY_MISSING")
    ...
    for i,x in enumerate(case.get("conflicts",[]) if isinstance(case.get("conflicts",[]),list) else [{}]):
    ...
    return {"status":status,"blocking_codes":sorted(set(b)),"readback_codes":sorted(set(r)),"gate_states":{"STRUCTURALLY_VALID":not any("MALFORMED" in x for x in b),"PROVENANCE_VALID":not any("RECEIPT" in x or "SHA" in x for x in b+r),"SEMANTICALLY_VALID":not any("ASSERTION" in x or "SEMANTIC" in x or "CORRELATED" in x for x in b),"ARTIFACT_VERIFIED":case.get("artifact_verified") is True,"UPSTREAM_VALID":not any("UPSTREAM" in x for x in b)}}
