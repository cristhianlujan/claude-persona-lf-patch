#!/usr/bin/env python3
"""LF Contract Check v0.17: governed strategy + repository-governance wrapper over exact v0.15."""
...
def validate_s30_governed_scope() -> None:
    failures: list[str] = []
    ...
    if failures:
        fail("FAIL_S30_GOVERNED_SCOPE_INVARIANT", ",".join(failures))
    print(
        "PASS_S30_GOVERNED_SCOPE_INVARIANT: "
        "strategies and repository-governance are receipt-required; exact legacy helper only"
    )


def validate_changed_files(changed_files: list[str]) -> list[str]:
    validate_s30_governed_scope()
