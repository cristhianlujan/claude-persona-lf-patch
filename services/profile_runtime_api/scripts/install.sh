...
for required in \
  ...
  "$source_dir/services/profile_runtime_api/deploy/lf-profile-semantic-judge-worker.service" \
  "$source_dir/services/profile_runtime_api/scripts/semantic_judge_worker.py" \
...
install -m 0644 "$release_dir/services/profile_runtime_api/deploy/lf-profile-semantic-judge-worker.service" \
  /etc/systemd/system/lf-profile-semantic-judge-worker.service
systemctl daemon-reload

if [[ "$start_service" == true ]]; then
  systemctl enable lf-profile-runtime-api.service lf-profile-semantic-judge-worker.service
  systemctl restart lf-profile-runtime-api.service
  systemctl restart lf-profile-semantic-judge-worker.service
  systemctl --no-pager --full status lf-profile-runtime-api.service
  systemctl --no-pager --full status lf-profile-semantic-judge-worker.service
else
  printf 'SERVICES_NOT_STARTED: run systemctl enable --now lf-profile-runtime-api.service lf-profile-semantic-judge-worker.service after config review\n'
fi
...
printf 'SEMANTIC_JUDGE_CONSUMER=lf-profile-semantic-judge-worker.service\n'
