#!/usr/bin/env python3
"""Execute the independent semantic judge as an execution-only runtime consumer.

The worker consumes only EJECUCION_PERFIL_LF executions whose deterministic
output_validate step is clean. The semantic-judge binding must already be resolved
upstream and frozen in the governed execution manifest. This worker validates that
immutable binding, executes it, and records evidence through the governed recorder.

It does not discover profiles, adapters, policies, judges, or validators.
"""
...
def _binding(payload: Any) -> dict[str, Any]:
    if (
        ...
        or payload.get("schema") != "LF_PROFILE_SEMANTIC_JUDGE_BINDING_V1"
    ):
...
def _resolved_binding(
    ...
    payload = manifest.get("semantic_judge_binding")
    digest = manifest.get("semantic_judge_binding_sha256")
    binding_ref = manifest.get("semantic_judge_binding_ref")
    activation_source = manifest.get("semantic_judge_binding_activation_source")
    target = manifest.get("semantic_judge_binding_target_code")

    if activation_source != "ROUTER":
        raise RuntimeError("SEMANTIC_JUDGE_BINDING_NOT_ROUTER_RESOLVED")
    if target != target_code:
        raise RuntimeError("SEMANTIC_JUDGE_BINDING_TARGET_MISMATCH")
    if not isinstance(binding_ref, str) or not binding_ref.strip():
        raise RuntimeError("SEMANTIC_JUDGE_BINDING_REF_MISSING")
    if (
        not isinstance(digest, str)
        or re.fullmatch(r"sha256:[0-9a-f]{64}", digest) is None
    ):
        raise RuntimeError("SEMANTIC_JUDGE_BINDING_DIGEST_INVALID")

    binding = _binding(payload)
    observed = "sha256:" + _canonical_json_sha256(binding)
    if observed != digest:
        raise RuntimeError("SEMANTIC_JUDGE_BINDING_DIGEST_MISMATCH")
...
def _prepare(claimed: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("SEMANTIC_JUDGE_EXECUTION_MANIFEST_MISSING")
    binding, binding_ref = _resolved_binding(manifest, str(claimed["target_code"]))
    ...
    validator_path = _bound_file(
        profile_root,
        binding["validator_path"],
        "SEMANTIC_JUDGE_VALIDATOR",
    )
    return {
        ...
        "deterministic_validation": (
            output_payload.get("output_contract_result")
            or output_payload.get("deterministic_validation")
            or {}
        ),
    }
...
def _judge(prepared: dict[str, Any]) -> dict[str, Any]:
    ...
    validator = _load_module(
        prepared["validator_path"],
        "lf_bound_semantic_judge_validator",
    )
    evaluate = getattr(
        validator,
        prepared["binding"]["validator_callable"],
        None,
    )
    ...
    passed = (
        ...
        and result.get("verdict") == prepared["binding"]["pass_verdict"]
    )
    return {
...
def _record(
    ...
    execution_id = claimed["execution_id"]
    raw_result = judged.get("result")
    if not isinstance(raw_result, dict):
        raw_result = {}
    unsupported = raw_result.get("unsupported_claims")
    if not isinstance(unsupported, list):
        unsupported = ["SEMANTIC_JUDGE_UNSUPPORTED_CLAIMS_INVALID"]
    ...
    refs = [
        ...
        + str(prepared["prompt_path"].relative_to(REPO_ROOT)),
        "github://cristhianlujan/claude-persona-lf-patch/"
        + str(prepared["validator_path"].relative_to(REPO_ROOT)),
    ]
    payload = {
        "semantic_judge_result": judged,
        "unsupported_claims": unsupported,
        "source_refs": refs,
        "blocking_codes": judged.get("validator_result", {}).get(
            "blocking_codes",
            [],
        ),
    }
    with conn.cursor() as cur:
        ...
        row = cur.fetchone()
        semantic = row[0] if row and isinstance(row[0], dict) else {}
        if (
            judged.get("status") != "PASS"
            or semantic.get("outcome") != "STEP_RECORDED"
        ):
            conn.commit()
            return {"semantic": semantic, "report": None}
        ...
        cur.execute(
            ...
            (
                ...
                (
                    "supabase://public.lf_operation_execution_steps/"
                    f"{execution_id}/report_output#semantic-judge-worker"
                ),
                Jsonb(report),
                execution_id,
            ),
        ...
        rr = cur.fetchone()
        report_record = rr[0] if rr and isinstance(rr[0], dict) else {}
    conn.commit()
    return {"semantic": semantic, "report": report_record}
...
def run_once() -> bool:
    ...
    try:
        ...
        print(
            "SEMANTIC_JUDGE_VALIDATOR_STATUS="
            + str(judged.get("validator_status"))
        )
        print(
            "SEMANTIC_JUDGE_RECORD="
            + json.dumps(recorded, ensure_ascii=False, sort_keys=True)
        )
        return True
    except Exception as exc:
        conn.rollback()
        print(
            f"SEMANTIC_JUDGE_ERROR={type(exc).__name__}:{str(exc)[:800]}",
            flush=True,
        )
        ...
        if claimed is not None:
            try:
                with conn.cursor() as cur:
                    cur.execute(
                        "select pg_advisory_unlock(pg_catalog.hashtextextended(%s,0))",
