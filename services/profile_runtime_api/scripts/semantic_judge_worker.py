...
This worker is deliberately separate from the producer worker. It consumes only
EJECUCION_PERFIL_LF executions whose deterministic output_validate step is clean,
loads the target profile's explicit semantic_judge_binding.json, runs exactly one
fresh judge model call, validates that receipt deterministically, and records the
canonical semantic_judge/report_output steps through Supabase.
"""
...
def _binding(profile_root: Path) -> dict[str, Any]:
    path = profile_root / "contracts/semantic_judge_binding.json"
    if not path.is_file():
        raise RuntimeError("SEMANTIC_JUDGE_BINDING_MISSING")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise RuntimeError("SEMANTIC_JUDGE_BINDING_INVALID") from exc
    ...
    if not isinstance(independence, dict) or independence != {
...
JUDGE_SCHEMA: dict[str, Any] = {
    ...
    "required": [
        "verdict", "source_refs_inspected", "observed_candidate_changes",
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            select e.execution_id,e.target_code,e.target_path,e.manifest,
                   i.evidence_payload as input_payload,
                   x.evidence_payload as execute_payload,
                   o.evidence_payload as output_payload
              from public.lf_operation_execution e
              join public.lf_operation_execution_steps i
                on i.execution_id=e.execution_id and i.step_id='input_validate'
              join public.lf_operation_execution_steps x
                on x.execution_id=e.execution_id and x.step_id='execute_profile'
              join public.lf_operation_execution_steps o
                on o.execution_id=e.execution_id and o.step_id='output_validate'
              left join public.lf_operation_execution_steps s
                on s.execution_id=e.execution_id and s.step_id='semantic_judge'
        ...
        for row in cur.fetchall():
            ...
            if locked and locked[0] is True:
                cols = [d.name for d in cur.description] if False else None
                return {
...
def _prepare(claimed: dict[str, Any]) -> dict[str, Any]:
    ...
    execute_payload = claimed["execute_payload"]
    output_payload = claimed["output_payload"]
    packet = input_payload.get("scope_authority_packet")
    ...
    return {
        ...
        "profile_slug": profile_root.name,
        "binding": binding,
        "binding_ref": str((profile_root / "contracts/semantic_judge_binding.json").relative_to(REPO_ROOT)),
        "prompt_path": prompt_path,
        "validator_path": validator_path,
        "candidate": candidate,
        "candidate_sha": candidate_sha,
        "scope_packet": packet,
        "scope_sha": scope_digest.split(":", 1)[1],
...
def _judge(prepared: dict[str, Any]) -> dict[str, Any]:
    ...
    system_prompt = prepared["prompt_path"].read_text(encoding="utf-8")
    user_payload = {
        "exact_candidate": prepared["candidate"],
        "scope_authority_packet": prepared["scope_packet"],
        "deterministic_validator_result": prepared["deterministic_validation"],
        "execution_id": prepared["execution_id"],
        "independence_contract": {
            "producer_prompt_reused": False,
            "producer_self_verdict_is_evidence": False,
            "independently_extract_candidate_changes": True,
        },
    }
    completion = client.chat(
        system_prompt=system_prompt,
        user_prompt=json.dumps(user_payload, ensure_ascii=False, sort_keys=True, separators=(",", ":")),
        schema=JUDGE_SCHEMA,
        profile_slug=prepared["profile_slug"],
        schema_mode="AUTO",
    )
    raw = completion.get("content")
    if not isinstance(raw, str):
        raise RuntimeError("SEMANTIC_JUDGE_MODEL_OUTPUT_MISSING")
    try:
        result = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise RuntimeError("SEMANTIC_JUDGE_MODEL_OUTPUT_INVALID_JSON") from exc
    if not isinstance(result, dict):
        raise RuntimeError("SEMANTIC_JUDGE_MODEL_OUTPUT_NOT_OBJECT")
    result["candidate_sha256"] = prepared["candidate_sha"]
    result["scope_packet_sha256"] = prepared["scope_sha"]
    ...
    if not callable(evaluate):
        raise RuntimeError("SEMANTIC_JUDGE_VALIDATOR_CALLABLE_MISSING")
    validated = evaluate(
        result,
        scope_packet=prepared["scope_packet"],
        expected_candidate_sha256=prepared["candidate_sha"],
        expected_scope_packet_sha256=prepared["scope_sha"],
    )
    if not isinstance(validated, dict):
        raise RuntimeError("SEMANTIC_JUDGE_VALIDATOR_RESULT_INVALID")
    pass_verdict = prepared["binding"]["pass_verdict"]
    passed = validated.get("status") == "PASS" and result.get("verdict") == pass_verdict
    return {
        "status": "PASS" if passed else "FAIL",
        "verdict": result.get("verdict"),
        "validator_status": validated.get("status"),
        "producer_independence_proven": True,
        "model_call_count": 1,
        "candidate_sha256": prepared["candidate_sha"],
        "scope_packet_sha256": prepared["scope_sha"],
        "result": result,
        "validator_result": validated,
        "judge_binding_ref": prepared["binding_ref"],
        "judge_prompt_ref": str(prepared["prompt_path"].relative_to(REPO_ROOT)),
...
def _record(conn: psycopg.Connection, claimed: dict[str, Any], prepared: dict[str, Any], judged: dict[str, Any]) -> dict[str, Any]:
    execution_id = claimed["execution_id"]
    raw_result = judged.get("result") if isinstance(judged.get("result"), dict) else {}
    ...
    payload = {
        "semantic_judge_result": judged,
        "unsupported_claims": unsupported,
        "source_refs": [
            "supabase://public.lf_operation_execution/" + execution_id,
            "supabase://public.lf_operation_execution_steps/" + execution_id + "/input_validate",
            "supabase://public.lf_operation_execution_steps/" + execution_id + "/execute_profile",
            "supabase://public.lf_operation_execution_steps/" + execution_id + "/output_validate",
            "github://cristhianlujan/claude-persona-lf-patch/" + prepared["binding_ref"],
            "github://cristhianlujan/claude-persona-lf-patch/" + str(prepared["prompt_path"].relative_to(REPO_ROOT)),
            "github://cristhianlujan/claude-persona-lf-patch/" + str(prepared["validator_path"].relative_to(REPO_ROOT)),
        ],
        "blocking_codes": judged.get("validator_result", {}).get("blocking_codes", []),
    }
    with conn.cursor() as cur:
        cur.execute(
            ...
            (
                execution_id,
                "semantic_judge",
                f"hetzner://profile-runtime/{execution_id}/independent-semantic-judge",
                Jsonb(payload),
                execution_id,
            ),
        )
        row = cur.fetchone()
        semantic_record = row[0] if row and isinstance(row[0], dict) else {}
        if judged.get("status") != "PASS" or semantic_record.get("outcome") != "STEP_RECORDED":
            conn.commit()
            return {"semantic": semantic_record, "report": None}

        report_payload = {
            "result": prepared["binding"]["pass_verdict"],
            "profile_code": claimed["target_code"],
            "execution_id": execution_id,
            "no_write_performed": True,
            "candidate_sha256": prepared["candidate_sha"],
            "scope_packet_sha256": prepared["scope_sha"],
            "evidence_refs": payload["source_refs"],
        }
        cur.execute(
            "select public.lf_record_profile_execution_step_v1(%s,%s,%s,%s,%s)",
            (
                execution_id,
                "report_output",
                f"supabase://public.lf_operation_execution_steps/{execution_id}/report_output#semantic-judge-worker",
                Jsonb(report_payload),
                execution_id,
            ),
        )
        report_row = cur.fetchone()
        report_record = report_row[0] if report_row and isinstance(report_row[0], dict) else {}
    conn.commit()
    return {"semantic": semantic_record, "report": report_record}
...
def run_once() -> bool:
    ...
    try:
        ...
        if claimed is None:
            return False
        prepared = _prepare(claimed)
        judged = _judge(prepared)
        recorded = _record(conn, claimed, prepared, judged)
        print("SEMANTIC_JUDGE_EXECUTION_ID=" + claimed["execution_id"])
        print("SEMANTIC_JUDGE_VERDICT=" + str(judged.get("verdict")))
        print("SEMANTIC_JUDGE_VALIDATOR_STATUS=" + str(judged.get("validator_status")))
        print("SEMANTIC_JUDGE_RECORD=" + json.dumps(recorded, ensure_ascii=False, sort_keys=True))
        return True
    except Exception as exc:
        conn.rollback()
        print(f"SEMANTIC_JUDGE_ERROR={type(exc).__name__}:{str(exc)[:800]}", flush=True)
        return claimed is not None
    finally:
        if claimed is not None:
            try:
                with conn.cursor() as cur:
                    cur.execute("select pg_advisory_unlock(pg_catalog.hashtextextended(%s,0))", (claimed["execution_id"],))
                conn.commit()
            except Exception:
                conn.rollback()
        conn.close()
