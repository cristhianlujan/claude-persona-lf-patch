...
def validate_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    all_ids: set[str] = set()
    for phase in PHASES:
        steps = _require_steps(manifest, phase, nonempty=phase in {"preflight", "forward", "tests", "rollback", "post_readback"})
        for index, step in enumerate(steps):
            step_id = step.get("id")
            ...
            if any(x in {"sh", "bash", "zsh", "cmd", "powershell", "pwsh"} for x in argv[:1]) and step.get("allow_shell_interpreter") is not True:
