...
RUNNER_PATH = Path(__file__).resolve().parents[1] / "procedimientos" / "lf_governed_canary_runner.py"
...
def manifest(tmp: Path):
    return {
        ...
        "steps": {
            ...
            "post_readback": [py_step("POST", "print('OK')")],
        },
        "evidence": {"output_path": str(tmp / "evidence.json"), "include_raw_output": False, "require_post_readback": True, "require_zero_residue": True},
    }
...
class RunnerTests(unittest.TestCase):
    ...
    def test_rollback_and_post_run_after_test_failure(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            m["steps"]["post_readback"] = [py_step("POST", f"from pathlib import Path; assert Path(r'{marker}').read_text()=='R'; print('OK')")]
            packet = runner.execute_manifest(m)
