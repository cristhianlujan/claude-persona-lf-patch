...
PROFILE_CODE = "PERFIL-UX-PRODUCT-EXPERIENCE-ARCHITECT-LF-20260531"
...
def validate_package(root: Path = ROOT) -> None:
    required = [
        ...
        "schemas/lf_adapter_invocation.schema.json", "schemas/marketplace_ux_binding.schema.json",
        "validators/validate_marketplace_lf_ux_adapter.py", "evals/run_cases.py",
        "judges/judge_marketplace_lf_ux_adapter.yaml",
    ]
    for rel in required:
    ...
    for rel in ("schemas/lf_adapter_invocation.schema.json", "schemas/marketplace_ux_binding.schema.json"):
...
def validate_result(result: Any) -> None:
    if not isinstance(result, dict) or set(result) != {"state", "marketplace_ux_binding", "lf_adapter_invocations"}:
    ...
    binding = result["marketplace_ux_binding"]
    required = {
        "source_refs", "marketplace_context", "prioritized_frictions", "improvements",
        "protected_constraints", "downstream_dependencies", "blockers",
    }
    ...
    if not isinstance(binding["protected_constraints"], list) or not binding["protected_constraints"]:
        fail("PROTECTED_CONSTRAINTS_REQUIRED")
...
def main() -> int:
    ...
    print("MARKETPLACE_LF_UX_ADAPTER_VALIDATOR_PASS")
    return 0
