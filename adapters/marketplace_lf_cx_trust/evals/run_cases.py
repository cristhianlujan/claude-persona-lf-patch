...
VALIDATOR = ROOT / "validators/validate_marketplace_lf_cx_trust_adapter.py"
spec = importlib.util.spec_from_file_location("cx_validator", VALIDATOR)
module = importlib.util.module_from_spec(spec)
assert spec and spec.loader
spec.loader.exec_module(module)

PROFILE = "PERFIL-CX-TRUST-EXPERIENCE-ARCHITECT-LF-20260531"


def base_result():
    return {
        "state": "BOUND_CANDIDATE_ONLY",
        "marketplace_cx_trust_binding": {
            "source_refs": ["marketplace:promise-1"],
            "claim_boundaries": ["do not claim debt closure without authority"],
            "trust_risks": [{"id": "TRUST-1", "evidence": "supplied"}],
            "improvements": [{"id": "P-1", "status": "PROPOSED_NOT_CANONICAL"}],
            "protected_qualifiers": ["subject to upstream confirmation"],
            "support_dependencies": ["support owner for escalation path"],
            "blockers": [],
        },
        "lf_adapter_invocations": [{
            "invocation_id": "lfai-cx-positive-001",
            "adapter_code": "ADAPTER_MARKETPLACE_LF_CX_TRUST",
            "assurance_revision": "v2",
            "activation_source": "ROUTER",
            "binding_ref": "public.v_lf_router_adapter_bindings:ADAPTER-MARKETPLACE-LF-CX-TRUST-20260531:" + PROFILE,
            "profile_id": PROFILE,
            "target_ref": PROFILE,
            "capsule_ref": "adapters/marketplace_lf_cx_trust/runtime/runtime_capsule.yaml",
            "capsule_char_count": 700,
            "source_refs": ["adapters/marketplace_lf_cx_trust/runtime/runtime_capsule.yaml"],
            "verdict": "APPLIED",
        }],
    }
...
def main() -> int:
    ...
    good = base_result(); module.validate_result(good); count = 1
    ...
    if count != 8:
        raise SystemExit(f"MARKETPLACE_LF_CX_TRUST_CASES_FAIL {count}/8")
    print("MARKETPLACE_LF_CX_TRUST_CASES_PASS 8/8")
    return 0
