...
def main():
    ...
    if not blocking:
        try:
            ...
            if 'BASIC_SKILL_OUTPUT_NOT_ACCEPTABLE' not in bad.get('blocking_codes', []):
                blocking.append('BAD_EXAMPLE_MISSING_BASIC_OUTPUT_BLOCKING_CODE')
            else:
                warnings.append('BAD_EXAMPLE_CORRECTLY_SHOWS_PROSE_ONLY_FAILURE')
    ...
    result = {
        ...
        'warnings': warnings,
        'recommended_action': 'READY_FOR_SANDBOX' if not blocking else 'RETURN_TO_WORKER_FOR_SELF_REPAIR'
    }
