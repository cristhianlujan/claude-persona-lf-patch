...
checks = {
    ...
    'completion_requires_clean_recorded_status': 'recordedStepIsClean' in RUNTIME and 'row.status === binding.clean_result_value' in RUNTIME,
    'blocked_recorded_step_remains_current': 'if (!s.required || recordedStepIsClean(s.step_id)) continue;' in RUNTIME,
    'remaining_required_uses_clean_status_not_row_presence': 'steps.filter((s: any) => s.required && !recordedStepIsClean(s.step_id)).length' in RUNTIME,
    'presence_only_completion_removed': 'rec.has(s.step_id)' not in RUNTIME,
