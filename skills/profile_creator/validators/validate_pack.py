...
def run(command):
    ...
    if completed.stdout: print(completed.stdout, end='' if completed.stdout.endswith('\n') else '\n')
...
def discover_profile_validators(repo_root: Path):
    profiles_root=(repo_root/'profiles').resolve(); discovered=[]; errors=[]; seen=set()
    if not profiles_root.is_dir(): return discovered,['PROFILES_ROOT_MISSING']
    for profile_dir in sorted(profiles_root.iterdir(), key=lambda p:p.name):
        if not profile_dir.is_dir() or profile_dir.name.startswith('_'): continue
        validator=profile_dir/'validators'/'validate_pack.py'
        if not validator.exists(): continue
        if validator.is_symlink(): errors.append(f'PROFILE_VALIDATOR_SYMLINK_FORBIDDEN:{profile_dir.name}'); continue
        resolved=validator.resolve()
        if profiles_root not in resolved.parents: errors.append(f'PROFILE_VALIDATOR_OUTSIDE_PROFILES_ROOT:{profile_dir.name}'); continue
        if resolved in seen: errors.append(f'PROFILE_VALIDATOR_DUPLICATE_TARGET:{profile_dir.name}'); continue
        seen.add(resolved); discovered.append((profile_dir.name,resolved,profile_dir.resolve()))
    return discovered,errors

def main():
    root=Path(sys.argv[1]).resolve() if len(sys.argv)>1 else Path.cwd().resolve(); repo_root=root.parent.parent
    checks=[
        ...
        ('PROFILE_UPDATE_LIFECYCLE_RECONCILIATION',[sys.executable,str(root/'evals/profile_update_lifecycle_reconciliation_v1.py')]),
