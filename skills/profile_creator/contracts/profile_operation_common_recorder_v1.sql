...
-- SOURCE_CANONICAL / LIVE_V1_MATERIALIZED / UPDATE_PREWRITE_SERVER_TRUST_ENABLED
...
begin
  ...
  if v_existing_retryable then update public.lf_operation_execution_steps set status=v_binding.clean_result_value,evidence_ref=p_evidence_ref,evidence_payload=v_payload,notes='Clean retry accepted transactionally; prior blocked attempts preserved in attempt_history.' where execution_id=p_execution_id and step_id=p_step_id; else insert into public.lf_operation_execution_steps(execution_id,step_order,step_id,status,evidence_ref,evidence_payload,notes,created_by_execution_id) values(p_execution_id,v_step.step_order,p_step_id,v_binding.clean_result_value,p_evidence_ref,v_payload,'Recorded transactionally by common governed Profile operation recorder.',p_actor_execution_id); end if;
  return jsonb_build_object('outcome','STEP_RECORDED','replay',false,'execution_id',p_execution_id,'operation_code',v_execution.operation_code,'step_id',p_step_id,'step_order',v_step.step_order,'execution_order',v_step.execution_order,'status',v_binding.clean_result_value,'mini_judge_code',v_binding.judge_code,'mini_judge_result',v_binding.clean_result_value,'next_gate',v_contract.next_if_pass,'resumed_from_blocked',v_existing_retryable,'prior_attempt_count',jsonb_array_length(v_attempt_history));
end;
$function$;

-- CREACION_PERFIL_LF keeps its existing recorder entrypoint.
-- ACTUALIZACION_PERFIL_LF pre-write is accepted only with server-derived trust context from run-creacion-perfil-lf.
-- RPC remains service-role only in production.