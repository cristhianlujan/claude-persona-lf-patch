...
def validate(payload):
    ...
    if declared_key != computed_key:
        return result(
            "BLOCK", "BLOCK_CAUSAL_LANE_KEY_MISMATCH", "RETURN_TO_ROUTER",
            {"declared": declared_key, "computed": computed_key},
        )
    ...
    if not currentness_ok:
        return result(
            "BLOCK", "BLOCK_FRESH_MAIN_CURRENTNESS_REQUIRED", "WAITING_UPSTREAM",
            {"base_main_sha": base_sha, "current_main_sha": current_sha},
        )
    ...
    if len(distinct) > 1:
        return result(
            "BLOCK", "BLOCK_CAUSAL_LANE_CARDINALITY_BREACH", "WAITING_UPSTREAM",
            {"active_writer_count": len(distinct)},
        )
    ...
    if active:
        ...
        if not (same_owner and same_execution):
            return result(
                "BLOCK", "BLOCK_CAUSAL_LANE_ALREADY_OWNED", "WAITING_UPSTREAM",
                {"current_owner": existing.get("owner"), "current_execution_id": existing.get("execution_id")},
            )
