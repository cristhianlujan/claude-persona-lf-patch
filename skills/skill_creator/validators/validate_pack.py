    ...
    if not blocking:
        try:
            ...
            for name, obj in [('good', good), ('self_repair', repair)]:
                for field in ['status','skill_pack_id','source_authority','deliverable_created','files_created','evidence_map','blocking_codes','next_gate']:
                    if field not in obj:
            ...
            if 'BASIC_SKILL_OUTPUT_NOT_ACCEPTABLE' not in bad.get('blocking_codes', []):
                ...
                warnings.append('BAD_EXAMPLE_CORRECTLY_SHOWS_PROMPT_ONLY_FAILURE')
