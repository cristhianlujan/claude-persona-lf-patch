...
function validateIJson(value, path='$') {
  if (value===null || typeof value==='boolean' || typeof value==='string') {
    if (typeof value==='string' && hasLoneSurrogate(value)) throw new Error(`RFC8785_LONE_SURROGATE:${path}`);
    return;
  }
  if (typeof value==='number') {
    if (!Number.isFinite(value)) throw new Error(`RFC8785_NON_FINITE_NUMBER:${path}`);
    return;
  }
  if (Array.isArray(value)) { value.forEach((v,n)=>validateIJson(v,`${path}[${n}]`)); return; }
  if (typeof value==='object') {
    for (const k of Object.keys(value)) {
      if (hasLoneSurrogate(k)) throw new Error(`RFC8785_LONE_SURROGATE_KEY:${path}`);
      validateIJson(value[k],`${path}.${k}`);
    }
    return;
  }
  throw new Error(`RFC8785_UNSUPPORTED_TYPE:${path}:${typeof value}`);
}

function canonicalize(value) {
  validateIJson(value); let out='';
  function serialize(v) {
    if (v===null || typeof v!=='object') {
      const s=JSON.stringify(v); if (s===undefined) throw new Error('RFC8785_UNSERIALIZABLE'); out+=s;
    } else if (Array.isArray(v)) {
      out+='['; for (let n=0;n<v.length;n++) { if(n) out+=','; serialize(v[n]); } out+=']';
    } else {
      out+='{'; const keys=Object.keys(v).sort();
      keys.forEach((k,n)=>{ if(n) out+=','; out+=JSON.stringify(k)+':'; serialize(v[k]); }); out+='}';
    }
  }
  serialize(value); return out;
}
