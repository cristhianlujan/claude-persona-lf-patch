...
### P0HR responsive review shell

When `HUMAN_REVIEW_READY=true`, presentation to the adjudicator MUST use the repository-owned `P0 HUMAN REVIEW SHELL V4` generated by `scripts/build_p0_human_review_html_v4.py` / `scripts/p0_human_review_shell_v4.py`. The shell is part of this agent capability, not an external ad-hoc dashboard. It must remain responsive: vertical scroll is always available; desktop/tablet use sticky navigation and evidence panels; mobile provides horizontally scrollable tabs and bounded swipe navigation without intercepting vertical scrolling.

The shell must expose source image + overlays, element list/search/filters, source-vs-system comparison, evidence refs/crops when present, semantic/visual/geometry/reconciliation detail, problems/omissions, coverage summary, challenge identity/expiry/binding state, and all seven governed human actions. It must fail closed for missing/expired challenge, `HUMAN_REVIEW_READY=false`, or evidence-binding failure. It MAY prepare a copyable `challenge_id=... action=...` command, but MUST NOT authenticate, post, infer, or persist a human decision; authenticated provider readback and append-only persistence remain separate governed steps.

For a P0 V4 closed-loop real-source rerun, the shell MUST consume the fresh rerun receipt directly (`--rerun-receipt`) and select the reader output bound to the final pass; demo/synthetic fixtures are test-only and cannot substitute for the adjudication view. The exact source image bytes supplied to the shell MUST hash to the receipt `source_sha256` or rendering fails closed. The view must surface the final candidate SHA, pass/reader execution, independent coverage, retained reader uncertainties and declared detector limitations.
