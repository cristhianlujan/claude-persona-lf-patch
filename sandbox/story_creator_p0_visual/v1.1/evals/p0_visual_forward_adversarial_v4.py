...
def blocked(label,c,cat=None,sweep=None):
 f,cov,_,_=run(c,sweep);cats={x['category'] for x in f};assert (cat in cats if cat else any(x['severity'] in MATERIAL_SEVERITIES for x in f) or not cov['coverage_pass']),(label,cats,cov);return label
def clean(label,c):
...
def main():
 ...
 c=C(E());checks.append(blocked('PARTIAL_INDEPENDENT_SWEEP',c,'OMISSION_SWEEP_INCOMPLETE',S(c,'INCOMPLETE')))
