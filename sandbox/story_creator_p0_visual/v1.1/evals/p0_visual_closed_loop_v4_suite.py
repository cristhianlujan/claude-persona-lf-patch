...
def main():
 ...
 binding=convergence_receipt_binding(r['convergence_receipt']);assert binding['canonical_sha256']==binding['canonical_bytes_sha256'] and binding['canonical_bytes_length']>0;checks.append('CONVERGENCE_CANONICAL_BYTES_DIGEST_PUBLISHED')
 ...
 def saturated(path,sha,candidate,*,execution_id):
  s=blank_sweep(path,sha,candidate,execution_id=execution_id);s['status']='BLOCKED';s['errors']=['SWEEP_UNIVERSE_TRUNCATED'];s['object_sweep'].update({'raw_count':501,'deduped_count':501,'emitted_count':500,'limit':500,'truncated':True});s['observations']=[{'observation_id':f'OBS-O-{i:04d}','detector':'TEST_FIXTURE','kind':'VISUAL_OBJECT','classification':'INFERRED','material':False,'text':None,'confidence':.7,'region':{'x':0,'y':0,'width':1,'height':1},'pixel_sha256':'a'*64,'evidence_refs':['r'],'match_status':'NON_MATERIAL','matched_element_id':None,'match_score':0.0} for i in range(500)];return s
