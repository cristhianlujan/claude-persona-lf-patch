...
def proofs(sha):
 return {
  'regression_proof':make_gate_proof(gate='regression_suite',source_sha256=sha,code_head_sha=H40,configuration_sha256=CFG,details={'fixture':'closed_loop_suite','executed_cases':1}),
  'adversarial_proof':make_gate_proof(gate='adversarial_suite',source_sha256=sha,code_head_sha=H40,configuration_sha256=CFG,details={'fixture':'closed_loop_suite','executed_cases':1}),
  'artifact_hash_proof':make_gate_proof(gate='artifact_hash_chain',source_sha256=sha,code_head_sha=H40,configuration_sha256=CFG,details={'fixture':'closed_loop_suite','bound':True}),
 }
def base(reader,**kw):
 p,sha=source();gate_proofs=proofs(sha);gate_proofs.update({k:kw.pop(k) for k in list(kw) if k in gate_proofs});r=run_loop(source_path=str(p),expected_source_sha256=sha,full_reader=reader,remediator=kw.pop('remediator',remediate),targeted_reread=kw.pop('targeted_reread',targeted),code_head_sha=H40,configuration_id='P0-V4',configuration_sha256=CFG,omission_sweep_runner=kw.pop('omission_sweep_runner',blank_sweep),**gate_proofs,**kw);p.unlink();return r
def main():
 checks=[];calls=[]
 ...
 p,sha=source();bad=make_gate_proof(gate='regression_suite',source_sha256=sha,code_head_sha=H40,configuration_sha256=CFG,details={'forced_failure':True},passed=False,exit_code=2);p.unlink();r=base(lambda p,c:candidate(False),regression_proof=bad);assert r['result']=='BLOCKED_CONVERGENCE' and not r['human_review_ready'];checks.append('ONE_GATE_FAILS_HUMAN_READY')
 ...
 r=run_loop(source_path=str((lambda x:(x.write_bytes(b'proof-required'),x)[1])(Path(tempfile.mkstemp()[1]))),expected_source_sha256='0'*64,full_reader=lambda p,c:candidate(False),remediator=remediate,targeted_reread=targeted,code_head_sha=H40,configuration_id='P0-V4',configuration_sha256=CFG,omission_sweep_runner=blank_sweep);assert not r['human_review_ready'];checks.append('NO_IMPLICIT_GATE_PROOFS')
