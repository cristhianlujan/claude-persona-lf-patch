...
from p0_visual_real_rerun_support_v4 import remediator,_apply_remediation_state
H='a'*64
...
def C(e):return {'width':300,'height':100,'fresh_source_read':True,'reader_origin':'SOURCE_PIXELS','uncertainties':[],'elements':[{'element_id':'ROOT','element_type':'CONTAINER','visible_text':None,'classification':'CONFIRMED','confidence':1,'region':{'x':0,'y':0,'width':300,'height':100},'parent_id':None,'evidence_refs':['synthetic://full'],'bbox_reproducible':True,'style':{},'style_provenance':{},'independent_redetection':True},e]}
...
def main():
 ...
 region={'x':120,'y':30,'width':24,'height':18};candidate=C(E('27',classification='INFERRED',independent_redetection=False,ocr_consensus_text='',ocr_variants=['27','27'],ocr_read_count=3,ocr_empty_reads=1,ocr_agreement_count=2,region=region));finding={'category':'SHORT_TEXT_WITHOUT_MATERIAL_SUPPORT','region':region}
 state,actions=remediator(candidate,[finding],{});assert actions==[{'category':'SHORT_TEXT_WITHOUT_MATERIAL_SUPPORT','region':region,'action':'RECLASSIFY_UNSUPPORTED_SHORT_TEXT','affected_findings':1}],actions;checks.append('UNSUPPORTED_SHORT_TEXT_GETS_STATE_TRANSITION')
 remediated=_apply_remediation_state(candidate,state);element=remediated['elements'][1];assert element['visible_text'] is None and element['element_type']=='ICON_OR_GLYPH' and element['classification']=='INFERRED' and element['remediation_disposition']['original_visible_text']=='27',element;assert 'SHORT_TEXT_WITHOUT_MATERIAL_SUPPORT' not in cats(remediated);checks.append('UNSUPPORTED_SHORT_TEXT_RECLASSIFIES_WITH_LINEAGE')
 confirmed_same=_apply_remediation_state(C(E('27',classification='CONFIRMED',region=region)),state);assert confirmed_same['elements'][1]['visible_text']=='27' and confirmed_same['elements'][1]['element_type']=='TEXT';checks.append('CONFIRMED_SHORT_TEXT_NOT_RECLASSIFIED')
 alpha=_apply_remediation_state(C(E('ok',classification='INFERRED',region=region)),state);assert alpha['elements'][1]['visible_text']=='ok' and alpha['elements'][1]['element_type']=='TEXT';checks.append('ALPHABETIC_SHORT_TEXT_NOT_RECLASSIFIED')
 restored_state,restore_actions=remediator(candidate,[{'category':'MATERIAL_OMISSION','region':region}],state);assert 'unsupported_short_text_regions' not in restored_state and restore_actions[0]['action']=='RESTORE_MATERIAL_TEXT_IF_SUPPRESSED';checks.append('MATERIAL_OMISSION_RELEASES_STALE_SUPPRESSION')
 print(json.dumps({'gate':'PASS_V4_HUMAN_FINDINGS_AS_REGRESSIONS','checks':len(checks),'known_classes_detected':10,'unseen_holdout_detected':1,'restores':4,'results':checks},sort_keys=True));return 0
