...
def synthetic_campaign_fixture() -> tuple[dict, dict]:
    elements = [{"element_id": "V4-ROOT", "element_type": "CONTAINER", "parent_id": None, "region": region(0, 0, 5000, 1500)}]
    elements.extend(text_element(index, 50 + index * 300, f"material text {index}") for index in range(1, 13))
    for index in range(1, 7):
        elements.append({
            "element_id": f"I-{index:02d}", "element_type": "ICON", "visible_text": None,
            "classification": "CONFIRMED", "region": region(200 + index * 400, 900, 24, 24),
            "crop_region": region(200 + index * 400, 900, 24, 24), "crop_sha256": f"ICON-CROP-{index}",
            "parent_id": "V4-ROOT",
        })
    ...
    for index, element in enumerate(elements[1:], 1):
        ...
        observations.append({
            "observation_id": f"OBS-{index:03d}", "kind": "COMPACT_VISUAL" if compact else "TEXT",
            "material": True, "text": None if compact else element["visible_text"],
            "region": copy.deepcopy(element["region"]), "match_status": "REPRESENTED",
            "matched_element_id": element["element_id"], "control_type": None, "repeated_control_group_id": None,
        })
...
def main() -> int:
    ...
    checks["INV2_SHARED_TOKEN_BLOCKS"] = any(issue["code"] == "SHARED_EVIDENCE_VIOLATION" for issue in exclusive_partition_issues(shared))
    ...
    checks["INV2_UNJUSTIFIED_PARTITION_BLOCKS"] = any(issue["code"] == "UNJUSTIFIED_PARTITION" for issue in exclusive_partition_issues(split))
    ...
    source_text = [{"text": "choice", "region": region(75, y, 80, 20)} for y in (40, 90, 150)] + [{"text": "status", "region": region(350, 95, 80, 20)}]
    ...
    for index, item in enumerate(controls, 1):
        observations.append({
            "observation_id": f"CONTROL-{index}", "material": True, "control_type": "CHECKBOX",
            "repeated_control_group_id": "GROUP-1", "match_status": "REPRESENTED" if index < 3 else "UNREPRESENTED",
    ...
    checks["F02_R07_400_OF_400_MUTATIONS"] = (
        ...
        and all(campaign["family_detected_counts"].get(family) == REQUIRED_FAMILY_COUNT for family in REQUIRED_FAMILIES)
    )
