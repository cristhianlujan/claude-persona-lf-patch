...
def C(e):return {'width':500,'height':300,'elements':[{'element_id':'ROOT','element_type':'CONTAINER','visible_text':None,'classification':'CONFIRMED','confidence':1.0,'region':{'x':0,'y':0,'width':500,'height':300},'parent_id':None,'evidence_refs':['region://root'],'bbox_reproducible':True,'style':{},'style_provenance':{},'independent_redetection':True},e],'coverage_map':[{'region_id':'FULL','material':True,'observed_count':2,'represented_count':2,'sweep_status':'COMPLETE'}]}
def run(c,passid='P-01'):
 ctx={'cycle_id':'C-01','pass_id':passid,'reader_execution_id':'R-'+passid,'source_sha256':H,'candidate_sha256':canonical_sha(c),'coverage_execution_id':'JC-'+passid};outs=run_all(c,ctx);return union_findings(outs),coverage_receipt(c,outs,ctx)
...
def main():
 ...
 _,cov=run(C(E()));assert cov['coverage_pass'] and cov['coverage_percent']==100.0;checks.append('coverage_green_restore');print(json.dumps({'gate':'PASS_V4_GRADERS','checks':len(checks),'known_failure_classes':8,'restores':4,'coverage_pass':True,'results':checks},sort_keys=True));return 0
