...
def run_loop(*,source_path,expected_source_sha256,full_reader,remediator,targeted_reread,code_head_sha,configuration_id,configuration_sha256,max_remediation_cycles=5,required_clean_passes=2,grader_runner=run_all,id_factory=default_id,regression_suite='PASS',adversarial_suite='PASS',artifact_hash_chain='PASS'):
 ...
 clean=remediations=pass_no=0;state={};cycles=[];clean_receipts=[];seen_readers=set();seen_graders=set()
 while pass_no<max_remediation_cycles+required_clean_passes+3:
  ...
  seen_readers.add(rex);rctx={'cycle_id':cid,'pass_id':pid,'reader_execution_id':rex,'source_sha256':actual,'remediation_state':json.loads(json.dumps(state))};cand=full_reader(source_path,rctx)
  ...
  cand=dict(cand);cand.pop('previous_candidate',None);cand['reader_execution_id']=rex;csha=canonical_sha({k:v for k,v in cand.items() if k!='reader_execution_id'});gctx={'cycle_id':cid,'pass_id':pid,'reader_execution_id':rex,'source_sha256':actual,'candidate_sha256':csha,'coverage_execution_id':id_factory('COVERAGE',pid,pass_no)};outs=grader_runner(cand,gctx);ids=[o.get('execution_id') for o in outs]
  ...
  seen_graders.update(ids);cov=coverage_receipt(cand,outs,gctx);fs=union_findings(outs);material=[f for f in fs if f.get('severity') in MATERIAL_SEVERITIES and f.get('status')=='OPEN'];cycle={'schema_version':'p0-loop-cycle-v4/v1','cycle_id':cid,'pass_id':pid,'reader_execution_id':rex,'source_sha256':actual,'candidate_sha256':csha,'grader_execution_ids':ids,'coverage_receipt_sha256':canonical_sha(cov),'finding_counts':counts(fs),'material_findings':len(material),'remediation_applied':False,'targeted_reread_sha256':None,'forced_full_reread_required':False,'clean_pass_count_after':clean,'result':'CLEAN_CONTINUE'}
  if not cov['coverage_pass']:cycle['result']='BLOCKED_DISCOVERY_COVERAGE';cycles.append(cycle);return {'result':'BLOCKED_DISCOVERY_COVERAGE','human_review_ready':False,'cycles':cycles,'coverage':cov}
  if material:
   ...
   tctx={'cycle_id':cid,'pass_id':pid,'source_sha256':actual,'candidate_sha256':csha,'reader_execution_id':rex};targeted=targeted_reread(source_path,actions,tctx)
