...
def run_loop(*,source_path,expected_source_sha256,full_reader,remediator,targeted_reread,code_head_sha,configuration_id,configuration_sha256,max_remediation_cycles=5,required_clean_passes=2,grader_runner=run_all,omission_sweep_runner=run_independent_omission_sweep,id_factory=default_id,regression_suite='PASS',adversarial_suite='PASS',artifact_hash_chain='PASS'):
 ...
 while pass_no<max_remediation_cycles+required_clean_passes+3:
  ...
  # A detector-level ERROR/BLOCKED (including SWEEP_UNIVERSE_TRUNCATED) is terminal.
  # It is not a candidate defect and therefore must never be routed through remediation cycles.
  ...
  if not cov['coverage_pass']:
   hard_errors=[e for e in cov.get('grader_errors',[]) if e.get('error')!='INDEPENDENT_SCREEN_COVERAGE_INCOMPLETE']
   candidate_complete=bool((cov.get('candidate_grader_coverage') or {}).get('complete'))
   # Genuine pixel-derived omissions/support contradictions may enter remediation.
   # Missing/invalid/error sweeps, grader failures, or candidate-grader gaps remain fail-closed.
