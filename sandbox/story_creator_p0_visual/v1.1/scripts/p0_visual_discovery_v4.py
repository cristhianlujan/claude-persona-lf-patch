...
def coverage_receipt(candidate:dict,outputs:list[dict],ctx:dict)->dict:
 ...
 for g in [x for x in GRADERS if x not in by]:errors.append({'grader_id':g,'error':'MISSING_GRADER_OUTPUT'})
 execs=[]
 for g,o in by.items():
 ...
 regions=[];material=[r for r in candidate.get('coverage_map',[]) if r.get('material',True)] or [{'region_id':'FULL'}];sweeps=set(by.get('J-COMPLETE',{}).get('screen_regions_evaluated') or [])
 for r in material:
  rid=str(r.get('region_id','FULL'));status='COMPLETE' if rid in sweeps or 'FULL' in sweeps else 'INCOMPLETE'
