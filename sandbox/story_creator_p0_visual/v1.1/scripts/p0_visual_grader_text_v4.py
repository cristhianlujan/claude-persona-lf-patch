...
def j_text(candidate:dict,ctx:dict)->dict:
 ...
 for e in els:
  ...
  txt=str(e.get('visible_text') or '').strip();variants=[str(x).strip() for x in e.get('ocr_variants',[]) if str(x).strip()];consensus=str(e.get('ocr_consensus_text') or '').strip();graphic=float(e.get('graphic_score',0) or 0);read_count=int(e.get('ocr_read_count',len(e.get('ocr_variants',[]) or [])) or 0);empty_reads=int(e.get('ocr_empty_reads',0) or 0);consensus_disposition=str(e.get('ocr_consensus_disposition') or '').strip();consensus_usable=consensus_disposition!='REJECTED_AS_INDEPENDENT_SAME_FAMILY'
  ...
  if txt and consensus and txt!=consensus and consensus_usable:
