...
def main()->int:
 ...
 c=enrich_candidate(legacy,image,cfg);reparented=repair_inferred_parent_containment(c,cfg);c,h=remediate_visual_fidelity(c,image,cfg,int(cfg.get('quality',{}).get('max_remediation_cycles',3)))
 if reparented:
  if not h:h=[{'cycle':0,'strategies':[],'result':'PENDING_VALIDATION'}]
  h[0]['strategies']=sorted(set(h[0].get('strategies',[])+['INFERRED_PARENT_REASSIGNMENT','GEOMETRY_RECOMPUTE_AFTER_REPARENT']));h[0]['reparented_element_ids']=reparented
 rep=validate_visual_fidelity(c,None,cfg);packet=human_review_packet(c,rep,None,h);html=build_human_html(packet,c,image)
