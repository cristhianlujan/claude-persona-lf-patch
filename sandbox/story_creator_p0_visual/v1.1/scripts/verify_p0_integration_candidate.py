...
def main()->int:
 ...
 evidence_raw=os.environ.get('P0_INTEGRATION_GATE_EVIDENCE_DIR','').strip();evidence_dir=Path(evidence_raw).resolve() if evidence_raw else None
 if evidence_dir is not None:
  evidence_dir.mkdir(parents=True,exist_ok=True)
  for old in evidence_dir.glob('*.log'):old.unlink()
 def record_gate(index:int,gate:str,p:subprocess.CompletedProcess[str])->None:
  if evidence_dir is None:return
  safe=''.join(ch if ch.isalnum() or ch in '._-' else '_' for ch in gate)
  output=(p.stdout or '')+(('\nSTDERR:\n'+p.stderr) if p.stderr else '')
  (evidence_dir/f'{index:02d}-{safe}.log').write_text(output,encoding='utf-8')
 gate_index=0
 for script,*args in SCRIPT_GATES:
  p=subprocess.run([sys.executable,str(ROOT/'scripts'/script),*args],text=True,capture_output=True,env=env);gate_index+=1;record_gate(gate_index,'scripts__'+script,p)
 ...
 for script in EVAL_GATES:
  p=subprocess.run([sys.executable,str(ROOT/'evals'/script)],text=True,capture_output=True,env=env);gate_index+=1;record_gate(gate_index,'evals__'+script,p)
 ...
 evidence_count=len(list(evidence_dir.glob('*.log'))) if evidence_dir is not None else 0
 ...
 checks={'inventory_exact':actual_set==declared_set,'hashes_exact':not mismatches,'symlink_count_zero':not symlinks,'architecture_source_pinned':base.get('architecture_source_sha256')==ARCHITECTURE_SHA256,'canonicalizer_identity_exact':canonicalizer.is_file() and sha256(canonicalizer)==CANONICALIZER_SHA256,'visual_quality_extension_version':ext.get('schema_version')=='p0-visual-quality-inventory-extension/v1','visual_fidelity_extension_version':v3.get('schema_version')=='p0-visual-fidelity-manifest/v3','v3_baseline_bound':v3.get('baseline_main_sha')=='26611cad05c2986b367ed55d3f38a395d4a3cc0c','v3_production_not_authorized':v3.get('production_authorized') is False,'v4_inventory_marker_version':v4.get('schema_version')=='p0-closed-loop-manifest-v4.2/atomicity-invariants-v1','v4_human_findings_retained':v4.get('audit_state')=='P0_HUMAN_FINDINGS_CONFIRMED','v4_retroactive_invalidation':v4.get('retroactive_invalidation_policy')=='REJECT_PASS_FROM_INVALIDATED_LOOP_VERSION','v4_empirical_readiness_blocked':str(v4.get('empirical_readiness','')).startswith('BLOCKED_'),'v4_production_not_authorized':v4.get('production_authorized') is False,'v4_exact_blob_inventory':v4.get('inventory_mode')=='EXACT_GIT_BLOB_SHA' and len(v4_paths)>=40 and {'scripts/p0_independent_omission_sweep_v4.py','scripts/p0_visual_atomicity_v4.py','scripts/p0_visual_mutation_campaign_v4.py','scripts/p0_visual_residual_v4.py','evals/p0_visual_atomicity_invariants_v4.py','evals/p0_reader_producer_contract_v4.py','evals/p0_grader_producer_field_audit_v4.py'}<=v4_paths,'p0_5_remains_separate':ext.get('p0_5_state')=='BLOCKED_BENCHMARK','legacy_compatibility_readable':legacy['pass'],'all_current_gates_pass':all(r['exit_code']==0 for r in gate_results),'gate_evidence_complete':evidence_dir is None or evidence_count==len(SCRIPT_GATES)+len(EVAL_GATES)}
 passed=all(checks.values());report={'result':'PASS_WITH_EVIDENCE' if passed else 'BLOCKED','checks':checks,'legacy_compatibility':legacy,'declared_file_count':len(declared_set),'actual_file_count':len(actual_set),'missing':sorted(declared_set-actual_set),'unexpected':sorted(actual_set-declared_set),'hash_mismatches':mismatches,'symlinks':symlinks,'gate_results':gate_results,'gate_evidence_log_count':evidence_count};print(json.dumps(report,sort_keys=True))
 if passed:print(f'PASS_P0_INTEGRATION_GATE_MATRIX={len(gate_results)}/{len(SCRIPT_GATES)+len(EVAL_GATES)}')
