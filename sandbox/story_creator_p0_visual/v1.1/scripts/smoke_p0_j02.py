...
def _local_j02_schema_errors(dec: dict) -> tuple[list[str], str]:
    """Use the same fail-closed local schema subset as the P0 gates."""
    path = j02.schema_path()
    if not path.is_file():
        raise j02.ValidationInputError("input_schema_unavailable")
    schema = j02.obj(j02.load_json(path), "screen_decomposition_schema")
    return validate_instance(schema, dec), j02.sha256_file(path)
...
def main() -> int:
    j02.schema_errors = _local_j02_schema_errors
