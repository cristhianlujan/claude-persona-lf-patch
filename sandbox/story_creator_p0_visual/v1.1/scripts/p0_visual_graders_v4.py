...
def j_text(candidate:dict,ctx:dict)->dict:
 ...
 for e in els:
  ...
  txt=str(e.get('visible_text') or '').strip();variants=[str(x).strip() for x in e.get('ocr_variants',[]) if str(x).strip()];consensus=str(e.get('ocr_consensus_text') or '').strip();graphic=float(e.get('graphic_score',0) or 0)
  if txt and len(txt)<=3 and e.get('classification')=='CONFIRMED' and (graphic>=.60 or len(set(variants))>1):fs.append(finding(ctx,g,'SHORT_TOKEN_UNCORROBORATED','HIGH',e,{'text':txt,'graphic_score':graphic,'ocr_variants':variants},'REREAD',.94,'short-token-evidence-policy'))
  if txt and consensus and txt!=consensus:
   if strip_marks(txt).casefold()==strip_marks(consensus).casefold():fs.append(finding(ctx,g,'DIACRITIC_MISMATCH','MEDIUM',e,{'candidate':txt,'independent_consensus':consensus},'AUTO_REMEDIATE',.96,'diacritic-preservation'))
   elif consensus in txt and not txt.startswith(consensus) and len(txt)-len(consensus)<=4:fs.append(finding(ctx,g,'OCR_PREFIX_GARBAGE','HIGH',e,{'candidate':txt,'independent_consensus':consensus},'AUTO_REMEDIATE',.94,'unsupported-prefix'))
...
def j_skeptic(candidate:dict,ctx:dict)->dict:
 ...
 if prev and prev==ctx['candidate_sha256']:fs.append(finding(ctx,g,'FULL_PASS_CANDIDATE_REUSED','HIGH',None,{'previous_candidate_sha256':prev,'candidate_sha256':ctx['candidate_sha256']},'BLOCK',.99,'context-reset-failure'))
 for e in els:
