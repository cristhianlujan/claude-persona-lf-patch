...
begin
  ...
  -- A. Preserve the insert authority guard, but let SCREEN_CANONICAL_GRAPH
  -- consume the exact request-local graph already read by the Curator.
  -- All other source refs continue through the canonical resolver unchanged.
  ...
  if position('perform programacion.fn_input_resolve_source_ref(v_ref,v_pantalla_id,v_version_id);' in v_def)=0 then
    raise exception 'S28_IG007_INSERT_GUARD_RESOLVER_ANCHOR_DRIFT';
  end if;
  v_def:=replace(v_def,
  ...
  -- C. Assertion-set builder using the same request-local graph.
  ...
  -- D. Candidate Curator rebind. Original runtime entrypoint remains untouched.
  ...
  -- Forced self-test explicitly requests a new successor; do not compute
  -- currentness only to ignore it. Normal calls retain fail-closed currentness.
  v_def:=replace(v_def,
  ...
  -- One canonical graph is published only for this transaction/request and is
  -- identity-bound before the insert triggers are allowed to reuse it.
  v_def:=replace(v_def,
...
begin
  ...
  if position('fn_input_v58_build_assertions_cached_v1' in v_candidate)=0
     ...
     or position('lf.input_screen_canonical_graph_v1' in v_candidate)=0
     or position('lf.input_screen_canonical_graph_v1' in v_guard)=0 then
