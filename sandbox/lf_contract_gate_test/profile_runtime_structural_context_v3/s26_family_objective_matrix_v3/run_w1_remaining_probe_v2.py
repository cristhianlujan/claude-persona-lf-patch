...
def run(cmd: list[str], *, cwd: Path | None = None, check: bool = True, env: dict[str, str] | None = None) -> subprocess.CompletedProcess[str]:
    return subprocess.run(cmd, cwd=cwd or REPO, text=True, capture_output=True, check=check, env=env)
...
ROOT = Path.cwd()
SOURCE_AUTHORITY_COMMIT = os.environ["S26_SOURCE_AUTHORITY_COMMIT"]
RESULTS = {}
...
def put(case_id, passed, observed, elapsed_ms, token_proxy, evidence_refs):
    RESULTS[case_id] = {
        "pass": bool(passed), "observed": observed, "latency_ms": round(elapsed_ms, 3),
        "model_calls": 0, "context_or_token_proxy": int(token_proxy), "retries": 0,
...
def git_json(commit: str, path: str):
    cp = subprocess.run(["git", "show", f"{commit}:{path}"], cwd=ROOT, text=True, capture_output=True)
    if cp.returncode != 0:
        raise RuntimeError(f"FROZEN_ORACLE_READBACK_FAILED:{commit}:{path}:{cp.stderr[-300:]}")
    return json.loads(cp.stdout)

resolver_path = ROOT / "sandbox/lf_contract_gate_test/profile_execution_runtime/runtime_authority_resolver_v1.py"
resolver = load(resolver_path, "s26_w1_remaining_resolver_v2")
fixture_dir = ROOT / ".s26_matrix_w1_remaining_v2"
...
def base_context(surface, task, run_id, required=None, sources=None):
    return {
        "surface_code": surface, "task_code": task, "current_run_id": run_id,
        "input_fields": {"profile_slug": "ui_architect", "output_contract_version": "UI_PRODUCTION_SPEC_V6"},
        "card_candidates": [], "required_authority_types": required or [],
        "authority_sources": sources or [], "required_adapter_codes": [],
    }

# F01-O5 — unseen routing/provenance surface: no route memorization; unknown authority fails closed.
...
route_ref = ".s26_matrix_w1_remaining_v2/unseen_route_authority.json"
...
f01_ok = (
    ...
    and out["card_resolution"]["schema_invention_allowed"] is False and blocked
)
put("S26V3-F01-O5-HOLDOUT", f01_ok, {
...
# F02-O3 — stale bound bytes reject; fresh provider digest must be explicitly rebound.
stale_path = fixture_dir / "stale_rebind_authority.json"
stale_ref = ".s26_matrix_w1_remaining_v2/stale_rebind_authority.json"
...
# F02-O4 — caller-declared currentness is inert; provider-byte readback remains authority.
...
# F02-O5 — absent canonical source fails closed; no synthetic authority/schema fallback.
missing_ref = ".s26_matrix_w1_remaining_v2/canonical_source_does_not_exist.json"
...
# F06 performance-only rebinds — semantic oracle is frozen source-authority Gate H, not candidate-local data.
hp = "sandbox/lf_contract_gate_test/profile_runtime_structural_context_v3/s26_hp001"
manual_path = ROOT / hp / "manual_profile_output.json"
req_path = ROOT / hp / "expected_requirements.json"
base = json.loads(manual_path.read_text(encoding="utf-8"))
req = json.loads(req_path.read_text(encoding="utf-8"))
gate_h = git_json(SOURCE_AUTHORITY_COMMIT, hp + "/gate_h_output.json")
...
f06o2 = base_check["pass"] and not search_check["pass"] and "header" in search_check["missing_sections"] and controls.get("missing_search") == "BLOCKS"
put("S26V3-F06-O2-FINAL-PERF-REBIND", f06o2, {
    "frozen_oracle_commit":SOURCE_AUTHORITY_COMMIT, "frozen_negative_control":"missing_search=BLOCKS",
    "base_valid":base_check["pass"], "mutation_rejected":not search_check["pass"],
    "missing_sections":search_check["missing_sections"], "semantic_oracle_changed":False,
}, bm + fm, proxy(base) + proxy(missing_search), [f"{SOURCE_AUTHORITY_COMMIT}:{hp}/gate_h_output.json", f"{FINAL_SHA}:{hp}/expected_requirements.json", f"{FINAL_SHA}:{hp}/manual_profile_output.json"])
...
put("S26V3-F06-O3-FINAL-PERF-REBIND", f06o3, {
    "frozen_oracle_commit":SOURCE_AUTHORITY_COMMIT,
...
}, bm + pm + prm, proxy(base) + proxy(missing_price) + proxy(missing_prof), [f"{SOURCE_AUTHORITY_COMMIT}:{hp}/gate_h_output.json", f"{FINAL_SHA}:{hp}/expected_requirements.json", f"{FINAL_SHA}:{hp}/manual_profile_output.json"])

print(json.dumps({"schema":"S26_MATRIX_V3_W1_REMAINING_CHILD_RESULTS_V2","results":RESULTS}, sort_keys=True))
'''
...
def main() -> int:
    ...
    authority_commits = {recipes[cid]["source_authority_commit"] for cid in target_ids}
    if len(authority_commits) != 1:
        raise SystemExit("BLOCK_W1_SOURCE_AUTHORITY_AMBIGUOUS")
    source_authority_commit = next(iter(authority_commits))

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="s26-matrix-w1-rem-v2-") as tmp:
        ...
        try:
            ...
            child_env["S26_SOURCE_AUTHORITY_COMMIT"] = source_authority_commit
            child = run([sys.executable, "-c", CHILD], cwd=candidate, check=False, env=child_env)
            if child.returncode != 0:
                (OUT_DIR / "remaining_v2_child_stderr.txt").write_text(child.stderr, encoding="utf-8")
                (OUT_DIR / "remaining_v2_child_stdout.txt").write_text(child.stdout, encoding="utf-8")
                raise SystemExit(f"BLOCK_W1_REMAINING_V2_CHILD_EXECUTION:{child.returncode}")
    ...
    for case_id in sorted(target_ids):
        ...
        receipt = {
            "case_id":case_id, "objective_id":recipe["objective_id"], "family":recipe["family"],
            "source_authority_commit":recipe["source_authority_commit"], "final_candidate_sha":FINAL_SHA,
            "final_wave":recipe["final_wave"], "executor":recipe["executor"],
            "target_component":recipe["target_component"], "oracle":recipe["oracle"],
            "failure_mode":recipe["failure_mode"], "expected":recipe["expected"],
            "observed":observed["observed"], "dimensions_measured":recipe["dimensions_to_close"],
            "evidence_refs":observed["evidence_refs"], "pass":observed["pass"],
            "unseen_holdout":recipe["unseen_holdout"], "independent_case_credit":recipe["independent_case_credit"],
        }
    ...
    summary = {
        "schema":"S26_FAMILY_OBJECTIVE_MATRIX_V3_W1_REMAINING_RECEIPT_V2",
        ...
        "final_candidate_sha":FINAL_SHA, "case_count":len(receipts),
        "pass_count":sum(r["pass"] for r in receipts), "fail_count":len(failed), "failed_cases":failed,
        "receipts":receipts,
    ...
    aggregate = {
        ...
        "final_candidate_sha":FINAL_SHA, "expected_w1_cases":len(all_w1), "observed_w1_cases":len(combined),
        "pass_count":sum(bool(r["pass"]) for r in combined.values()), "fail_count":len(aggregate_failed),
        "missing_cases":missing, "unexpected_cases":unexpected, "failed_cases":aggregate_failed,
        "case_ids":sorted(combined), "model_calls_total":sum(int(r.get("model_calls", 0)) for r in combined.values()),
    ...
    print(json.dumps({
        "remaining_status":summary["status"], "remaining_pass_count":summary["pass_count"],
        "w1_status":aggregate["status"], "w1_expected":aggregate["expected_w1_cases"],
        "w1_observed":aggregate["observed_w1_cases"], "w1_pass_count":aggregate["pass_count"],
        "w1_fail_count":aggregate["fail_count"], "w1_missing":aggregate["missing_cases"],
        "model_calls_total":aggregate["model_calls_total"],
    }, sort_keys=True))
