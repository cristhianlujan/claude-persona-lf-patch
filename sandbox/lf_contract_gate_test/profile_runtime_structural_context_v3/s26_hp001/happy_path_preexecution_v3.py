...
def _load_resolver():
    spec = importlib.util.spec_from_file_location("s26_hp001_runtime_authority_v3", RESOLVER_PATH)
...
def run_preexecution() -> dict[str, Any]:
    ...
    if gate_c_output.get("next_gate") != "D_AUTHORITY_RESOLUTION":
        raise RuntimeError("GATE_C_DID_NOT_AUTHORIZE_STAGE_D")
    if gate_c_output.get("run_id") != gate_b_output.get("run_id"):
        raise RuntimeError("GATE_C_RUN_ID_MISMATCH")
    ...
    runtime_context = {
        ...
        "card_candidates": c_input.get("card_candidates"),
    ...
    if resolved.get("input_literal_sha256") != input_literal_sha256:
        raise RuntimeError("GATE_C_TO_DOWNSTREAM_INPUT_SHA_DRIFT")
    ...
    for key in ("status", "mode", "card_id", "card_ref", "card_sha256", "schema_invention_allowed", "reason"):
        if card_resolution.get(key) != expected_selection.get(key):
            raise RuntimeError(f"GATE_C_TO_RUNTIME_CARD_DIVERGENCE:{key}")
    ...
    return {
        "gate": "S26_HP001_PREEXECUTION_A_E_V3",
        "result": "PASS",
        "upstream_gate_a": {
        ...
        "stage_b": {
            ...
            "input_sha256": gate_b["input_sha256"],
        ...
        "stage_c": {
            "gate": gate_c_output["gate"],
            "status": gate_c_output["status"],
            "run_id": gate_c_output["run_id"],
            "input_sha256": gate_c["input_sha256"],
            "output_sha256": gate_c["output_sha256"],
            "candidate_count": gate_c["candidate_count"],
            "decision": gate_c["decision"],
        },
        "stage_d_input": {
            "source_gate": "C_CARD_SELECTION",
            "source_output_sha256": gate_c["output_sha256"],
            "run_id": gate_c_output["run_id"],
