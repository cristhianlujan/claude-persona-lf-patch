# S26 HP001 — Claude independent quality review — operator v3

## Execution contract

Run this review in a NEW Claude chat/context that did not produce the artifact. Claude is deliberately LAST: execute this only after the GPT receipt has been sealed and validated. Claude MUST NOT see, read, summarize, compare against, or be told the GPT receipt/verdict/score/findings before producing its own receipt. Do not ask the operator for prior chats, prior verdicts, expected scores, or extra explanations. Do not repair the artifact. Do not call paid external APIs/models or download weights.
...
```json
{
  ...
  "quality_review":{
    "review_id":"S26-HP001-COLD-CLAUDE-QUALITY-REVIEW-001",
