...
def select_read_only_context(
    ...
    *,
    ...
    """Select only source-bound KB rows explicitly listed by an exact consumer binding.

    This function is intentionally deterministic and side-effect free. It never expands
    scope from topic similarity and never calls an LLM. New learning becomes consumable
    only after its kb_id is explicitly classified into the exact binding.
    """
