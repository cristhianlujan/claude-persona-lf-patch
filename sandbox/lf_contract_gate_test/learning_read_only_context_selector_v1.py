...
def select_read_only_context(rows: Iterable[dict[str, Any]], *, binding: BindingSpec) -> dict[str, Any]:
    ...
    for kb_id in binding.source_learning_ids:
        ...
        selected.append({"kb_id":kb_id,"kb_category":REQUIRED_KB_CATEGORY,"topic":_text(row.get("topic")),"summary":_text(row.get("summary")),"source_url":_text(row.get("source_url")),"competitor":_text(row.get("competitor")),"quality_score":row.get("quality_score"),"evidence_ref":f"public.lf_knowledge_base/{kb_id}"})
        if len(selected)>=binding.max_evidence_refs: break
    return {"schema":SCHEMA,"mode":"READ_ONLY","selector":"DETERMINISTIC_EXACT_ID","required_kb_category":REQUIRED_KB_CATEGORY,"llm_calls":0,"round_trips":0,"consumer_id":binding.consumer_id,"capability_id":binding.capability_id,"selected_count":len(selected),"selected":selected,"fallback":"NO_COMPETITIVE_CONTEXT" if not selected else None}
