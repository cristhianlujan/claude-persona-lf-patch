...
begin
  d := programacion.fn_v09_sha256_jsonb(jsonb_build_object(
    'schema_version',1,'status',p_status,'query',p_query,'selected',p_selected,
    'missing_critical_context',p_missing,'filtered_counts',p_filtered));
  begin
...
declare
  ...
  n integer := 0;
begin
  -- Positive structural baseline: canonical BLOCKED + [] remains accepted.
  d := programacion.fn_v09_sha256_jsonb(jsonb_build_object(
  ...
  if n <> 11 then raise exception 'OP24_EXPECTED_11_OF_11_GOT_%',n; end if;
end;
$test$;
...
select jsonb_build_object(
  'original_function_restored',md5(pg_get_functiondef('programacion.fn_guard_retrieval_run_insert()'::regprocedure)) = :'op24_original_guard_md5',
  'recent_execution50_rows_after_rollback',(select count(*) from programacion.retrieval_runs where execution_id=50 and created_at > clock_timestamp()-interval '2 minutes')
