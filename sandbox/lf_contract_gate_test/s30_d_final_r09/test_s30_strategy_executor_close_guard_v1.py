...
def main() -> int:
    ...
    assert r.can_close and r.action == FINAL_REPORT_ALLOWED, r

    x = base()
    x["safe_work_remaining_count"] = 1
    r = evaluate_strategy_close_guard(x)
    assert not r.can_close and r.action == CONTINUE_SAFE_SCOPE_PERSIST_BLOCKED_SCOPE, r
    ...
    r = evaluate_strategy_close_guard(x)
    assert not r.can_close and r.action == BLOCK_CLOSE_GUARD, r
