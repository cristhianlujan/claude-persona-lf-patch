...
def validate(data):
    ...
    if shared.get("contract_status") != "ACTIVE_ENFORCEMENT": errors.append("CONTRACT_STATUS_MISMATCH")
    if shared.get("step_contract_status") != "ACTIVE_ENFORCEMENT": errors.append("STEP_CONTRACT_STATUS_MISMATCH")
    if shared.get("judge_status") != "ACTIVE_ENFORCEMENT": errors.append("JUDGE_STATUS_MISMATCH")
    if shared.get("judge_binding_status") != "ACTIVE_ENFORCEMENT": errors.append("JUDGE_BINDING_STATUS_MISMATCH")
    ...
    if natural.get("expected_status") != "BLOCKED" or natural.get("expected_blocking_code") != "BLOCK_OPERATION_NOT_REGISTERED":
        errors.append("NATURAL_INFERENCE_GAP_NOT_FAIL_CLOSED")
    return errors
