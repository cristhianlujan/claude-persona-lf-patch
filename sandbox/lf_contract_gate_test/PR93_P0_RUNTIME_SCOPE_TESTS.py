#!/usr/bin/env python3
"""Regression matrix for pinned PR93 runtime source gate plus P0 V2/V3 quality gates."""
...
def ensure_p0_visual_test_dependencies(repo_root: Path) -> None:
    if os.environ.get("CI") != "true":
        return
    requirements = repo_root / "sandbox/story_creator_p0_visual/v1.1/requirements-p0-visual-quality.txt"
    subprocess.run(
    ...
    if shutil.which("tesseract") is None:
        ...
        subprocess.run(["sudo", "apt-get", "install", "-y", "-qq", "tesseract-ocr", "tesseract-ocr-spa"], check=True)
    probe = subprocess.run(["tesseract", "--list-langs"], text=True, capture_output=True, check=False)
    langs = set(probe.stdout.split()) if probe.returncode == 0 else set()
    if "spa" not in langs:
        subprocess.run(["sudo", "apt-get", "update", "-qq"], check=True)
        subprocess.run(["sudo", "apt-get", "install", "-y", "-qq", "tesseract-ocr-spa"], check=True)
    os.environ["P0_CI_ENGINEERING_REGRESSION"] = "1"
    print("PASS_P0_VISUAL_TEST_DEPENDENCIES=PINNED_PYTHON_PLUS_TESSERACT_SPA")


def run_p0_quality_regressions(repo_root: Path) -> None:
    ensure_p0_visual_test_dependencies(repo_root)
    p0 = repo_root / "sandbox/story_creator_p0_visual/v1.1"
    ...
    for label, command in commands:
        completed = subprocess.run(command, cwd=repo_root, text=True, capture_output=True, check=False, env=env)
...
def main() -> int:
    ...
    missing_solo_builder_terms = [term for term in required_solo_builder_terms if term not in combined_reconcile_contract]
    if missing_solo_builder_terms:
        raise SystemExit(f"RECONCILE_CANONICAL_EXACT: solo-builder contract incomplete: {missing_solo_builder_terms}")
