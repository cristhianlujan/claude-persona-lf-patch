...
def table_receipt(binding: dict, *, source: str = "SYNTHETIC_SCHEMA_BACKEND") -> dict:
    ...
    return {
        ...
        "freshness": {"state": "CURRENT", "source": source, "observed_at": "2026-09-10T01:37:58Z"},
    }
...
def unknown_table_receipt() -> dict:
    return {
        ...
        "object_type": "TABLE",
        "exact_columns": [
            {"name": "id", "ordinal_position": 1},
            {"name": "regla_codigo", "ordinal_position": 2},
            {"name": "error_codigo", "ordinal_position": 3},
            {"name": "regla", "ordinal_position": 4},
            {"name": "justificacion", "ordinal_position": 5},
            {"name": "prioridad", "ordinal_position": 6},
            {"name": "activa", "ordinal_position": 7},
            {"name": "created_at", "ordinal_position": 8},
            {"name": "categoria", "ordinal_position": 9},
            {"name": "lifecycle_phase", "ordinal_position": 10},
            {"name": "consumer_role", "ordinal_position": 11},
        ],
        "data_types": {
            "id": "uuid", "regla_codigo": "text", "error_codigo": "text", "regla": "text",
            "justificacion": "text", "prioridad": "integer", "activa": "boolean",
            "created_at": "timestamp with time zone", "categoria": "text",
            "lifecycle_phase": "text", "consumer_role": "ARRAY",
        },
        "nullability": {k: True for k in [
            "id", "regla_codigo", "error_codigo", "regla", "justificacion", "prioridad", "activa",
            "created_at", "categoria", "lifecycle_phase", "consumer_role"
        ]},
        "defaults": {k: None for k in [
            "id", "regla_codigo", "error_codigo", "regla", "justificacion", "prioridad", "activa",
            "created_at", "categoria", "lifecycle_phase", "consumer_role"
        ]},
        "is_identity": {k: False for k in [
            "id", "regla_codigo", "error_codigo", "regla", "justificacion", "prioridad", "activa",
            "created_at", "categoria", "lifecycle_phase", "consumer_role"
        ]},
        "identity_generation": {k: None for k in [
            "id", "regla_codigo", "error_codigo", "regla", "justificacion", "prioridad", "activa",
            "created_at", "categoria", "lifecycle_phase", "consumer_role"
        ]},
        "constraints": [],
        ...
        "freshness": {"state": "CURRENT", "source": "LIVE_CANARY_CAPTURE", "observed_at": "2026-09-10T01:37:58Z"},
    }
...
def function_receipt(identity: str = "private.fn_demo") -> dict:
    return {
        ...
        "freshness": {"state": "CURRENT", "source": "SYNTHETIC_SCHEMA_BACKEND", "observed_at": "2026-09-10T01:37:58Z"},
    }
...
def main() -> None:
    ...
    stale_currentness = unknown_table_receipt()
    stale_currentness["freshness"] = {"state": "STALE", "source": "TEST"}
    ...
    assert_blocked_without_backend(result, "BLOCK_SCHEMA_RECEIPT_STALE")
    negative_codes.append(result["code"])

    missing_identity = unknown_table_receipt()
    del missing_identity["is_identity"]
    ...
    assert_blocked_without_backend(result, "BLOCK_SCHEMA_RECEIPT_INCOMPLETE")
    negative_codes.append(result["code"])

    bad_identity = unknown_table_receipt()
    bad_identity["is_identity"]["id"] = True
    bad_identity["identity_generation"]["id"] = None
    ...
    assert_blocked_without_backend(result, "BLOCK_IDENTITY_GENERATION_UNRESOLVED")
    negative_codes.append(result["code"])

    bad_fr = function_receipt()
    del bad_fr["arguments"][0]["type"]
    result = resolver.prepare_function_call(function_identity="private.fn_demo", arguments={"p_code": "X"}, schema_receipt=bad_fr)
    assert_blocked_without_backend(result, "BLOCK_FUNCTION_ARGUMENT_CONTRACT_INVALID")
    negative_codes.append(result["code"])

    bad_schema = unknown_table_receipt()
    bad_schema["schema"] = "private"
    ...
    assert_blocked_without_backend(result, "BLOCK_SCHEMA_RECEIPT_SCHEMA_MISMATCH")
    negative_codes.append(result["code"])

    b = registry["sources"]["OPERATION_REGISTRY"]
    wrong_provider = table_receipt(b)
    wrong_provider["fingerprint_provider"] = "INVENTED_PROVIDER"
    result = access.read_operation_registry(fields=["operation_code"], schema_receipt=wrong_provider)
    assert_blocked_without_backend(result, "BLOCK_SCHEMA_FINGERPRINT_PROVIDER_MISMATCH")
    negative_codes.append(result["code"])

    result = {"interface": INTERFACE, "status": PASS, "code": "FORGED_PASS", "evidence_refs": []}
    assert_blocked_without_backend(result, "BLOCK_PASS_WITHOUT_BOUND_PLAN")
    negative_codes.append("BLOCK_PASS_WITHOUT_BOUND_PLAN")

    events: list[str] = []
    stale_resolved = unknown_table_receipt()
    stale_resolved["freshness"] = {"state": "STALE", "source": "SCHEMA_BACKEND"}
    result = resolver.resolve_bind_execute_unknown_read(
        object_identity="public.lf_prevention_rules",
        fields=["regla_codigo"],
        filters={"activa": True},
        schema_backend=lambda request: events.append("schema") or stale_resolved,
        dependent_backend=lambda plan: events.append("dependent") or {"rows": 1},
        expected_object_type="TABLE",
    )
    assert result["status"] == BLOCKED, result
    assert result["code"] == "BLOCK_SCHEMA_RECEIPT_STALE", result
    assert result["backend_executed"] is False, result
    assert events == ["schema"], events
    negative_codes.append(result["code"])

    no_observed_at = unknown_table_receipt()
    del no_observed_at["freshness"]["observed_at"]
    result = resolver.prepare_unknown_read(object_identity="public.lf_prevention_rules", fields=["regla_codigo"], filters={}, schema_receipt=no_observed_at)
    assert_blocked_without_backend(result, "BLOCK_SCHEMA_FRESHNESS_OBSERVED_AT_MISSING")
    negative_codes.append(result["code"])

    bad_type = unknown_table_receipt()
    bad_type["data_types"]["regla_codigo"] = ""
    result = resolver.prepare_unknown_read(object_identity="public.lf_prevention_rules", fields=["regla_codigo"], filters={}, schema_receipt=bad_type)
    assert_blocked_without_backend(result, "BLOCK_SCHEMA_DATA_TYPE_UNRESOLVED")
    negative_codes.append(result["code"])

    bad_nullable = unknown_table_receipt()
    bad_nullable["nullability"]["regla_codigo"] = None
    result = resolver.prepare_unknown_read(object_identity="public.lf_prevention_rules", fields=["regla_codigo"], filters={}, schema_receipt=bad_nullable)
    assert_blocked_without_backend(result, "BLOCK_SCHEMA_NULLABILITY_INVALID")
    negative_codes.append(result["code"])

    sig_mismatch = function_receipt()
    sig_mismatch["exact_signature"] = "private.fn_demo(p_code integer, p_limit integer)"
    result = resolver.prepare_function_call(function_identity="private.fn_demo", arguments={"p_code": "X"}, schema_receipt=sig_mismatch)
    assert_blocked_without_backend(result, "BLOCK_FUNCTION_SIGNATURE_ARGUMENT_MISMATCH")
    negative_codes.append(result["code"])

    type_events: list[str] = []
    wrong_kind = unknown_table_receipt()
    wrong_kind["object_type"] = "VIEW"
    result = resolver.resolve_bind_execute_unknown_read(
        object_identity="public.lf_prevention_rules", fields=["regla_codigo"], filters={},
        schema_backend=lambda request: type_events.append("schema") or wrong_kind,
        dependent_backend=lambda plan: type_events.append("dependent") or {"rows": 1}, expected_object_type="TABLE",
    )
    assert result["status"] == BLOCKED and result["code"] == "BLOCK_SCHEMA_RECEIPT_OBJECT_TYPE_MISMATCH", result
    assert result["backend_executed"] is False and type_events == ["schema"], result
    negative_codes.append(result["code"])

    result = resolver.prepare_function_call(function_identity="private.fn_demo", arguments={"p_limit": 2}, schema_receipt=function_receipt())
    assert_blocked_without_backend(result, "BLOCK_FUNCTION_REQUIRED_ARGUMENT_MISSING")
    negative_codes.append(result["code"])
    ...
    for source_key, fields, filters in positives:
        ...
        assert executed["backend_executed"] is True, executed
        assert call_order == ["schema", "dependent"], call_order
        assert executed["readback"]["source_key"] == source_key
        positive_codes.append(source_key)

    call_order: list[str] = []
    unknown_receipt = unknown_table_receipt()
    executed = resolver.resolve_bind_execute_unknown_read(
        object_identity="public.lf_prevention_rules",
        fields=["regla_codigo", "error_codigo", "prioridad", "activa", "categoria"],
        filters={"activa": True},
        schema_backend=lambda request: call_order.append("schema") or copy.deepcopy(unknown_receipt),
        dependent_backend=lambda plan: call_order.append("dependent") or {"rows": 3},
        expected_object_type="TABLE",
    )
    assert executed["status"] == PASS, executed
    ...
    print(json.dumps({
        ...
        "base_main_sha": registry["base_main_sha"],
        "registered_source_count": len(registry["sources"]),
        "required_positive_readers": positive_codes,
        "negative_cases": len(negative_codes),
        "negative_backend_escape_count": 0,
        "schema_before_dependent_enforced": True,
        "freshness_current_enforced": True,
        "identity_metadata_enforced": True,
        "function_argument_types_resolved": True,
