...
def validate_freeze_record(record: dict[str, Any]) -> None:
    ...
    for key, value in record["hashes"].items():
        if not isinstance(key, str) or not valid_sha256(value):
...
def validate_freeze_transition(previous: dict[str, Any], current: dict[str, Any]) -> None:
    ...
    pi = PHASES.index(previous["phase"])
    ci = PHASES.index(current["phase"])
    if ci != pi + 1:
...
def _violation(case: dict[str, Any]) -> bool:
    ...
    rules = {
        ...
        "ekb_not_consumed": lambda: not set(s["required_ekb"]).issubset(set(s["consumed_ekb"])),
        "field_unbound": lambda: not set(s["required_bindings"]).issubset(set(s["resolved_bindings"])),
        "import_closure_missing": lambda: not set(s["imports"]).issubset(set(s["resolved_imports"])),
        "required_file_absent": lambda: not set(s["required_files"]).issubset(set(s["present_files"])),
        "dependency_unresolved": lambda: not set(s["required_dependencies"]).issubset(set(s["resolved_dependencies"])),
        ...
        "bundle_incomplete": lambda: not set(s["manifest_files"]).issubset(set(s["bundle_files"])),
        "artifact_not_materialized": lambda: not set(s["declared_artifacts"]).issubset(set(s["materialized_files"])),
        ...
        "clean_reviewer_insufficient_evidence": lambda: not set(s["required_evidence"]).issubset(set(s["available_evidence"])),
        ...
        "invented_argument": lambda: not set(s["attempted_args"]).issubset(set(s["allowed_args"])),
...
def replay_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    if _violation(case):
        return {
            ...
            "decision": "BLOCK",
            "observed_stage": PRE_MATERIAL,
            "first_bad_hop": case["expected_first_bad_hop"],
            "readback": "DETECTED_BEFORE_MATERIAL_WORK",
        }
    return {
        "case_id": case["case_id"],
        "decision": "PASS",
        "observed_stage": "MATERIAL-WORK",
        "first_bad_hop": None,
        "readback": "VIOLATION_NOT_DETECTED",
    }
...
def replay_corpus(corpus: dict[str, Any]) -> dict[str, Any]:
    ...
    false_na = [r for r in results if r["decision"] == "N/A"]
    false_pass = [r for r in results if r["decision"] == "PASS"]
    cross_mut = [r for r in escapes if by_id[r["case_id"]]["fault_kind"] == "freeze_cross_mutation"]
    unbound = [r for r in escapes if by_id[r["case_id"]]["fault_kind"] in {"field_unbound", "invented_argument", "tool_schema_unresolved"}]
    schema_late = [r for r in escapes if by_id[r["case_id"]]["category"] == "DB/schema"]
    missing_readback = [r for r in results if not r.get("readback")]
    metrics = {
        "PREVENTABLE_FIRST_HOP_ESCAPE_COUNT": len(escapes),
        "avoidable_retry_count": len(escapes),
        "schema_introspection_after_error_count": len(schema_late),
        "unbound_field_or_argument_attempts": len(unbound),
        "gate_cross_mutation_count": len(cross_mut),
        "false_N/A": len(false_na),
        "false_PASS": len(false_pass),
        "missing_readback": len(missing_readback),
    }
    return {
...
def create_frozen_bundle(work: Path) -> tuple[Path, dict[str, Any]]:
    ...
    payload = {
        "artifact_id": "S30-R06-SYNTHETIC-001",
        "producer": "S30-C-synthetic-producer",
        "value": {"status": "candidate", "number": 30},
    }
    contract = {
        "contract_id": "S30-R06-UPSTREAM-CONTRACT-v1",
    ...
    manifest = {
        "bundle_id": "S30-R06-FROZEN-BUNDLE-v1",
        "files": {
            name: sha256_file(persisted / name)
            for name in ["artifact_payload.json", "upstream_contract.json", "independent_reviewer.py"]
        },
        "artifact_payload_sha256": sha256_file(persisted / "artifact_payload.json"),
    }
    (persisted / "manifest.json").write_bytes(canonical_bytes(manifest))
    bundle = work / "frozen_bundle.zip"
    with zipfile.ZipFile(bundle, "w") as zf:
        for name in ["artifact_payload.json", "upstream_contract.json", "independent_reviewer.py", "manifest.json"]:
            _fixed_zip_write(zf, name, (persisted / name).read_bytes())
...
def independent_review(bundle: Path) -> tuple[dict[str, Any], Path]:
    ...
    proc = subprocess.run(
        [sys.executable, "-I", str(clean / "independent_reviewer.py"), str(clean)],
        cwd=clean, env=env, text=True, capture_output=True, timeout=20
    )
...
def run_artifact_e2e() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="s30c_r06_") as td:
        work = Path(td)
        bundle, manifest = create_frozen_bundle(work)
        bundle_sha = sha256_file(bundle)
        receipt, clean = independent_review(bundle)
        try:
            clean_has_only_bundle_material = not (clean / ".git").exists()
            artifact_hash_match = receipt["artifact_payload_sha256"] == manifest["artifact_payload_sha256"]
            contract_available = (clean / "upstream_contract.json").is_file() and bool(receipt["upstream_contract_id"])
            network_off = receipt["network_mode"] == "OFF" and receipt["review_completed"]
            reconciliation = (
                receipt["decision"] == "PASS_SYNTHETIC"
                and receipt["deterministic_checks"] == "PASS"
                and artifact_hash_match
            )
            probes = {
                "PRIVATE_REPO_UNAVAILABLE_BUT_FROZEN_BUNDLE_SELF_CONTAINED": clean_has_only_bundle_material and receipt["private_repo_required"] is False,
                "ARTIFACT_PAYLOAD_HASH_RECOMPUTES_EXACTLY": artifact_hash_match,
                "UPSTREAM_CONTRACT_AVAILABLE_WITHOUT_CHAT_MEMORY": contract_available,
                "INDEPENDENT_REVIEWER_CAN_COMPLETE_WITH_NETWORK_OFF": network_off,
            }
            return {
...
def import_closure(paths: list[Path]) -> dict[str, Any]:
    ...
    unresolved = sorted(x for x in imported if x not in stdlib and x not in internal)
...
def workflow_wiring(workflow: Path) -> dict[str, Any]:
    ...
    required = [
        "python sandbox/lf_contract_gate_test/s30_c_reliability_harness/test_s30c_harness.py",
...
def build_self_check(root: Path) -> dict[str, Any]:
    ...
    workflow = root / ".github/workflows/s30-c-reliability-harness.yml"
    corpus = json.loads((harness_dir / "replay_corpus.json").read_text(encoding="utf-8"))
    e2e = run_artifact_e2e()
    closure = import_closure([harness_dir / "s30c_harness.py", harness_dir / "test_s30c_harness.py"])
    ...
    return {
        "r06": e2e,
        "r07": {"freeze_contract_loaded": (harness_dir / "freeze_contract.json").is_file()},
        "r09_build_replay": replay,
...
def ci_receipt(root: Path) -> dict[str, Any]:
    ...
    ok = (
        ...
        and check["r09_build_replay"]["metrics"]["PREVENTABLE_FIRST_HOP_ESCAPE_COUNT"] == 0
        and check["r09_build_replay"]["build_replay_status"] == "PASS"
        and check["dependency_import_closure"]["complete"]
        and check["ci_wiring"]["wired"]
    )
    return {
        "receipt_version": "S30-C-CI-READBACK-v1",
        "semantic_status": "BUILD_HARNESS_VERIFIED" if ok else "BUILD_HARNESS_FAILED",
        "CI_RUNNER_ACTUALLY_EXECUTES_NEW_REGRESSION": "PASS" if probes["CI_RUNNER_ACTUALLY_EXECUTES_NEW_REGRESSION"] else "FAIL",
        "r06_probes": probes,
        "r06_reconciliation": check["r06"]["reconciliation"],
        "r09_build_metrics": check["r09_build_replay"]["metrics"],
        "r09_final": "NOT_RUN_WAITING_FOR_FROZEN_S30_A_AND_S30_B",
        "exit_state": "READY_FOR_FINAL_R09_INTEGRATION" if ok else "NOT_READY",
        "claim_ceiling": "EVIDENCE_FREEZE_AND_REPLAY_HARNESS_READY" if ok else "NONE",
    }

def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("command", choices=["self-check", "ci-receipt", "replay"])
    parser.add_argument("--root", default=str(Path(__file__).resolve().parents[3]))
    args = parser.parse_args()
    root = Path(args.root).resolve()
    if args.command == "self-check":
        print(json.dumps(build_self_check(root), indent=2, sort_keys=True))
    elif args.command == "ci-receipt":
        receipt = ci_receipt(root)
        print(json.dumps(receipt, sort_keys=True))
        if receipt["exit_state"] != "READY_FOR_FINAL_R09_INTEGRATION":
            return 1
    else:
        p = root / "sandbox/lf_contract_gate_test/s30_c_reliability_harness/replay_corpus.json"
        print(json.dumps(replay_corpus(json.loads(p.read_text(encoding="utf-8"))), indent=2, sort_keys=True))
    return 0
