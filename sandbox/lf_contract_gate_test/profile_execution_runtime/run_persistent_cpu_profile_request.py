...
def main()->int:
 ...
 # Persistent hosts must remove the materialized private raster and prompt/output scratch
 # after each execution. TemporaryDirectory guarantees cleanup on success and failure.
 with tempfile.TemporaryDirectory(prefix='lf-persistent-cpu-runtime-') as tmp:
  work_dir=Path(tmp)
