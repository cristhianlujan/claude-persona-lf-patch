...
The router decides *when* a text observation may advance to an orthogonal OCR
challenger. It does not perform OCR and contains no screen-, product-,
coordinate-, or business-specific rules.
...
Required order:
  ...
  4. allow Paddle only when a structured machine invariant remains unresolved
     after at least two traceable targeted Tesseract variants;
  5. abstain/review every remaining ambiguity.
...
contract from traceable repeated observations. Confidence is deliberately
ignored because values are not comparable across OCR engines.
"""
...
def _targeted_attempt_count(observation: dict) -> int:
    return len(_traceable_attempts(observation.get("targeted_attempts"), "TESSERACT"))
