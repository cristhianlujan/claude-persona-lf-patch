...
-- Dependency: OP24_WORKING_CONTEXT_BUDGET_GATE_V1.sql.
-- No runtime activation. No production authorization. No model call.
-- Context Pack and Retrieval digests are distinct canonical payloads; equality is NOT required.
-- Receipt authority is resolved from the canonical provenance-channel registry, not hard-coded to a legacy channel.
...
begin
      ...
      if v_retrieval.execution_id is distinct from p_execution_id then
        ...
        if v_retrieval.provenance_receipt_id is null then
          ...
          if found and not exists (
            ...
            where pc.channel_code=v_receipt.issuer_channel
              and 'RETRIEVAL_PASS'=any(pc.allowed_kinds)
          ) then
            v_errors := v_errors || jsonb_build_array('RETRIEVAL_PROVENANCE_CHANNEL_NOT_AUTHORIZED_FOR_KIND');
          end if;
        end if;
      end if;
    end if;
  ...
  return jsonb_build_object(
    ...
    'retrieval_provenance_issuer_channel',v_receipt.issuer_channel,
...
comment on function private.sbx_fn_lf_working_context_admission_v1(bigint,bigint,text,text,text,text,bigint,interval) is
  'OP24 sandbox-only pre-model admission evaluator. Preserves distinct pack/retrieval hashes; requires same request, exact verified RETRIEVAL_PASS provenance from a registry-authorized channel and GREEN fresh budget; never authorizes a model call itself.';
