...
-- Budget fixtures only. No provenance receipt is forged.
...
declare
  ...
  failed boolean;
begin
  -- Historical retrieval PASS is pre-provenance-guard and must remain blocked.
  ...
  -- Same logical request_ref can have multiple executions, but retrieval must bind exact execution.
  r := private.sbx_fn_lf_working_context_admission_v1(
  ...
  -- Execution 50 has no budget fixture and legacy pack/retrieval shapes: all must fail closed.
  r := private.sbx_fn_lf_working_context_admission_v1(
  ...
  -- A dummy token must not be able to forge SOURCE_VERIFIER_V1 / RETRIEVAL_PASS.
  failed := false;
  begin
