...
def main() -> int:
 ...
 if p.get('schema')!='ACT0058_CANDIDATE_JUDGE_QPG_GATE_V1' or p.get('status')!='BENCHMARK_REQUIRED': fail('header')
 s=p.get('candidate_scope') or {}
 if (s.get('judges'),s.get('cases'),s.get('cases_per_judge'))!=(10,30,3): fail('scope')
 if s.get('active_bindings_before')!=4 or s.get('candidate_bindings_to_activate')!=0: fail('binding boundary')
 ...
 print('ACT0058_QPG_GATE=PASS state=BENCHMARK_REQUIRED quality=30/30_required deterministic=100% activation=0')
 return 0
