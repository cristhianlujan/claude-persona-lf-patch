-- Fast worker-spec path used only after the dispatcher has already proved a CURRENT run.
-- Null current_run_id delegates to the canonical original worker_spec unchanged.
create or replace function programacion.fn_input_governance_worker_spec_known_current_v1(
...
begin
  if p_current_run_id is null then
    return programacion.fn_input_governance_worker_spec(p_pantalla_id,p_consumer);
  end if;
  ...
  if not exists(
    ...
    where r.id=p_current_run_id and r.version_id=v_version and r.pantalla_id=p_pantalla_id
      and r.status='COMPLETED' and r.invalidated_at is null
  ) then raise exception 'INPUT_GOVERNANCE_KNOWN_CURRENT_RUN_BINDING_INVALID:%',p_current_run_id; end if;
  ...
  v_payload:=jsonb_build_object(
    'schema_version',1,'worker_contract','INPUT_GOVERNANCE_WORKER_SPEC_V1','agent_code','INPUT_GOVERNANCE_AGENT','version_id',v_version,
    'pantalla_id',p_pantalla_id,'screen_code',v_code,'consumer',p_consumer,'required_role',v_role,'role_reason',v_reason,
    'latest_run_id',v_latest,'latest_run_status',v_latest_status,'current_run_id',v_current,
    ...
    'runtime_binding_status','NOT_REQUIRED_CURRENT_RUN','runtime_binding','NONE','runtime_orchestrator','SUPABASE_EDGE_FUNCTION:input-governance-agent-v1','runtime_action','REUSE_CURRENT_RUN',
    'new_screen_policy','GOVERNED_CANONICAL_BOOTSTRAP_FAIL_CLOSED','bootstrap_decision','DEC-INPUT-GOV-BOOTSTRAP-001',
    'auto_canonicalization',coalesce(v_exec->>'auto_canonicalization','DENY'),'safe_canonical_write_allowlist',coalesce(v_exec->'safe_canonical_write_allowlist','[]'::jsonb),
    'safe_autofix_contract',v_exec->'safe_autofix_contract','promotion_authorized',false,'production_authorized',false,'generated_at',now());
