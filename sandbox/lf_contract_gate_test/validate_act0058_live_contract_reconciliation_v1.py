...
def main() -> int:
    ...
    if p.get('schema')!='ACT0058_LIVE_CONTRACT_RECONCILIATION_EVIDENCE_V2': fail('schema')
    ...
    c=p.get('coverage') or {}
    if c.get('active_steps')!=14 or c.get('required_steps')!=13 or c.get('active_bindings')!=4 or c.get('missing_bindings')!=10 or c.get('required_missing_bindings')!=9: fail('coverage')
    ...
    if rows[110].get('required_evidence_keys_live')!=['retry_count','error_detail','stage_status']: fail('retry evidence keys')
    if rows[110].get('execution_sql_returns')!=['pipeline_run_id','retry_count','stage_status','error_detail']: fail('retry return contract')
    if rows[110].get('synthetic_next_action_allowed') is not False: fail('synthetic next action')
    ...
    print('ACT0058_LIVE_RECONCILIATION=PASS active=14 required=13 bound=4 missing=10 required_missing=9 cases=30 retry_material_evidence=PASS source_only=PASS')
    return 0
