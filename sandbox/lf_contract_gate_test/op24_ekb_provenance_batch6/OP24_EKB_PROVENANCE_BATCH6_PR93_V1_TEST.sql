...
begin;
\ir OP24_EKB_PROVENANCE_BATCH6_PR93_V1.sql
...
begin
  ...
  where codigo in ('AUD-010','CFG-001','CI-001','GOV-002','GOV-003','SEC-001','SEC-003','SQL-003','SQL-014','SRC-002');

  if v_total<>10 or v_bound<>10 or v_bad_pr<>0 then
    raise exception 'BATCH6_ASSERT_FAIL total=% bound=% bad_pr=%',v_total,v_bound,v_bad_pr;
  end if;
end
$assert$;
...
begin
  ...
  if v_nonnull<>0 then
    raise exception 'BATCH6_ROLLBACK_RESIDUE:%',v_nonnull;
  end if;
end
$post$;
