...
-- pre-insert SUPERSEDES cycle prevention and append-only lineage edges.
...
create table private.sbx_lf_learned_context_lineage_v2 (
  ...
  relation_type text not null check (
    relation_type in (
      'DERIVED_FROM','TRANSFORMED_TO','MOVED_TO','MERGED_INTO','SPLIT_INTO','SUPERSEDES','RETIRES'
    )
  ),
...
create or replace function private.sbx_fn_lf_lineage_ref_resolves_v2(
  p_type text,
  p_ref text
)
returns boolean
...
revoke all on function private.sbx_fn_lf_lineage_ref_resolves_v2(text,text)
...
begin
  ...
  if new.relation_type='SUPERSEDES' then
    ...
    select exists(
      select 1 from reach
      where node_type=new.source_type and node_ref=new.source_ref
    ) into v_cycle;
    if v_cycle then
...
revoke all on function private.sbx_fn_lf_lineage_guard_v2()
  from public, anon, authenticated, service_role;

create trigger trg_sbx_lf_lineage_guard_v2
before insert on private.sbx_lf_learned_context_lineage_v2
for each row execute function private.sbx_fn_lf_lineage_guard_v2();
...
revoke all on function private.sbx_fn_lf_lineage_append_only_v2()
  from public, anon, authenticated, service_role;

create trigger trg_sbx_lf_lineage_append_only_v2
before update or delete on private.sbx_lf_learned_context_lineage_v2
for each row execute function private.sbx_fn_lf_lineage_append_only_v2();
