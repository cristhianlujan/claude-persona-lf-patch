...
-- Secret key material is removed before rowset hashing.
...
rowsets(table_name,row_json) as (
  select 'writer_hmac_keys_v7'::pg_catalog.text,
  ...
  select
    ...
    pg_catalog.encode(
      pg_catalog.sha256(pg_catalog.convert_to(
        coalesce(
          pg_catalog.jsonb_agg(rows.row_json order by rows.row_json::pg_catalog.text)
            filter (where rows.row_json is not null),
          '[]'::pg_catalog.jsonb
        )::pg_catalog.text,
        'UTF8'::pg_catalog.text
      )),
      'hex'::pg_catalog.text
    ) as rowset_sha256
  from table_names names
  left join rowsets rows on rows.table_name OPERATOR(pg_catalog.=) names.table_name
...
select pg_catalog.jsonb_build_object(
  'state_strength','ROWSET_SHA256_WITH_SECRET_REDACTION'::pg_catalog.text,
  'database_name',pg_catalog.current_database()::pg_catalog.text,
  'tables',state.tables,
  'test_key_rows',(
    select pg_catalog.count(*) from private.lf_writer_hmac_keys_v7 k
  ...
  'test_reconciliation_run_rows',(
    select pg_catalog.count(*) from private.lf_github_reconciliation_runs_v3 r
