-- Called only after the dispatcher has already proved the run CURRENT.
...
begin
  ...
  select count(*),count(*) filter(where validator_outcome='PASS'),count(*) filter(where evaluation_outcome='POSITIVE'),
         ...
         coalesce(jsonb_agg(jsonb_build_object(
           'assessment_id',id,'family_code',family_code,'evaluation_outcome',evaluation_outcome,
           'terminal_for_current_evaluation',terminal_eval,'story_ready_status',story_ready_status,'severity',severity,
           'applicability',applicability,'coverage_status',coverage_status,'well_defined_status',well_defined_status,
           'blockers',blockers,'source_refs',source_refs,'rationale',rationale,'validator_outcome',validator_outcome
         ) order by family_code),'[]'::jsonb)
  into v_total,v_pass,v_positive,v_na,v_negative,v_unresolved,v_terminal,v_items
  from classified;

  return jsonb_build_object(
    'schema_version',1,'run_id',p_run_id,'evaluation_contract','CURRENT_CUT_TERMINAL_OUTCOME_V1',
    'decision_authority','DEC-INPUT-GOV-EVAL-OUTCOME-001','run_is_current',true,
    'expected_family_count',v_expected,'total_family_count',v_total,'validator_pass_count',v_pass,
    'positive_count',v_positive,'not_applicable_count',v_na,'negative_confirmed_count',v_negative,
    'unresolved_or_not_validated_count',v_unresolved,'terminal_count',v_terminal,
    'current_evaluation_complete',(v_total=v_expected and v_pass=v_expected and v_terminal=v_expected and v_unresolved=0),
    'negative_does_not_mean_not_applicable',true,'negative_does_not_relax_story_gate',true,
    'remediation_is_separate_from_evaluation',true,'items',v_items);
end;
$function$;
