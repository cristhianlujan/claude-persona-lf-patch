...
from s38_governed_resolution_v0_2 import (
    PASS as TRUST_PASS,
    BLOCKED,
...
D_SCHEMA_PATH = S31_ROOT / "lf_shared_authority_typed_context_v0_2_candidate.schema.json"
F_SCHEMA_PATH = ROOT / "lf_common_evidence_envelope_v0_5_candidate.schema.json"
POLICY_PATH = ROOT / "s38_governed_trust_policy_v0_1.json"
POLICY = json.loads(POLICY_PATH.read_text(encoding="utf-8"))
TRUSTED_RESOLVER_ID = POLICY["resolver_id"]
EvidenceResolver = S38GovernedRefResolver
CROSS_BINDING_VERSION = "S38_F_CROSS_BINDING_V0_3"
TIER_ORDER = {name: i for i, name in enumerate(POLICY["claim_tier"]["order"])}
...
def validate_shared_typed_context(value: Mapping[str, Any], resolver: EvidenceResolver | None = None) -> dict[str, Any]:
    ...
    return {"status": PASS, "code": "PASS_D_SHARED_TYPED_CONTEXT_S38_V0_5"}
...
def validate_capability_manifest(value: Mapping[str, Any], resolver: EvidenceResolver | None = None) -> dict[str, Any]:
    ...
    if resolved.get("status") != PASS:
        return resolved
    return {"status": PASS, "code": "PASS_E_CAPABILITY_MANIFEST_S38_V0_5"}
...
def _earned_tier(value: Mapping[str, Any], resolver: EvidenceResolver) -> tuple[str | None, dict[str, Any] | None]:
    ...
    if TIER_ORDER.get(level, 99) <= TIER_ORDER[base]:
        return level, None
    ...
    if max_tier not in TIER_ORDER or TIER_ORDER[level] > TIER_ORDER[max_tier]:
...
def validate_evidence_envelope(value: Mapping[str, Any], resolver: EvidenceResolver | None = None) -> dict[str, Any]:
    schema = json.loads(F_SCHEMA_PATH.read_text(encoding="utf-8"))
    ...
    if level not in TIER_ORDER or ceiling not in TIER_ORDER:
        return _block("BLOCK_F_EVIDENCE_LEVEL_UNKNOWN")
    earned, earned_error = _earned_tier(value, resolver)
    if earned_error:
        return earned_error
    if TIER_ORDER[level] > TIER_ORDER[earned] or TIER_ORDER[ceiling] > TIER_ORDER[earned]:
        return _block("BLOCK_F_CLAIM_TIER_NOT_EARNED", requested=max(level, ceiling, key=lambda x:TIER_ORDER[x]), earned=earned)
    ...
    if level == "STRUCTURAL":
        return {"status":PASS,"code":"PASS_F_STRUCTURAL_EVIDENCE_ENVELOPE_S38_V0_5","earned_tier":"STRUCTURAL"}
    ...
    return {"status":PASS,"code":"PASS_F_EVIDENCE_ENVELOPE_S38_V0_5_EARNED","earned_tier":earned,"cross_binding_sha256":expected_cross}
...
def validate_runtime_port_request(value: Mapping[str, Any], resolver: EvidenceResolver | None = None) -> dict[str, Any]:
    ...
    return {"status":PASS,"code":"PASS_G_RUNTIME_PORT_REQUEST_S38_V0_5"}
