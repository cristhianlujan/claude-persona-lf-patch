-- Source-first candidate: cache unique proposal source_ref readbacks inside one Validator run.
-- No contract/judge/result semantics are changed; every proposal keeps the same source_ref + observed SHA evidence.
...
begin
  ...
  -- Resolve each distinct source once in the same Validator execution.
  for u in
