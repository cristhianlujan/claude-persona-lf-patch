...
class Case:
    ...
    server_risk_signals: tuple[str, ...] = tuple()


LOT_A = (
    Case("A01","F01_PROFILE_ONLY",("profiles/ui_architect/profile.yaml",),False,"FAST_ALLOWED","FAST","single profile yaml",True),
    Case("A02","F01_PROFILE_ONLY",("profiles/story_creator/profile.yaml",),False,"FAST_ALLOWED","FAST","single profile contract source",True),
    Case("A03","F01_PROFILE_ONLY",("profiles/ui_architect/README.md",),False,"FAST_ALLOWED","FAST","profile documentation inside governed package",True),
    Case("A04","F01_PROFILE_ONLY",("profiles/a/profile.yaml","profiles/b/profile.yaml"),False,"FAST_ALLOWED","FAST","two profile packages",True),
    Case("A05","F01_PROFILE_ONLY",("profiles/a/tests/case.yaml",),False,"FAST_ALLOWED","FAST","profile-local deterministic test data",True),
    Case("A06","F01_PROFILE_ONLY",("profiles/a/profile.yaml","profiles/a/README.md"),False,"FAST_ALLOWED","FAST","profile source plus local documentation",True),
    Case("A07","F02_CARD_SKILL_ADAPTER",("cards/client_screen/card.yaml",),False,"DEEP_REQUIRED","DEEP","card-only remains DEEP until same-request Card materialization receipt/currentness evidence proves a safe FAST boundary"),
    Case("A08","F02_CARD_SKILL_ADAPTER",("skills/story_creator/skill.md",),False,"DEEP_REQUIRED","DEEP","skill-only remains DEEP until lifecycle/evidence contracts prove a safe FAST boundary"),
    ...
    Case("A10","F02_CARD_SKILL_ADAPTER",("adapters/input_governance/adapter.yaml","profiles/a/profile.yaml"),False,"DEEP_REQUIRED","DEEP","adapter plus profile mixed behavior",True),
    ...
    Case("A16","F08_CI_WORKFLOW",(".github/workflows/new-fast-gate.yml","profiles/a/profile.yaml"),False,"DEEP_REQUIRED","DEEP","workflow plus profile",True),
    ...
    Case("A19","F11_MIXED_SURFACE",("profiles/a/profile.yaml","docs/audits/a.md"),False,"FAST_ALLOWED","FAST","two explicitly low-impact surfaces",True),
    Case("A20","F11_MIXED_SURFACE",("profiles/a/profile.yaml","supabase/migrations/20260904000000_x.sql"),False,"DEEP_REQUIRED","DEEP","profile plus schema",True),
...
def decide(case: Case):
    return classify(
        case.paths,
        final_evidence=case.final_evidence,
        server_context_resolved=case.server_context_resolved,
        server_risk_signals=case.server_risk_signals,
    )
...
def run_lot(cases: tuple[Case, ...]) -> dict[str, object]:
    ...
    for case in cases:
        decisions = [decide(case) for _ in range(3)]
        ...
        rows.append({"case_id":case.case_id,"family":case.family,"safety":case.safety,"target_tier":case.target_tier,"actual_tier":decision.tier,"reason":decision.reason,"note":case.note,"server_context_resolved":case.server_context_resolved,"server_risk_signals":list(case.server_risk_signals)})
...
def main() -> int:
    ...
    if summary_path:
        with open(summary_path,"a",encoding="utf-8") as handle:
            ...
            handle.write("- profile FAST cases require modeled server-derived currentness context.\n")
