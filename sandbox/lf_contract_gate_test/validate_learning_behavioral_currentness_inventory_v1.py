...
req(sum(1 for r in D['runs'] if r['is_current'])==D['current_runs'],'CURRENT_COUNT')
req(sum(1 for r in D['runs'] if not r['is_current'])==D['stale_runs'],'STALE_COUNT')
req(D['current_runs']+D['stale_runs']==len(D['runs']),'COUNT_PARTITION')
...
print(f"LEARNING_BEHAVIORAL_CURRENTNESS_INVENTORY=PASS current={D['current_runs']} stale={D['stale_runs']} declared_targets=0 reusable_authority=0 behavioral=INSUFFICIENT_EVIDENCE")
