...
def main() -> int:
    ...
    tight=BindingSpec('PD','NEGOCIACION_DEUDA',('kb-big','kb-small'),2,512)
    tight_out=select_read_only_context([row('kb-big',summary='X'*2000),row('kb-small',summary='s')],binding=tight)
    assert 'kb-big' in tight_out['budget_blocked_learning_ids']
    assert tight_out['context_bytes']<=512
    ...
    print('LEARNING_READ_ONLY_CONTEXT_SELECTOR_V2=PASS exact_id=1 grounded=1 ready=1 max_refs=5 max_bytes=6000 llm_calls=0 round_trips=0 tool_calls=0')
    return 0
