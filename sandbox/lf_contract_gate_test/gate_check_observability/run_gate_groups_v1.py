...
def main() -> int:
    ...
    for gid in selected_ids:
        ...
        group_monotonic = time.monotonic()
        rc = subprocess.run(command).returncode
        group_duration_ms = round((time.monotonic() - group_monotonic) * 1000, 3)
        ...
        row = {
            ...
            "assurance_depth": group.get("assurance_depth"),
    ...
    total_duration_ms = round((time.monotonic() - gate_monotonic) * 1000, 3)
    ...
    summary = {
        ...
        "performance_measurement": {
            "status": "MEASURED",
            "sample_type": "SINGLE_FULL_GATE_RUN" if full_selection else "TARGETED_REPAIR_RUN",
            "total_duration_ms": total_duration_ms,
            "group_duration_ms": {row["group_id"]: row["duration_ms"] for row in rows},
        },
    }
    write_json(artifact_dir / "lf_gate_group_summary_v1.json", summary)
    print(json.dumps({
