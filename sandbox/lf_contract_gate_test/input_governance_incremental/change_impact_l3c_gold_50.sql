-- INPUT_GOVERNANCE_AGENT L3C Change Impact adjudication-bound gold-candidate benchmark
...
-- - Governance issue #414 CLOSED/completed.
-- - Cross-Audit system_audit_run=75 / message=41.
-- - Verdict: ADJUDICATED_WITH_CORRECTIONS.
-- - 49/50 decision agreement; 1 decision correction; 8 impact-set corrections; 4 stale API anchors rebound.
-- - unsafe_under_block target/result from adjudication: 0.
-- - Source parity: PASS_RESUMED, no DDL replay.
--
-- Decision meanings:
...
-- GLOBAL_ESCALATE: mutation changes broader functional/security/API semantics with enough authority to classify globally.
-- HUMAN_REQUIRED: semantics/authority remain insufficient for a deterministic bounded/global decision.
--
-- Historical baseline note:
-- The original PR411 global-block baseline was measured before run218/current API behavioral authority.
-- It is retained only as STALE_HISTORICAL evidence and MUST NOT be presented as the current Router baseline.
...
with gold(case_code,case_family,mutation,expected_decision,expected_impact_families_json,source_anchor,rationale) as (
  ...
  ('CI-COPY-04','COPY_RECONCILIATION','Copy nueva no respaldada por fuente canónica.','HUMAN_REQUIRED','["ACTIONS", "PERMISSIONS", "UI_MESSAGES", "VISUAL_EVIDENCE", "SOURCE_AUTHORITY_PROVENANCE"]','permission:B2B_LOAD_HISTORY_EXPORT','Nueva copy material sin autoridad; la ausencia de autoridad es parte explícita del impacto.'),
  ...
  ('CI-ACT-04','ACTION_SEMANTICS','Agregar segunda action no declarada.','HUMAN_REQUIRED','["ACTIONS", "PERMISSIONS", "API_DATA_CONTRACT", "SOURCE_AUTHORITY_PROVENANCE"]','element:B2B_CARGA001_EXPORT_HISTORY','Nueva semántica sin fuente; requiere autoridad explícita.'),
  ...
  ('CI-PERM-02','PERMISSION_BINDING','Reemplazar por B2B_EVIDENCE_EXPORT.','GLOBAL_ESCALATE','["PERMISSIONS", "ACTIONS", "SECURITY", "API_DATA_CONTRACT"]','permission:B2B_LOAD_HISTORY_EXPORT|permission:B2B_EVIDENCE_EXPORT','Cambia recurso protegido y el contrato conductual asociado.'),
  ('CI-PERM-03','PERMISSION_BINDING','Quitar permission ref.','GLOBAL_ESCALATE','["PERMISSIONS", "ACTIONS", "SECURITY", "API_DATA_CONTRACT"]','element:B2B_CARGA001_EXPORT_HISTORY','Acción pierde autoridad y afecta el contrato conductual.'),
  ...
  ('CI-PERM-05','PERMISSION_BINDING','Crear permiso nuevo ad hoc.','HUMAN_REQUIRED','["PERMISSIONS", "SECURITY", "ACTIONS", "SOURCE_AUTHORITY_PROVENANCE"]','element:B2B_CARGA001_EXPORT_HISTORY','No inventar permisos; autoridad faltante forma parte del impacto.'),
  ...
  ('CI-FIELD-04','FIELD_CONTRACT','Agregar filtro nuevo.','HUMAN_REQUIRED','["FIELDS", "VALIDATIONS", "API_DATA_CONTRACT", "DESIGN_SYSTEM", "SOURCE_AUTHORITY_PROVENANCE"]','screen:B2B-CARGA-001','Nuevo campo requiere definición y autoridad explícita.'),
  ...
  ('CI-VAL-02','VALIDATION','Eliminar validación de USER editable requerido.','GLOBAL_ESCALATE','["VALIDATIONS", "FIELDS", "API_DATA_CONTRACT"]','screen:B2B-CARGA-001:required_editable=2','Input requerido queda sin validación. Run75 confirmó AGREE/GLOBAL_ESCALATE.'),
  ...
  ('CI-STATE-04','STATE_TRANSITION','Agregar estado/transición.','HUMAN_REQUIRED','["STATES", "TRANSITIONS", "ACTIONS", "SOURCE_AUTHORITY_PROVENANCE"]','state_set:UPLOAD_BATCH','Nueva semántica; requiere autoridad explícita.'),
  ...
  ('CI-API-01','API_DATA_CONTRACT','Pedido artifact-only de copy con API_DATA_CONTRACT global abierto.','SCOPED_CANDIDATE','["ACTIONS", "PERMISSIONS", "VISUAL_EVIDENCE"]','api:API_CONTRACT_RESOLUTION_V1:behavioral_contract_present|operation_schema_authority:NOT_MATERIALIZED','El contrato conductual ya existe; el gap de schema de implementación no debe globalizar un delta artifact-only no relacionado.'),
  ('CI-API-02','API_DATA_CONTRACT','Cambiar comportamiento de filtros/query.','GLOBAL_ESCALATE','["API_DATA_CONTRACT", "FIELDS", "VALIDATIONS", "OBJECTIVE_OUTCOMES"]','api:API_CONTRACT_RESOLUTION_V1:behavioral_contract_present|operation_schema_authority:NOT_MATERIALIZED','El contrato conductual vigente define filtros/query; modificarlo toca API directamente.'),
  ('CI-API-03','API_DATA_CONTRACT','Cambiar paginación.','GLOBAL_ESCALATE','["API_DATA_CONTRACT", "FIELDS", "OBJECTIVE_OUTCOMES"]','api:API_CONTRACT_RESOLUTION_V1:behavioral_contract_present|operation_schema_authority:NOT_MATERIALIZED','El contrato conductual vigente define paginación server-side; el cambio es global.'),
  ('CI-API-04','API_DATA_CONTRACT','Cambiar payload/formato de exportación.','HUMAN_REQUIRED','["API_DATA_CONTRACT", "ACTIONS", "PERMISSIONS", "SOURCE_AUTHORITY_PROVENANCE"]','permission:B2B_LOAD_HISTORY_EXPORT|api:API_CONTRACT_RESOLUTION_V1','Behavioral export contract existe, pero operation/request-response schema authority sigue NOT_MATERIALIZED; el cambio necesita autoridad explícita.'),
  ('CI-API-05','API_DATA_CONTRACT','Inventar endpoint/schema para cerrar blocker.','HUMAN_REQUIRED','["API_DATA_CONTRACT", "SOURCE_AUTHORITY_PROVENANCE"]','api:API_CONTRACT_RESOLUTION_V1:behavioral_contract_present|operation_schema_authority:NOT_MATERIALIZED','Existe contrato conductual, pero no authority de operation/schema; autocanonicalization sigue prohibida.')
), typed as (
  ...
  select
    count(*) filter(where expected_decision in ('SCOPED_CANDIDATE','SCOPED_BLOCK')) as historical_unnecessary_global_blocks,
    count(*) filter(where expected_decision='SCOPED_CANDIDATE') as historical_lost_scoped_review_opportunities,
    0::int as adjudicated_unsafe_under_blocks
  from typed
)
select jsonb_build_object(
  ...
  'adjudication',jsonb_build_object(
    'source','GOV414_CROSS_AUDIT_RUN75',
    'system_audit_run_id',75,
    'handoff_message_id',41,
    'verdict','ADJUDICATED_WITH_CORRECTIONS',
    'agreement_count',49,
    'adjudicated_cases',50,
    'corrected_decisions',1,
    'corrected_impact_sets',8,
    'stale_anchor_cases',4,
    'unsafe_under_block',0,
    'source_parity_state','PASS_RESUMED'
  ),
  ...
  'historical_global_block_baseline',jsonb_build_object(
    'status','STALE_HISTORICAL',
    'unnecessary_global_blocks',b.historical_unnecessary_global_blocks,
    'unnecessary_global_block_rate',round(b.historical_unnecessary_global_blocks::numeric/nullif(m.total_cases,0),4),
    'lost_scoped_review_opportunities',b.historical_lost_scoped_review_opportunities,
    'unsafe_under_blocks',b.adjudicated_unsafe_under_blocks
  ),
