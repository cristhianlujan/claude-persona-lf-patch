...
def resolve(case_family: str, mutation: str, ctx: ResolverContext | None = None) -> ResolverResult:
    ...
    if authority_unknown or schema_unknown:
        ...
        "misma copy canónica", "artifact-only de copy",
    )):
        decision = "SCOPED_CANDIDATE"
    else:
        decision = "GLOBAL_ESCALATE"

    if case_family == "COPY_RECONCILIATION" and ("copy" in m or "artefacto" in m):
    ...
    if case_family == "PERMISSION_BINDING" and decision != "SCOPED_CANDIDATE":
        ...
        if any(t in m for t in ("reemplazar", "quitar permission")): impacts.add("API_DATA_CONTRACT")
    ...
    if case_family == "STATE_TRANSITION":
        ...
        if "permission guard" in m: impacts |= {"PERMISSIONS", "SECURITY"}; depth += 1
        if "agregar estado/transición" in m: impacts.add("ACTIONS")
    if case_family == "ERROR_UI_MESSAGE":
        if "403 -> 200" in m: impacts |= {"SECURITY", "API_DATA_CONTRACT"}; depth += 1
        if "retryable" in m: impacts |= {"SECURITY", "TIMEOUT_RETRY"}; depth += 1
