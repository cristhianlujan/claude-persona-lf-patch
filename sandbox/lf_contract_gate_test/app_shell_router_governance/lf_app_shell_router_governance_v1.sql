...
-- Parent source authorization: EXEC-ADMIN-APP-SHELL-LIFECYCLE-SANDBOX-20260915-001
-- Bootstrap provenance is operation-scoped through VULNERABILITY_COVERAGE_REPAIR_LF, matching the canonical provenance guard.
...
-- Operation-scoped bootstrap executions. These are rollback-only and satisfy fn_lf_operation_provenance_guard_v1.
select public.fn_lf_operation_reserve_execution_v1(
  ...
  'sandbox/lf_contract_gate_test/app_shell_router_governance/lf_app_shell_router_governance_v1.sql',
...
select operation_code,'MINI_JUDGE_'||operation_code||'_V1',
  ...
  '{"required_evidence_present":true}'::jsonb,'{"missing_required_evidence":true}'::jsonb,
  '["PASS_CLEAN","BLOCKED_BY_ENFORCEMENT","RETURN_TO_WORKER"]'::jsonb,'ACTIVE_ENFORCEMENT',bootstrap_id,bootstrap_id
from o;
...
select o.operation_code,s.step_order,s.step_id,'MINI_JUDGE_'||o.operation_code||'_V1',
  'PASS_CLEAN','BLOCKED_BY_ENFORCEMENT','RETURN_TO_WORKER',s.keys,'ACTIVE_ENFORCEMENT',o.bootstrap_id,o.bootstrap_id
from o cross join s;
...
('APP_SHELL','APP_SHELL_CREATE','CREACION_APP_SHELL_LF','STATIC',false,false,true,'ACTIVE',
 'Existence resolved inside governed APP_SHELL operation; CANDIDATO only.',
 'EXEC-BOOTSTRAP-CREACION-APP-SHELL-LF-20260915-001','EXEC-BOOTSTRAP-CREACION-APP-SHELL-LF-20260915-001'),
('APP_SHELL','APP_SHELL_UPDATE','ACTUALIZACION_APP_SHELL_LF','STATIC',false,false,true,'ACTIVE',
 'Only existing CANDIDATO APP_SHELL can be updated; VIGENTE direct update blocked.',
 'EXEC-BOOTSTRAP-ACTUALIZACION-APP-SHELL-LF-20260915-001','EXEC-BOOTSTRAP-ACTUALIZACION-APP-SHELL-LF-20260915-001');
...
begin
  ...
  if upper(p_action_code)='APP_SHELL_CREATE' then
    ...
    -- app_shell_id has no sequence/default today. Serialize governed writers before allocating the next numeric id.
  ...
  return jsonb_build_object('result',case when upper(p_action_code)='APP_SHELL_CREATE' then 'APP_SHELL_CANDIDATE_CREATED' else 'APP_SHELL_CANDIDATE_UPDATED' end,
    'operation_code',v_expected,'execution_id',p_execution_id,'before',case when upper(p_action_code)='APP_SHELL_UPDATE' then to_jsonb(v_before) else null end,'after',to_jsonb(v_after));
end;$fn$;
...
do $canary$
declare r jsonb; step_r jsonb; wr jsonb; c integer;
begin
  ...
  if r->>'status'<>'READY_TO_EXECUTE' or r->>'operation_code'<>'CREACION_APP_SHELL_LF' then raise exception 'LF_APP_SHELL_CREATE_ROUTER_CANARY_FAIL:%',r; end if;
  r:=public.fn_lf_operation_reserve_execution_v1('EXEC-CANARY-APP-SHELL-CREATE-001','CREACION_APP_SHELL_LF','APP_SHELL','TEST_ADMIN_PRIV_SHELL','CANARY:APP_SHELL:CREATE:001',repeat('1',64),'EXEC-CANARY-APP-SHELL-CREATE-001',null,null,'{"sandbox_canary":true,"production":false}'::jsonb);
  if r->>'result'<>'RESERVED_NEW_EXECUTION' then raise exception 'LF_APP_SHELL_RESERVE_CANARY_FAIL:%',r; end if;
  step_r:=public.lf_record_operation_step_core_v1('EXEC-CANARY-APP-SHELL-CREATE-001','preflight','canary://app-shell/preflight',
    ...
    'EXEC-CANARY-APP-SHELL-CREATE-001','CREACION_APP_SHELL_LF','APP_SHELL','ACTIVE_ENFORCEMENT','ACTIVE_ENFORCEMENT','ACTIVE_ENFORCEMENT',
    '{"valid":true,"server_assertions":["required_evidence_present"],"server_hard_fails":[]}'::jsonb,false,'APP_SHELL_SANDBOX_CANARY');
  if step_r->>'outcome'<>'STEP_RECORDED' then raise exception 'LF_APP_SHELL_RECORDER_CANARY_FAIL:%',step_r; end if;
  wr:=public.lf_app_shell_candidate_write_v1('EXEC-CANARY-APP-SHELL-CREATE-001','APP_SHELL_CREATE','TEST_ADMIN_PRIV_SHELL','LF_DS_V1','Canary Admin Shell','Rollback-only synthetic shell','v0.1',null,null);
  ...
  if r->>'status'<>'READY_TO_EXECUTE' or r->>'operation_code'<>'ACTUALIZACION_APP_SHELL_LF' then raise exception 'LF_APP_SHELL_UPDATE_ROUTER_CANARY_FAIL:%',r; end if;
  r:=public.fn_lf_operation_reserve_execution_v1('EXEC-CANARY-APP-SHELL-UPDATE-001','ACTUALIZACION_APP_SHELL_LF','APP_SHELL','TEST_ADMIN_PRIV_SHELL','CANARY:APP_SHELL:UPDATE:001',repeat('2',64),'EXEC-CANARY-APP-SHELL-UPDATE-001',null,null,'{"sandbox_canary":true,"production":false}'::jsonb);
  wr:=public.lf_app_shell_candidate_write_v1('EXEC-CANARY-APP-SHELL-UPDATE-001','APP_SHELL_UPDATE','TEST_ADMIN_PRIV_SHELL','LF_DS_V1','Canary Admin Shell v2','Rollback-only synthetic shell','v0.2',null,null);
  if wr#>>'{after,name}'<>'Canary Admin Shell v2' then raise exception 'LF_APP_SHELL_UPDATE_WRITER_CANARY_FAIL:%',wr; end if;
  r:=public.fn_lf_operation_reserve_execution_v1('EXEC-CANARY-APP-SHELL-VIGENTE-BLOCK-001','ACTUALIZACION_APP_SHELL_LF','APP_SHELL','CLIENT_APP_SHELL','CANARY:APP_SHELL:VIGENTE:001',repeat('3',64),'EXEC-CANARY-APP-SHELL-VIGENTE-BLOCK-001',null,null,'{"sandbox_canary":true,"production":false}'::jsonb);
  begin
    perform public.lf_app_shell_candidate_write_v1('EXEC-CANARY-APP-SHELL-VIGENTE-BLOCK-001','APP_SHELL_UPDATE','CLIENT_APP_SHELL','LF_DS_V1','MUST NOT WRITE',null,'v9.9',null,null);
