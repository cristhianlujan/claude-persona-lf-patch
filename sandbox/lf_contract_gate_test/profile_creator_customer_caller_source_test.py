...
checks = {
    ...
    "runtime_completion_requires_clean_recorded_status": 'recordedStepIsClean' in runtime and 'row.status === binding.clean_result_value' in runtime,
    "runtime_blocked_recorded_step_remains_current": 'if (!s.required || recordedStepIsClean(s.step_id)) continue;' in runtime,
    "runtime_remaining_required_uses_clean_status": 'steps.filter((s: any) => s.required && !recordedStepIsClean(s.step_id)).length' in runtime,
    "runtime_presence_only_completion_removed": 'rec.has(s.step_id)' not in runtime,
    ...
    "workflow_execution_id_is_operation_neutral": "EXEC-[A-Za-z0-9][A-Za-z0-9_-]{7,172}" in workflow and "EXEC-[A-Z0-9][A-Z0-9_-]{7,172}" not in workflow and 'EXEC-CREACION-PERFIL-LF-OIDC-[0-9a-f-]+' not in workflow,
    ...
    "update_init_research_policy_workflow_inputs": "research_baseline_mode" in workflow and "research_baseline_contract_json" in workflow and "PRE_RESEARCH_ALWAYS" in workflow,
    "update_init_research_policy_caller_transport": "research_baseline_mode: researchBaselineMode" in caller and "research_baseline_contract: researchBaselineContract" in caller,
    "update_init_research_policy_runtime_validation": all(token in runtime for token in ["PROFILE_UPDATE_RESEARCH_BASELINE_MODE_INVALID","PROFILE_UPDATE_RESEARCH_BASELINE_CONTRACT_REQUIRED","PROFILE_UPDATE_RESEARCH_BASELINE_CONTRACT_FORBIDDEN","PROFILE_RESEARCH_BASELINE_BINDING_V1","PROFILE_OUTPUT_VALIDATOR_BOUND_V1"]),
    "update_init_research_policy_manifest_binding": "research_baseline_mode: researchBaselinePolicy.mode" in runtime and "research_baseline_contract: researchBaselinePolicy.contract" in runtime,
    "update_init_research_policy_idempotency_binding": "research_baseline_mode: researchBaselinePolicy.mode" in runtime and "research_baseline_contract: researchBaselinePolicy.contract" in runtime and "canonicalJson({" in runtime,
    "update_init_research_policy_profile_agnostic": all(token not in runtime for token in ["PERFIL-SYSTEMIC-ROOT-CAUSE-REPAIR-LF","SRCR_BASELINE_SOLUTION_V1","baseline_solution_snapshot","leading_solution_summary"]),
