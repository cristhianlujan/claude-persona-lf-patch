...
PROFILE_UPDATE_CALLER_MAINTENANCE_BRANCH="fix/profile-update-governed-batch-v1-20260920"
PROFILE_UPDATE_CALLER_MAINTENANCE_BLOBS={
 CUSTOMER_PROFILE_CREATOR_WORKFLOW:"9d9e9cea368c4756199909c2b211e0a661d08045",
...
def _evaluate_profile_update_caller_init_maintenance_scope(changed_files:Sequence[str],*,branch:str,blob_by_path:Mapping[str,str],mode_by_path:Mapping[str,str]|None=None)->bool:
 ...
 return True
def _evaluate_profile_update_caller_maintenance_scope(changed_files:Sequence[str],*,branch:str,blob_by_path:Mapping[str,str],mode_by_path:Mapping[str,str]|None=None)->bool:
 changed=set(changed_files)
 if changed!=set(PROFILE_UPDATE_CALLER_MAINTENANCE_PATHS): raise RuntimeScopeError("FAIL_RUNTIME_PROFILE_UPDATE_CALLER_MAINTENANCE_SCOPE",f"exclusive Profile Update caller maintenance scope mismatch: unexpected={sorted(changed-set(PROFILE_UPDATE_CALLER_MAINTENANCE_PATHS))!r} missing={sorted(set(PROFILE_UPDATE_CALLER_MAINTENANCE_PATHS)-changed)!r}")
 if branch!=PROFILE_UPDATE_CALLER_MAINTENANCE_BRANCH: raise RuntimeScopeError("FAIL_RUNTIME_BRANCH_MISMATCH",f"Profile Update caller maintenance requires {PROFILE_UPDATE_CALLER_MAINTENANCE_BRANCH!r}; got {branch!r}")
 for path,expected_blob in PROFILE_UPDATE_CALLER_MAINTENANCE_BLOBS.items():
  observed=blob_by_path.get(path)
...
def _customer_scope_self_test():
 ...
 exec_id=dict(PROFILE_UPDATE_CALLER_MAINTENANCE_BLOBS); exec_id_modes={path:"100644" for path in exec_id}; exec_id_paths=list(PROFILE_UPDATE_CALLER_MAINTENANCE_PATHS); assert _evaluate_profile_update_caller_maintenance_scope(exec_id_paths,branch=PROFILE_UPDATE_CALLER_MAINTENANCE_BRANCH,blob_by_path=exec_id,mode_by_path=exec_id_modes)
 for label,branch,blobs,changed in [("branch","feature/arbitrary",exec_id,exec_id_paths),("blob",PROFILE_UPDATE_CALLER_MAINTENANCE_BRANCH,{**exec_id,CUSTOMER_PROFILE_CREATOR_WORKFLOW:"0"*40},exec_id_paths),("extra",PROFILE_UPDATE_CALLER_MAINTENANCE_BRANCH,exec_id,[*exec_id_paths,"sandbox/lf_contract_gate_test/unexpected.py"])]:
  try: _evaluate_profile_update_caller_maintenance_scope(changed,branch=branch,blob_by_path=blobs,mode_by_path={path:"100644" for path in changed})
  except RuntimeScopeError: continue
...
def _profile_update_caller_init_changed_files():
 if os.environ.get("GITHUB_EVENT_NAME")=="push":
  ...
  return _base.e16.git_changed_files(merge_base,"HEAD")
 return _base.e16.get_changed_files()
def _profile_update_caller_maintenance_changed_files():
 if os.environ.get("GITHUB_EVENT_NAME")=="push":
  subprocess.run(["git","fetch","--no-tags","origin",MAIN_BRANCH],check=True,stdout=subprocess.DEVNULL); merge_base=_base.e16.run_git(["merge-base",f"origin/{MAIN_BRANCH}","HEAD"]).strip()
