...
CUSTOMER_PROFILE_CREATOR_MAINTENANCE_BRANCH="lf/profile-creator-v22-receipt-parser-fix-20260911"
CUSTOMER_PROFILE_CREATOR_MAINTENANCE_BLOBS={
 CUSTOMER_PROFILE_CREATOR_WORKFLOW:"d2f683572094a3e9cd98057e7af8cccea0015ac0",
...
PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_BRANCH="lf/profile-update-runtime-init-v2-20260919"
PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_BLOBS={
...
PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_PATHS=frozenset(PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_BLOBS)
...
def _evaluate_customer_profile_creator_scope(changed_files:Sequence[str],*,branch:str,blob_by_path:Mapping[str,str],mode_by_path:Mapping[str,str]|None=None,main_merge_verified:bool=False)->bool:
 ...
 if branch==MAIN_BRANCH:
  if not (main_merge_verified or _verify_customer_main_merge_via_github()): raise RuntimeScopeError("FAIL_RUNTIME_MAIN_NOT_MERGED","Customer Profile Creator main transition requires an exact merged governed PR")
 elif branch!=CUSTOMER_PROFILE_CREATOR_BRANCH: raise RuntimeScopeError("FAIL_RUNTIME_BRANCH_MISMATCH",f"Customer Profile Creator runtime requires {CUSTOMER_PROFILE_CREATOR_BRANCH!r} or verified main; got {branch!r}")
...
def _evaluate_customer_profile_creator_maintenance_scope(changed_files:Sequence[str],*,branch:str,blob_by_path:Mapping[str,str],mode_by_path:Mapping[str,str]|None=None)->bool:
 ...
 if branch!=CUSTOMER_PROFILE_CREATOR_MAINTENANCE_BRANCH: raise RuntimeScopeError("FAIL_RUNTIME_BRANCH_MISMATCH",f"Profile Creator workflow maintenance requires {CUSTOMER_PROFILE_CREATOR_MAINTENANCE_BRANCH!r}; got {branch!r}")
 for path,expected_blob in CUSTOMER_PROFILE_CREATOR_MAINTENANCE_BLOBS.items():
  observed=blob_by_path.get(path)
...
def _evaluate_profile_update_bound_revision_maintenance_scope(changed_files:Sequence[str],*,branch:str,blob_by_path:Mapping[str,str],mode_by_path:Mapping[str,str]|None=None)->bool:
 ...
 return True
def _evaluate_profile_update_runtime_init_maintenance_scope(changed_files:Sequence[str],*,branch:str,blob_by_path:Mapping[str,str],mode_by_path:Mapping[str,str]|None=None)->bool:
 changed=set(changed_files)
 if changed!=set(PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_PATHS): raise RuntimeScopeError("FAIL_RUNTIME_PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_SCOPE",f"exclusive Profile Update runtime-init maintenance scope mismatch: unexpected={sorted(changed-set(PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_PATHS))!r} missing={sorted(set(PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_PATHS)-changed)!r}")
 if branch!=PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_BRANCH: raise RuntimeScopeError("FAIL_RUNTIME_BRANCH_MISMATCH",f"Profile Update runtime-init maintenance requires {PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_BRANCH!r}; got {branch!r}")
 for path,expected_blob in PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_BLOBS.items():
  observed=blob_by_path.get(path)
...
def _evaluate_profile_update_caller_init_maintenance_scope(changed_files:Sequence[str],*,branch:str,blob_by_path:Mapping[str,str],mode_by_path:Mapping[str,str]|None=None)->bool:
 ...
 return True
def _evaluate_profile_update_execution_id_maintenance_scope(changed_files:Sequence[str],*,branch:str,blob_by_path:Mapping[str,str],mode_by_path:Mapping[str,str]|None=None)->bool:
 changed=set(changed_files)
 if changed!=set(PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_PATHS): raise RuntimeScopeError("FAIL_RUNTIME_PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_SCOPE",f"exclusive Profile Update execution-id maintenance scope mismatch: unexpected={sorted(changed-set(PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_PATHS))!r} missing={sorted(set(PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_PATHS)-changed)!r}")
 if branch!=PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_BRANCH: raise RuntimeScopeError("FAIL_RUNTIME_BRANCH_MISMATCH",f"Profile Update execution-id maintenance requires {PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_BRANCH!r}; got {branch!r}")
 for path,expected_blob in PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_BLOBS.items():
  observed=blob_by_path.get(path)
  if observed!=expected_blob: raise RuntimeScopeError("FAIL_RUNTIME_BLOB_MISMATCH",f"Profile Update execution-id maintenance blob mismatch for {path}: expected={expected_blob} observed={observed}")
  if mode_by_path is not None and mode_by_path.get(path)!="100644": raise RuntimeScopeError("FAIL_RUNTIME_MODE_MISMATCH",f"Profile Update execution-id maintenance path must be regular file 100644: {path}")
 return True
def evaluate_controlled_runtime_scope(changed_files:Sequence[str],*,branch:str,blob_by_path:Mapping[str,str],mode_by_path:Mapping[str,str]|None=None,main_merge_verified:bool=False)->bool:
 if branch==PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_BRANCH and set(changed_files)&set(PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_PATHS): return _evaluate_profile_update_execution_id_maintenance_scope(changed_files,branch=branch,blob_by_path=blob_by_path,mode_by_path=mode_by_path)
...
def _customer_scope_self_test():
 ...
 for label,branch,blobs,changed in [("branch","feature/arbitrary",exact,paths),("blob",CUSTOMER_PROFILE_CREATOR_BRANCH,{**exact,CUSTOMER_PROFILE_CREATOR_WORKFLOW:"0"*40},paths),("edge",CUSTOMER_PROFILE_CREATOR_BRANCH,exact,[*paths,"supabase/functions/arbitrary/index.ts"])]:
  try: _evaluate_customer_profile_creator_scope(changed,branch=branch,blob_by_path=blobs,mode_by_path={path:"100644" for path in changed})
 ...
 maintenance=dict(CUSTOMER_PROFILE_CREATOR_MAINTENANCE_BLOBS); maintenance_modes={path:"100644" for path in maintenance}; maintenance_paths=list(CUSTOMER_PROFILE_CREATOR_MAINTENANCE_PATHS); assert _evaluate_customer_profile_creator_maintenance_scope(maintenance_paths,branch=CUSTOMER_PROFILE_CREATOR_MAINTENANCE_BRANCH,blob_by_path=maintenance,mode_by_path=maintenance_modes)
 for label,branch,blobs,changed in [("branch","feature/arbitrary",maintenance,maintenance_paths),("blob",CUSTOMER_PROFILE_CREATOR_MAINTENANCE_BRANCH,{**maintenance,CUSTOMER_PROFILE_CREATOR_WORKFLOW:"0"*40},maintenance_paths),("extra",CUSTOMER_PROFILE_CREATOR_MAINTENANCE_BRANCH,maintenance,[*maintenance_paths,"sandbox/lf_contract_gate_test/unexpected.py"])]:
  try: _evaluate_customer_profile_creator_maintenance_scope(changed,branch=branch,blob_by_path=blobs,mode_by_path={path:"100644" for path in changed})
  except RuntimeScopeError: continue
 ...
 runtime_init=dict(PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_BLOBS); runtime_init_modes={path:"100644" for path in runtime_init}; runtime_init_paths=list(PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_PATHS); assert _evaluate_profile_update_runtime_init_maintenance_scope(runtime_init_paths,branch=PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_BRANCH,blob_by_path=runtime_init,mode_by_path=runtime_init_modes)
 for label,branch,blobs,changed in [("branch","feature/arbitrary",runtime_init,runtime_init_paths),("blob",PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_BRANCH,{**runtime_init,"supabase/functions/run-creacion-perfil-lf/index.ts":"0"*40},runtime_init_paths),("extra",PROFILE_UPDATE_RUNTIME_INIT_MAINTENANCE_BRANCH,runtime_init,[*runtime_init_paths,"supabase/functions/arbitrary/index.ts"])]:
  try: _evaluate_profile_update_runtime_init_maintenance_scope(changed,branch=branch,blob_by_path=blobs,mode_by_path={path:"100644" for path in changed})
  except RuntimeScopeError: continue
 ...
 exec_id=dict(PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_BLOBS); exec_id_modes={path:"100644" for path in exec_id}; exec_id_paths=list(PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_PATHS); assert _evaluate_profile_update_execution_id_maintenance_scope(exec_id_paths,branch=PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_BRANCH,blob_by_path=exec_id,mode_by_path=exec_id_modes)
 for label,branch,blobs,changed in [("branch","feature/arbitrary",exec_id,exec_id_paths),("blob",PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_BRANCH,{**exec_id,CUSTOMER_PROFILE_CREATOR_WORKFLOW:"0"*40},exec_id_paths),("extra",PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_BRANCH,exec_id,[*exec_id_paths,"sandbox/lf_contract_gate_test/unexpected.py"])]:
  try: _evaluate_profile_update_execution_id_maintenance_scope(changed,branch=branch,blob_by_path=blobs,mode_by_path={path:"100644" for path in changed})
  except RuntimeScopeError: continue
  raise SystemExit(f"FAIL_PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_NEGATIVE_{label.upper()}")
 print("PASS_PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_SCOPE=4/4")
...
def _profile_update_bound_revision_changed_files():
 if os.environ.get("GITHUB_EVENT_NAME")=="push":
  ...
  return _base.e16.git_changed_files(merge_base,"HEAD")
 return _base.e16.get_changed_files()
def _profile_update_runtime_init_changed_files():
 if os.environ.get("GITHUB_EVENT_NAME")=="push":
  subprocess.run(["git","fetch","--no-tags","origin",MAIN_BRANCH],check=True,stdout=subprocess.DEVNULL); merge_base=_base.e16.run_git(["merge-base",f"origin/{MAIN_BRANCH}","HEAD"]).strip()
...
def _profile_update_caller_init_changed_files():
 if os.environ.get("GITHUB_EVENT_NAME")=="push":
  ...
  return _base.e16.git_changed_files(merge_base,"HEAD")
 return _base.e16.get_changed_files()
def _profile_update_execution_id_changed_files():
 if os.environ.get("GITHUB_EVENT_NAME")=="push":
  subprocess.run(["git","fetch","--no-tags","origin",MAIN_BRANCH],check=True,stdout=subprocess.DEVNULL); merge_base=_base.e16.run_git(["merge-base",f"origin/{MAIN_BRANCH}","HEAD"]).strip()
  if BLOB_RE.fullmatch(merge_base) is None: raise RuntimeScopeError("FAIL_RUNTIME_PROFILE_UPDATE_EXECUTION_ID_BASE_UNRESOLVED","Profile Update execution-id maintenance could not resolve merge-base with main")
  return _base.e16.git_changed_files(merge_base,"HEAD")
 return _base.e16.get_changed_files()
def _customer_get_changed_files():
 branch=current_event_branch()
 if branch==PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_BRANCH:
  changed_files=_profile_update_execution_id_changed_files(); blobs={path:git_blob_for_path(path) for path in PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_BLOBS}; modes={path:git_mode_for_path(path) for path in PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_BLOBS}
  _base._runtime_scope_enabled=_evaluate_profile_update_execution_id_maintenance_scope(changed_files,branch=branch,blob_by_path=blobs,mode_by_path=modes); _base.e16.base.ALLOWED_GITHUB_EXACT.add(CUSTOMER_PROFILE_CREATOR_WORKFLOW); _base.e16.base.ALLOWED_EXACT.add(CUSTOMER_PROFILE_CREATOR_WORKFLOW); print(f"PASS_PROFILE_UPDATE_EXECUTION_ID_MAINTENANCE_PR_SCOPE_PARITY={len(changed_files)}"); return changed_files
 branch=current_event_branch()
