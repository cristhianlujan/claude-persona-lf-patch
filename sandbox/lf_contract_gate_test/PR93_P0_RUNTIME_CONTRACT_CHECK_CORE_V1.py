...
CUSTOMER_PROFILE_CREATOR_BLOBS = {
    ...
    "supabase/functions/lf-profile-creator-governance-caller-v1/index.ts": "a0d6d05d5ace871433332eb715a3c8e1dc77a1e5",
...
def _customer_get_changed_files():
    changed_files=_customer_branch_scope_for_push() if os.environ.get("GITHUB_EVENT_NAME")=="push" and current_event_branch()==CUSTOMER_PROFILE_CREATOR_BRANCH else _original_get_changed_files()
    if CUSTOMER_PROFILE_CREATOR_WORKFLOW in set(changed_files):
        if current_event_branch()!=CUSTOMER_PROFILE_CREATOR_BRANCH: raise RuntimeScopeError("FAIL_RUNTIME_BRANCH_MISMATCH","Customer workflow admission requires exact governed branch")
