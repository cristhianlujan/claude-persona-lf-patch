...
begin
  ...
  select array_agg(codigo order by codigo)
    ...
    from (
      select codigo
       ...
       where upper(btrim(coalesce(estado,''))) in ('ACTIVO','ACTIVE','ABIERTO','OPEN')
       group by codigo
      having count(*) <> 1
    ) d;
  ...
  select array_agg(req order by req)
   ...
   where not exists (
     select 1 from public.lf_error_knowledge e
      where upper(btrim(coalesce(e.estado,''))) in ('ACTIVO','ACTIVE','ABIERTO','OPEN') and e.codigo=req
   );
  ...
  with selected as (
    select e.*
     ...
     where upper(btrim(coalesce(e.estado,''))) in ('ACTIVO','ACTIVE','ABIERTO','OPEN')
       and (
  ...
  select array_agg(e.codigo order by e.codigo)
   ...
   where e.codigo = any(v_selected_codes)
     and upper(btrim(coalesce(e.severidad,''))) in ('ALTA','HIGH','CRITICA','CRITICAL','P0')
  ...
  v_core := jsonb_build_object(
    ...
    'active_state_vocabulary',jsonb_build_array('ACTIVO','ACTIVE','ABIERTO','OPEN'),
    'high_severity_vocabulary',jsonb_build_array('ALTA','HIGH','CRITICA','CRITICAL','P0'),
    'ekb_read',true,
