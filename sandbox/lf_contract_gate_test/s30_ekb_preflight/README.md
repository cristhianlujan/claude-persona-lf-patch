...
A PASS requires exactly one control binding for every selected High/Critical/ALTA error and every explicitly required code, plus observed execution evidence (`executed=true`, evidence reference and SHA-256). Human review must be an explicit reviewed PASS; `REVIEW_REQUIRED` remains pending.
