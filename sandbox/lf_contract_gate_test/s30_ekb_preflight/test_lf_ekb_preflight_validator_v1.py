...
class EKBPreflightValidatorV1Test(unittest.TestCase):
    ...
    def test_router_active_state_vocabulary_is_accepted(self) -> None:
        for state in ("activo", "ACTIVO", "active", "ACTIVE", "abierto", "ABIERTO", "open", "OPEN"):
            payload = copy.deepcopy(BASE)
            for row in payload["errors"]:
                row["estado"] = state
            result = resolve(payload)
            self.assertEqual("EKB_RESOLVED_CONTROLS_PENDING", result["result"], state)

    def test_nonactive_required_code_is_missing(self) -> None:
        payload = copy.deepcopy(BASE)
        for row in payload["errors"]:
            row["estado"] = "activo"
        target = next(x for x in payload["errors"] if x["codigo"] == "AUD-018")
        target["estado"] = "VERIFYING"
        result = resolve(payload)
        self.assertEqual("BLOCK_EKB_REQUIRED_CODE_MISSING", result["result"])
        self.assertIn("AUD-018", result["missing_required_codes"])

    def test_critica_and_p0_require_controls(self) -> None:
        payload = copy.deepcopy(BASE)
        payload["required_codes"] = []
        payload["lifecycle_phases"] = ["risk"]
        payload["consumer_roles"] = ["governance"]
        payload["errors"] = [
            {"codigo":"EKB-CRITICA","estado":"ACTIVE","categoria":"X","severidad":"CRITICA","lifecycle_phase":"RISK","consumer_role":["GOVERNANCE"],"has_inline_prevention":True},
            {"codigo":"EKB-P0","estado":"OPEN","categoria":"X","severidad":"P0","lifecycle_phase":"RISK","consumer_role":["GOVERNANCE"],"has_inline_prevention":True},
        ]
        payload["active_rule_codes_by_error"] = {}
        payload["control_coverage"] = {"coverage_version":"LF_EKB_CONTROL_COVERAGE_V1","bindings":[]}
        result = resolve(payload)
        self.assertEqual("BLOCK_EKB_CONTROL_COVERAGE_INCOMPLETE", result["result"])
        self.assertEqual(["EKB-CRITICA", "EKB-P0"], result["missing_control_bindings"])
