...
CROSS_BINDING_VERSION = "S31_F_CROSS_BINDING_V0_2"
...
def validate_evidence_envelope(
    ...
    """IR-004 hardening over v0.2 without coupling all evidence to repo HEAD.
    ...
    deriving one semantic cross-binding digest including claim tier, and requiring
    the same resolver-bound tier and digest in every non-structural receipt. Historical immutable source refs remain
    ...
    if value.get("evidence_level") == "STRUCTURAL":
        return {
            "status": PASS,
            "code": "PASS_F_STRUCTURAL_EVIDENCE_ENVELOPE_V0_4_STRICT",
        }
    ...
    for key, record in receipts.items():
        ...
        if (
            record.get("evidence_level") != value.get("evidence_level")
            or record.get("claim_ceiling") != value.get("claim_ceiling")
        ):
            return _block(
                "BLOCK_F_CLAIM_TIER_BINDING_MISMATCH",
                binding=key,
                expected={
                    "evidence_level": value.get("evidence_level"),
                    "claim_ceiling": value.get("claim_ceiling"),
    ...
    return {
        "status": PASS,
        "code": "PASS_F_EVIDENCE_ENVELOPE_V0_4_CLAIM_BOUND",
