...
spec = importlib.util.spec_from_file_location(
    "s31_dg_v04", ROOT / "validate_s31_dg_contracts_v0_4.py"
)
...
EXPECTED_CROSS_BINDING = "508fcddc9fbef6e30f289017218cdbb7ff6ede72bdefdbed09b4665481dfaf98"
POS = 0
NEG = 0
...
def f_base(level: str = "STRUCTURAL", ceiling: str = "STRUCTURAL") -> dict:
    ...
    value = {
        ...
        "extensions": {"s31.f.v04": {"cross_binding": m.CROSS_BINDING_VERSION}},
    ...
    if executed:
        value["resolved_evidence"] = {
            "execution_receipt": binding("f_execution_receipt_v0_4.json"),
            "authority_currentness_receipt": binding("f_authority_receipt_v0_4.json"),
            "provenance_receipt": binding("f_provenance_receipt_v0_4.json"),
        }
    return value
...
# IR-004 residual: identical resolver-bound receipt set cannot be recomposed
# at a higher claim tier. The old v0.3 binding omitted claim tier and passed.
import validate_s31_dg_contracts_v0_3 as prior
legacy = f_base("SEMANTIC", "SEMANTIC")
legacy["resolved_evidence"] = {
...
legacy["evidence_level"] = "BEHAVIORAL"
legacy["claim_ceiling"] = "BEHAVIORAL"
assert prior.validate_evidence_envelope(legacy, RESOLVER)["status"] == prior.PASS

x = f_base("SEMANTIC", "SEMANTIC")
x["evidence_level"] = "BEHAVIORAL"
x["claim_ceiling"] = "BEHAVIORAL"
result = m.validate_evidence_envelope(x, RESOLVER)
assert result["code"] == "BLOCK_F_CLAIM_TIER_BINDING_MISMATCH", result
NEG += 1
...
print(
    json.dumps(
        {
            "contract": "S31_DG_IR004_HARDENING_V0_4",
            "positive_cases": POS,
            "negative_fail_closed_cases": NEG,
            "v0_2_composition_exploits_demonstrated_then_closed": 7,
            "ir004_claim_tier_escalation_demonstrated_then_closed": 1,
