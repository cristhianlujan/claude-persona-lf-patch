...
def validate_bundle(bundle: dict[str, Any], expected_candidate_sha: str) -> dict[str, Any]:
    ...
    if snapshot.get("s31_candidate_head") != expected_candidate_sha:
        return _block(
            "BLOCK_CANDIDATE_SNAPSHOT_MISMATCH",
            expected=expected_candidate_sha,
            observed=snapshot.get("s31_candidate_head"),
        )
...
def evaluate_pair(bundle: dict[str, Any], handoff_text: str, bundle_text: str, review_case_id: str, expected_candidate_sha: str) -> dict[str, Any]:
    b = validate_bundle(bundle, expected_candidate_sha)
    if b["status"] != "PASS":
        return b
...
def main() -> int:
    ...
    results = []
    failed = False
    for name, bundle_name, handoff_name, case_id, candidate_sha in cases:
        bundle_text = (ROOT / bundle_name).read_text(encoding="utf-8")
        bundle = json.loads(bundle_text)
        handoff = (ROOT / handoff_name).read_text(encoding="utf-8")
        result = evaluate_pair(bundle, handoff, bundle_text, case_id, candidate_sha)
        failed = failed or result.get("status") != "PASS"
        results.append({"case": name, "candidate_sha": candidate_sha, **result})
