#!/usr/bin/env python3
"""Fail-closed migration source vs ledger transport representation comparator.

This module supports the controlled-impact Input Governance parity gate. It
recognizes only two observed ledger storage representations while preserving
exact version, exact name, exact content semantics and ledger statement
cardinality:
...
Any other representation or malformed SQL lexical state fails closed.
"""
...
def canonical(sql: str) -> bytes:
    """Mirror the repository migration parity canonicalization exactly."""
    sql = sql.replace("\r\n", "\n").replace("\r", "\n")
...
def split_postgres_statements(sql: str) -> list[str]:
    """Split top-level SQL without treating quoted semicolons as delimiters."""
    sql = sql.replace("\r\n", "\n").replace("\r", "\n")
    parts: list[str] = []
    start = 0
    i = 0
