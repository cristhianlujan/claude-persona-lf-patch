...
async function requireOidc(req: Request): Promise<{ payload: JWTPayload; method: string; workflowRef: string }> {
  ...
  if (ref === CUSTOMER_REF && workflowRef === CUSTOMER_WORKFLOW_REF) {
    return { payload, method: CUSTOMER_CALLER_METHOD, workflowRef: CUSTOMER_WORKFLOW_REF };
  }
  if (ref === UPDATE_REF && workflowRef === UPDATE_WORKFLOW_REF && payload.event_name === "workflow_dispatch") {
    return { payload, method: UPDATE_CALLER_METHOD, workflowRef: UPDATE_WORKFLOW_REF };
  }
