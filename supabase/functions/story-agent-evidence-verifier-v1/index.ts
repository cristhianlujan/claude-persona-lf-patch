...
const MACHINE_WORKFLOW_SHA = "11646eab03530d5fd61209609e27c2ec8368639f";
...
type Pending = {
  ...
  producer_workflow_sha: string;
  worker_tests: Record<string, string>;
  task_files_expected: string[];
  max_changed_files: number;
  allow_deletions: boolean;
  comparison_base_head_sha: string;
};
...
function argsForV1(p: Pending, verifierIdentity: string, runId: string, workflowSha: string, overrides: Record<string, unknown> = {}): Record<string, unknown> {
  const payload = {
    schema_version: 1,
    execution_id: String(p.execution_id),
    evidence_id: String(p.evidence_id),
    head_sha: p.head_sha,
    ...
    github_workflow_ref: LEGACY_WORKFLOW_REF,
    github_run_id: runId,
    github_workflow_sha: workflowSha,
    context_pack_id: String(p.context_pack_id),
    context_pack_sha256: p.context_pack_sha256,
  };
  return {
    p_execution_id: p.execution_id,
    p_evidence_id: p.evidence_id,
    p_expected_head_sha: p.head_sha,
    p_expected_evidence_sha256: p.evidence_sha256,
    p_expected_source_system: p.source_system,
    p_expected_source_ref: p.source_ref,
    p_verification_method: METHOD_V1,
    p_verifier_identity: verifierIdentity,
    p_verification_payload: payload,
    p_verification_ref: `github-actions://run/${runId}/evidence/${p.evidence_id}`,
    ...overrides,
  };
}
...
function argsForV2(p: Pending, verifierIdentity: string, runId: string, workflowSha: string, observed: Record<string, unknown>, overrides: Record<string, unknown> = {}): Record<string, unknown> {
  const payload = {
    ...
    verification_status: "VERIFIED",
    verifier_identity: verifierIdentity,
    github_repository: MACHINE_REPOSITORY,
    github_repository_id: MACHINE_REPOSITORY_ID,
    github_workflow_ref: MACHINE_WORKFLOW_REF,
    github_run_id: runId,
    github_workflow_sha: workflowSha,
    context_pack_id: String(p.context_pack_id),
    context_pack_sha256: p.context_pack_sha256,
  ...
  return {
    ...
    p_expected_source_system: p.source_system,
    p_expected_source_ref: p.source_ref,
    p_verification_method: METHOD_V2,
    p_verifier_identity: verifierIdentity,
    p_verification_payload: payload,
    p_verification_ref: `github-actions://cristhianlujan/libertad-financiera/actions/runs/${runId}/evidence/${p.evidence_id}`,
    ...overrides,
  };
}
...
function argsForV10(p: PendingV10, verifierIdentity: string, runId: string, workflowSha: string, channelToken: string, overrides: Record<string, unknown> = {}): Record<string, unknown> {
  const payload = {
    schema_version: 1,
    execution_id: String(p.execution_id),
    agent_task_id: String(p.agent_task_id),
    task_code: p.task_code,
    task_version: String(p.task_version),
    task_sha256: p.task_sha256,
    evaluation_id: String(p.evaluation_id),
    gate_code: p.gate_code,
    evidence_id: String(p.evidence_id),
    head_sha: p.head_sha,
    candidate_head_sha: p.candidate_head_sha,
    evidence_sha256: p.evidence_sha256,
    source_system: p.source_system,
    source_ref: p.source_ref,
    worker_receipt_status: p.worker_receipt_status,
    verification_status: "VERIFIED",
    verifier_identity: verifierIdentity,
    github_repository: LEGACY_REPOSITORY,
    github_workflow_ref: LEGACY_WORKFLOW_REF,
  ...
  return {
    ...
    p_verifier_identity: verifierIdentity,
    p_verification_payload: payload,
    p_verification_ref: `github-actions://run/${runId}/worker-v10/evidence/${p.evidence_id}`,
    ...overrides,
  };
}

async function pending(taskId: number): Promise<Pending> {
  ...
  if (!/^[0-9a-f]{40}$/.test(p.head_sha) || !/^[0-9a-f]{64}$/.test(p.evidence_sha256) || !/^[0-9a-f]{40}$/.test(p.context_git_tree_sha) || !/^[0-9a-f]{40}$/.test(p.comparison_base_head_sha) || !/^[0-9a-f]{40}$/.test(p.producer_workflow_sha)) throw new Error("PENDING_EVIDENCE_IDENTITY_INVALID");
  if (!Number.isSafeInteger(Number(p.producer_run_id)) || Number(p.producer_run_id) < 1 || !Number.isSafeInteger(Number(p.producer_job_id)) || Number(p.producer_job_id) < 1) throw new Error("PENDING_EVIDENCE_PRODUCER_ID_INVALID");
  if (!Array.isArray(p.task_files_expected) || p.task_files_expected.length < 1 || !Number.isSafeInteger(Number(p.max_changed_files)) || Number(p.max_changed_files) < 1) throw new Error("PENDING_EVIDENCE_SCOPE_INVALID");
  return p;
}
...
async function verifyWorkerV10(taskId: number, verifierIdentity: string, runId: string, workflowSha: string, channelToken: string): Promise<Record<string, unknown>[]> {
  const items = await rpc<PendingV10[]>("fn_agent_task_pending_worker_v10_evidence_v1", { p_task_id: taskId });
  if (!Array.isArray(items) || items.length < 1) throw new Error("PENDING_V10_EVIDENCE_CONTRACT_INVALID");
  const first = items[0];
  ...
  for (const item of items) {
    const verified = await rpc<Record<string, unknown>>("fn_agent_task_external_verify_worker_v10_evidence_v1", argsForV10(item, verifierIdentity, runId, workflowSha, channelToken));
...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (body.action === "describe_pending_agent_task_v2") {
      assertMachineClaims(claims);
      return response({ outcome: "DESCRIBED", task_id: taskId, pending: await pending(taskId) }, 200);
    }
    ...
    if (body.action === "verify_pending_agent_task_v1") {
      ...
      try { verified = await verifyLegacy(taskId, verifierIdentity, runId, workflowSha); }
      catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        if (!message.includes("PENDING_WORKER_EVIDENCE_NOT_FOUND") && !message.includes("PENDING_EVIDENCE_CONTRACT_INVALID")) throw error;
        sourceAlreadyVerified = true;
      }
      let machine: Record<string, unknown> | null = null;
      try { machine = await materialize(taskId, verifierIdentity, runId, workflowSha); }
      catch (error) {
