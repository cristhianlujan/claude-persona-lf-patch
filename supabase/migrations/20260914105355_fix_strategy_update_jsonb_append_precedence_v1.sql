...
begin
  ...
  if not found or x.operation_code<>'ACTUALIZACION_ESTRATEGIA_LF' or x.status<>'IN_PROGRESS' or x.target_type<>'STRATEGY' then raise exception 'LF_STRATEGY_UPDATE_EXECUTION_BINDING_MISMATCH'; end if;
  select * into s from public.lf_strategy_snapshots where id=p_snapshot_id for update;
  if not found or x.target_code<>s.snapshot_code or x.target_path is distinct from format('supabase://public/lf_strategy_snapshots/%s',s.id) then raise exception 'LF_STRATEGY_UPDATE_TARGET_MISMATCH'; end if;
  ...
  if not baseline_progress_missing and s.metadata->'progress'->>'contract_version'<>'STRATEGY_PROGRESS_CONTRACT_V1' then raise exception 'LF_STRATEGY_UPDATE_BASELINE_PROGRESS_CONTRACT_INVALID'; end if;
  ...
  return jsonb_build_object('result','STRATEGY_UPDATED_WITH_EVIDENCE','snapshot_id',a.id,'snapshot_code',a.snapshot_code,'before_revision_sha256',before_sha,'after_revision_sha256',after_sha,'progress_key_count',coalesce(array_length(new_progress_keys,1),0),'progress_bootstrapped',baseline_progress_missing,'changed_paths',jsonb_build_array('metadata.progress','metadata.merge','backlog','evidence_refs.append','change_log.append','updated_at','updated_by_execution_id'),'status_runtime_impact_preserved',true);
end
$function$;
