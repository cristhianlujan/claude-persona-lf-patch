-- Performance-only currentness helpers.
-- Preserve the exact classifier/currentness algorithms while reusing one canonical graph
-- across the 47 family recomputations inside a single currentness check.
...
begin
  ...
  v_def:=replace(v_def,
    '    for v_assessment in',
    '    v_current_graph:=programacion.fn_input_screen_canonical_graph(v_pantalla_id,v_run.version_id);\n    for v_assessment in');
  v_def:=replace(v_def,
