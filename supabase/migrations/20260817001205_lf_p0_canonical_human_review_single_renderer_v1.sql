...
BEGIN
  ...
  INSERT INTO private.lf_p0_review_evidence_objects_v1(
    evidence_object_id,review_id,execution_id,object_role,object_name,mime_type,content_bytes,content_sha256,content,data_classification,source_head_sha,metadata
  ) VALUES (
    v_manifest_id,p_review_id,p_execution_id,'PACKET_MANIFEST','review-manifest-canonical-v42-'||substr(v_old.source_head_sha,1,12)||'.json',
    'application/json',octet_length(v_manifest_bytes),v_manifest_sha,v_manifest_bytes,v_old.data_classification,v_old.source_head_sha,
    jsonb_build_object('canonical_v42_refresh',true,'refreshed_from_challenge_id',v_old.challenge_id,'renderer_blob_sha',p_renderer_blob_sha,'renderer_ref',p_renderer_ref)
  );
  ...
  INSERT INTO private.lf_p0_review_evidence_objects_v1(
    ...
    v_html_id,p_review_id,p_execution_id,'BROWSER_REVIEW','human-review-canonical-v42-'||substr(v_old.source_head_sha,1,12)||'.html',
    'text/html',octet_length(v_html_bytes),v_html_sha,v_html_bytes,v_old.data_classification,v_old.source_head_sha,
    jsonb_build_object('shell_version','4.2','canonical_v42_refresh',true,'refreshed_from_challenge_id',v_old.challenge_id,
      'renderer_contract','p0-human-review-shell-v4.2-canonical-single-renderer/v1','renderer_blob_sha',p_renderer_blob_sha,
      'renderer_ref',p_renderer_ref,'human_language_contract','p0-human-review-human-language/v2','human_language_presentation_only',true,
      'source_canvas_mode','SINGLE_BACKGROUND_WITH_CANONICAL_OVERLAY','selected_crop_policy','SELECTED_ELEMENT_ONLY')
  ) RETURNING content_sha256 INTO v_html_sha;

  INSERT INTO private.lf_p0_human_review_challenges_v1(
    challenge_id,review_id,execution_id,source_head_sha,visual_output_sha256,packet_manifest_sha256,required_reviewer_role,reviewer_actions,evidence_store_ref,
    issued_at,expires_at,data_classification,dual_review_required,review_lane,review_subject,source_evidence_object_id,source_sha256,visual_output_object_id,
    packet_manifest_object_id,browser_review_object_id,browser_review_sha256,semantic_fingerprint,element_count,uncertain_count,inferred_count,changed_count,
    pending_human_count,delta,review_mode,carry_forward_safe,supersedes_challenge_id,convergence_justification
  ) VALUES (
    p_challenge_id,p_review_id,p_execution_id,v_old.source_head_sha,v_old.visual_output_sha256,v_manifest_sha,v_old.required_reviewer_role,v_old.reviewer_actions,
    v_old.evidence_store_ref,p_issued_at,p_expires_at,v_old.data_classification,v_old.dual_review_required,v_old.review_lane,v_old.review_subject,
    v_old.source_evidence_object_id,v_old.source_sha256,v_old.visual_output_object_id,v_manifest_id,v_html_id,v_html_sha,v_old.semantic_fingerprint,
    v_old.element_count,v_old.uncertain_count,v_old.inferred_count,v_old.changed_count,v_old.pending_human_count,v_old.delta,v_old.review_mode,v_old.carry_forward_safe,
    v_old.challenge_id,'CANONICAL_V42_SINGLE_RENDERER_REFRESH_NO_HUMAN_DEBT_CHANGE'
  );

  RETURN jsonb_build_object('outcome','CANONICAL_V42_REVIEW_READY','challenge_id',p_challenge_id,'review_id',p_review_id,
    'browser_review_object_id',v_html_id,'browser_review_sha256',v_html_sha,'packet_manifest_object_id',v_manifest_id,'packet_manifest_sha256',v_manifest_sha,
    'renderer_blob_sha',p_renderer_blob_sha,'renderer_ref',p_renderer_ref,'source_head_sha',v_old.source_head_sha,'source_sha256',v_old.source_sha256,
    'semantic_fingerprint',v_old.semantic_fingerprint,'element_count',v_old.element_count,'changed_count',v_old.changed_count,
    'pending_human_count',v_old.pending_human_count,'supersedes_challenge_id',v_old.challenge_id,'production_authorized',false,'p0_5_authorized',false);
END;
$$;
...
BEGIN
  RAISE EXCEPTION 'CANONICAL_V42_MATERIALIZER_REQUIRED'
    USING HINT = 'Generate with canonical p0_human_review_shell_v4.py and publish through fn_lf_p0_publish_canonical_review_v42_v1; copying an existing BROWSER_REVIEW is forbidden.';
END;
$$;

COMMENT ON FUNCTION public.fn_lf_p0_refresh_actionable_review_v1(text) IS
  'Retired copy-based refresh. Canonical V4.2 must be regenerated from the frozen Python renderer and then persisted via fn_lf_p0_publish_canonical_review_v42_v1.';
COMMENT ON FUNCTION private.fn_lf_p0_assert_canonical_browser_review_v42_v1(text,text) IS
  'Fail-closed structural gate for frozen P0 Human Review V4.2. Enforces counts/list/source background+overlay/single selected crop/page order/actions and Human Language v2.';