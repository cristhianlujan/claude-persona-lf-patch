...
begin
  ...
  for a in select * from programacion.input_family_assessments where run_id=v_parent order by family_code loop
    ...
    values(v_new,a.family_code,a.severity,a.applicability,a.coverage_status,a.well_defined_status,a.story_ready_status,a.implementation_ready_status,a.qa_ready_status,a.production_ready_status,a.source_refs,
      case when a.family_code='VISUAL_EVIDENCE' then a.rationale||' | Readback 2026-08-22: la evidencia candidata actual ya incluye Desktop, Tablet y Mobile por variant_code; la assertion se recura por semántica estable y no por artifact_id histórico. La familia conserva sus gates físicos/estatus existentes.' else a.rationale||' | Revalidado contra fuentes canónicas vigentes por reconciliación semántica PR #179 2026-08-22.' end,
      a.blockers,a.negative_requirements,a.test_obligations,jsonb_build_object('component_id',46,'execution_id',gen_random_uuid()::text,'execution_mode','INDEPENDENT_CURATOR','contract_revision','5.11','parent_run_id',v_parent,'parent_assessment_id',a.id,'remediation_revision','PR179_LIVE_SOURCE_SEMANTIC_RECONCILIATION_20260822','direct_source_readback',true),repeat('0',64));
  end loop;
