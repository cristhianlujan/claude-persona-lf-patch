...
begin
  ...
  v_applied_sets:=jsonb_build_array(jsonb_build_object(
    'set_id',v_cfg #>> '{base_set,set_id}',
    'capabilities',to_jsonb(v_required_codes)
  ));
  ...
  loop
    ...
    if v_applies then
      ...
      v_applied_sets:=v_applied_sets||jsonb_build_array(jsonb_build_object(
        'set_id',v_set->>'set_id',
        'capabilities',to_jsonb(v_set_codes)
      ));
    end if;
  end loop;
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
           'code',a.codigo_activo,
           'version',a.version,
           'inventory_status',a.metadata #>> '{transversal_inventory,inventory_status}'
         ) order by a.codigo_activo),'[]'::jsonb)
  into v_capabilities
  from public.lf_activos a
  where a.codigo_activo=any(v_required_codes)
    and a.archived_at is null;
  ...
  from (
    select
      ...
      jsonb_build_object(
        ...
        'titulo',left(k.titulo,180),
        'detail_ref','supabase://public.lf_error_knowledge/'||k.codigo
      ) as item
    from public.lf_error_knowledge k
    where upper(coalesce(k.estado,'')) in ('ACTIVO','ACTIVE','ABIERTO','OPEN')
      and upper(coalesce(k.severidad,'')) in ('ALTA','HIGH','CRITICA','CRITICAL','P0')
