...
begin
  ...
  select count(*) into contract_count from public.lf_operation_contracts
   where operation_code='CREACION_PERFIL_LF' and status='ACTIVE_ENFORCEMENT';
  if contract_count<>1 then raise exception 'LF_PROFILE_CREATION_ACTIVE_CONTRACT_NOT_EXACT:%',contract_count; end if;
  select * into oc from public.lf_operation_contracts where operation_code='CREACION_PERFIL_LF' and status='ACTIVE_ENFORCEMENT' limit 1;
  ...
  r:=public.fn_lf_operation_reserve_execution_v1(
    ...
    p_manifest || jsonb_build_object('contract_code',oc.contract_code,'contract_sha',oc.contract_sha,'parent_execution_id',p_actor_execution_id)
  );
  ...
  return r || jsonb_build_object('init_step','RECORDED','contract_code',oc.contract_code,'contract_sha',oc.contract_sha,'next_step','router');
end;
$$;