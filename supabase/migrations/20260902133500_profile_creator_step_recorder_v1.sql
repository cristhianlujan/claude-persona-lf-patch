-- Profiles LF — transactional continuation for CREACION_PERFIL_LF.
-- This is an internal control-plane RPC used only by the already-governed Edge caller.
-- It never writes GitHub bytes; it only records verified step evidence in execution order.
...
declare
  ...
  v_gate jsonb;
begin
  if p_execution_id is null or p_execution_id = '' or p_step_id is null or p_step_id = '' then
  ...
  for update;
  if not found or v_execution.operation_code <> 'CREACION_PERFIL_LF' or v_execution.target_type <> 'PERFIL' then
  ...
  where operation_code='CREACION_PERFIL_LF' and step_id=p_step_id and active is true;
  if not found then
  ...
  where operation_code='CREACION_PERFIL_LF' and step_id=p_step_id and status='ACTIVE';
  if not found then
    return jsonb_build_object('outcome','BLOCKED','code','STEP_CONTRACT_MISSING');
  end if;

  select * into v_binding
  from public.lf_operation_step_judge_bindings
  where operation_code='CREACION_PERFIL_LF' and step_id=p_step_id and status='ACTIVE_ENFORCEMENT';
  if not found then
  ...
  if found then
    if v_existing.status='STEP_PASS_WITH_EVIDENCE'
       and v_existing.evidence_payload @> jsonb_build_object('evidence_digest',p_evidence_payload->>'evidence_digest') then
  ...
  where s.operation_code='CREACION_PERFIL_LF' and s.required is true and s.active is true
    ...
    and ((s.step_id='init_execution' and es.status<>'STEP_CLEAN_PASS')
      or (s.step_id<>'init_execution' and es.status<>'STEP_PASS_WITH_EVIDENCE'));
  ...
  for v_key in select jsonb_array_elements_text(v_binding.required_evidence_keys)
  loop
  ...
  -- Critical deterministic invariants. Semantic judgement remains in the evidence producer,
  -- but these non-negotiable machine gates cannot be asserted away.
  if p_step_id='destination_validate' then
    if p_evidence_payload->>'repo' <> 'cristhianlujan/claude-persona-lf-patch'
       or p_evidence_payload->>'base_folder' not in ('profiles','/profiles/')
       or coalesce(p_evidence_payload->>'package_root','') not like 'profiles/%'
       or p_evidence_payload->>'artifact_type' <> 'PERFIL'
       or coalesce(p_evidence_payload->>'destination_source','') not ilike '%artifact_destination_registry%' then
      return jsonb_build_object('outcome','BLOCKED','code','BLOCKED_DESTINATION_VALIDATE_NOT_CLEAN');
    end if;
  elsif p_step_id='pre_destination_resolution_gate' then
    if jsonb_typeof(p_evidence_payload->'missing_required_roles')<>'array'
       or jsonb_array_length(p_evidence_payload->'missing_required_roles')<>0
       or jsonb_typeof(p_evidence_payload->'write_plan')<>'array'
       or jsonb_array_length(p_evidence_payload->'write_plan')=0 then
      return jsonb_build_object('outcome','BLOCKED','code','BLOCKED_DESTINATION_ROLE_NOT_CONFIGURED');
    end if;
  elsif p_step_id='pre_write_execution_binding_gate' then
    if p_evidence_payload->>'estado_recibido'<>'CANDIDATO_NO_OFICIAL'
       or coalesce((p_evidence_payload->>'files_ready')::boolean,false) is not true
       or coalesce((p_evidence_payload->>'pre_write_gate_passed')::boolean,false) is not true
       or coalesce((p_evidence_payload->>'no_blockers')::boolean,false) is not true then
      return jsonb_build_object('outcome','BLOCKED','code','BLOCKED_PRE_WRITE_GATE_NOT_PASSED');
    end if;
  elsif p_step_id='github_write' then
    if coalesce((p_evidence_payload->>'partial_write_detected')::boolean,true) is not false
       or coalesce(p_evidence_payload->>'commit_sha','')=''
       or jsonb_typeof(p_evidence_payload->'written_files')<>'array'
       or jsonb_array_length(p_evidence_payload->'written_files')=0 then
      return jsonb_build_object('outcome','BLOCKED','code','BLOCKED_GITHUB_WRITE_NOT_CLEAN');
    end if;
  elsif p_step_id='github_readback' then
    if p_evidence_payload->>'sha_match_status'<>'PASS'
       or jsonb_typeof(p_evidence_payload->'readback_files')<>'array'
       or jsonb_array_length(p_evidence_payload->'readback_files')=0 then
      return jsonb_build_object('outcome','BLOCKED','code','BLOCKED_GITHUB_READBACK_NOT_CLEAN');
    end if;
  elsif p_step_id='contract_judge' then
    if coalesce((p_evidence_payload->>'required_steps_clean_pass')::boolean,false) is not true
       or p_evidence_payload->>'judge_result'<>'PASS'
       or jsonb_array_length(coalesce(p_evidence_payload->'missing_contracts','[]'::jsonb))>0
       or jsonb_array_length(coalesce(p_evidence_payload->'missing_judges','[]'::jsonb))>0
       or jsonb_array_length(coalesce(p_evidence_payload->'generic_critical_contracts','[]'::jsonb))>0 then
      return jsonb_build_object('outcome','BLOCKED','code','BLOCKED_CONTRACT_JUDGE_NOT_CLEAN');
    end if;
  end if;
  ...
  v_payload := p_evidence_payload || jsonb_build_object(
    ...
    'recorded_by_rpc','lf_record_creacion_perfil_step_v1'
  );
  ...
  insert into public.lf_operation_execution_steps(
    ...
    p_execution_id,v_step.step_order,p_step_id,'STEP_PASS_WITH_EVIDENCE',p_evidence_ref,v_payload,
    'Recorded transactionally by governed Profile Creator continuation RPC.',p_actor_execution_id
  );

  if p_step_id='close' then
    v_gate := public.lf_creacion_perfil_lf_no_close_gate(p_execution_id);
    if v_gate->>'verdict' <> 'PASS_CLEAN' then
      raise exception 'PROFILE_CLOSE_GATE_FAILED:%',v_gate::text;
    end if;
    update public.lf_operation_execution
    set status='CONTROLLED_READ_ONLY_PASS',
        manifest=coalesce(manifest,'{}'::jsonb) || jsonb_build_object('no_close_gate',v_gate,'next_gate','report_output'),
        updated_by_execution_id=p_actor_execution_id,updated_at=now()
    where execution_id=p_execution_id;
  end if;
  ...
  return jsonb_build_object(
    ...
    'status','STEP_PASS_WITH_EVIDENCE','mini_judge_code',v_binding.judge_code,
