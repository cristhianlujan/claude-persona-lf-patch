...
-- Internal control-plane RPC for the existing governed Edge caller.
-- It records verified evidence in execution_order; it never writes GitHub bytes.
...
begin
  ...
  if found then
    if v_existing.status='STEP_PASS_WITH_EVIDENCE' and v_existing.evidence_payload @> jsonb_build_object('evidence_digest',p_evidence_payload->>'evidence_digest') then
  ...
  where s.operation_code='CREACION_PERFIL_LF' and s.required is true and s.active is true
    and coalesce(s.execution_order,0)<coalesce(v_step.execution_order,0)
    and ((s.step_id='init_execution' and es.status<>'STEP_CLEAN_PASS') or (s.step_id<>'init_execution' and es.status<>'STEP_PASS_WITH_EVIDENCE'));
  ...
  for v_key in select jsonb_array_elements_text(v_binding.required_evidence_keys) loop
    if not (p_evidence_payload ? v_key) or p_evidence_payload->v_key is null or p_evidence_payload->v_key='null'::jsonb
      or (jsonb_typeof(p_evidence_payload->v_key)='string' and btrim(p_evidence_payload->>v_key)='') then v_missing_keys:=array_append(v_missing_keys,v_key); end if;
  end loop;
  ...
  if p_step_id='destination_validate' then
    if p_evidence_payload->>'repo'<>'cristhianlujan/claude-persona-lf-patch'
      ...
      or coalesce(p_evidence_payload->>'destination_source','') not ilike '%artifact_destination_registry%' then return jsonb_build_object('outcome','BLOCKED','code','BLOCKED_DESTINATION_VALIDATE_NOT_CLEAN'); end if;
  elsif p_step_id='pre_destination_resolution_gate' then
    if jsonb_typeof(p_evidence_payload->'missing_required_roles')<>'array' or jsonb_array_length(p_evidence_payload->'missing_required_roles')<>0
      or jsonb_typeof(p_evidence_payload->'write_plan')<>'array' or jsonb_array_length(p_evidence_payload->'write_plan')=0 then return jsonb_build_object('outcome','BLOCKED','code','BLOCKED_DESTINATION_ROLE_NOT_CONFIGURED'); end if;
  elsif p_step_id='pre_write_execution_binding_gate' then
    if p_evidence_payload->>'estado_recibido'<>'CANDIDATO_NO_OFICIAL' or coalesce((p_evidence_payload->>'files_ready')::boolean,false) is not true
      or coalesce((p_evidence_payload->>'pre_write_gate_passed')::boolean,false) is not true or coalesce((p_evidence_payload->>'no_blockers')::boolean,false) is not true then return jsonb_build_object('outcome','BLOCKED','code','BLOCKED_PRE_WRITE_GATE_NOT_PASSED'); end if;
  elsif p_step_id='github_write' then
    if coalesce((p_evidence_payload->>'partial_write_detected')::boolean,true) is not false or coalesce(p_evidence_payload->>'commit_sha','')=''
      or jsonb_typeof(p_evidence_payload->'written_files')<>'array' or jsonb_array_length(p_evidence_payload->'written_files')=0 then return jsonb_build_object('outcome','BLOCKED','code','BLOCKED_GITHUB_WRITE_NOT_CLEAN'); end if;
  elsif p_step_id='github_readback' then
    if p_evidence_payload->>'sha_match_status'<>'PASS' or jsonb_typeof(p_evidence_payload->'readback_files')<>'array'
      or jsonb_array_length(p_evidence_payload->'readback_files')=0 then return jsonb_build_object('outcome','BLOCKED','code','BLOCKED_GITHUB_READBACK_NOT_CLEAN'); end if;
  elsif p_step_id='contract_judge' then
    if coalesce((p_evidence_payload->>'required_steps_clean_pass')::boolean,false) is not true or p_evidence_payload->>'judge_result'<>'PASS'
      or jsonb_array_length(coalesce(p_evidence_payload->'missing_contracts','[]'::jsonb))>0 or jsonb_array_length(coalesce(p_evidence_payload->'missing_judges','[]'::jsonb))>0
      or jsonb_array_length(coalesce(p_evidence_payload->'generic_critical_contracts','[]'::jsonb))>0 then return jsonb_build_object('outcome','BLOCKED','code','BLOCKED_CONTRACT_JUDGE_NOT_CLEAN'); end if;
  end if;
  ...
  if p_step_id='close' then
    update public.lf_operation_execution set manifest=coalesce(manifest,'{}'::jsonb)||jsonb_build_object('closure_allowed',true,'blocked_from_closure',false,'scope','GOVERNED_STEPS_1_39','next_gate','close'),updated_by_execution_id=p_actor_execution_id,updated_at=now() where execution_id=p_execution_id;
  end if;
  ...
  values(p_execution_id,v_step.step_order,p_step_id,'STEP_PASS_WITH_EVIDENCE',p_evidence_ref,v_payload,'Recorded transactionally by governed Profile Creator continuation RPC.',p_actor_execution_id);
  if p_step_id='close' then
