...
begin
  if not exists (
    select 1 from programacion.input_readiness_runs r
    where r.id=v_parent and r.version_id=19 and r.pantalla_id=v_screen
      and r.status='COMPLETED' and r.contract_revision='5.11' and r.invalidated_at is null
  ) then raise exception 'V511_DRIFT_PARENT_NOT_ELIGIBLE screen=% run=%',v_screen,v_parent; end if;
  if exists (
    select 1 from programacion.input_readiness_runs s
    where s.supersedes_run_id=v_parent and s.status in ('COMPLETED','BLOCKED')
  ) then raise exception 'V511_DRIFT_PARENT_ALREADY_SUPERSEDED screen=% run=%',v_screen,v_parent; end if;
  if programacion.fn_input_readiness_run_is_current(v_parent) then raise exception 'V511_DRIFT_PARENT_EXPECTED_STALE screen=% run=%',v_screen,v_parent; end if;

  v_cur := 'INPUT_CURATOR:v0.5r1-auth-'||v_screen||'-v511-canonical-drift-'||substr(md5(random()::text||clock_timestamp()::text),1,8);
  ...
  select version_id,pantalla_id,universe_rule_id,id,
    scope || jsonb_build_object('mode','CANDIDATE_V511_CANONICAL_DRIFT_REVALIDATION','parent_run_id',id,'remediation','PR179_LIVE_SOURCE_DRIFT_RECONCILIATION_20260820'),
  ...
  for a in select * from programacion.input_family_assessments where run_id=v_parent order by family_code loop
    ...
    values(v_new,a.family_code,a.severity,a.applicability,a.coverage_status,a.well_defined_status,a.story_ready_status,a.implementation_ready_status,a.qa_ready_status,a.production_ready_status,a.source_refs,
      a.rationale||' | Recurado contra fuentes canónicas vigentes por reconciliación PR #179 2026-08-20.',a.blockers,a.negative_requirements,a.test_obligations,
      jsonb_build_object('component_id',46,'execution_id',gen_random_uuid()::text,'execution_mode','INDEPENDENT_CURATOR','contract_revision','5.11','parent_run_id',v_parent,'parent_assessment_id',a.id,'remediation_revision','PR179_LIVE_SOURCE_DRIFT_RECONCILIATION_20260820','direct_source_readback',true),repeat('0',64));
  end loop;

  v_val := 'INPUT_VALIDATOR:v0.5r1-auth-'||v_screen||'-v511-canonical-drift-'||substr(md5(random()::text||clock_timestamp()::text),1,8);
  ...
  if not programacion.fn_input_readiness_run_is_current(v_new) then raise exception 'V511_DRIFT_POSTCHECK_CURRENTNESS_FAILED screen=% run=%',v_screen,v_new; end if;
end$$;