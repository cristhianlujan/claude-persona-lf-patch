...
begin
  if programacion.fn_input_readiness_run_is_current(v_parent) then
    raise exception 'AUTH004_PARENT_EXPECTED_STALE_AFTER_VISUAL_CHANGE';
  end if;
  if not exists (select 1 from programacion.input_readiness_runs where id=v_parent and version_id=19 and pantalla_id=54 and contract_revision='5.11' and status='COMPLETED') then
    raise exception 'AUTH004_PARENT_NOT_ELIGIBLE';
  end if;

  v_cur:='INPUT_CURATOR:v0.5r1-auth-54-v511-visual-refresh-'||substr(md5(random()::text||clock_timestamp()::text),1,8);
  ...
  select version_id,pantalla_id,universe_rule_id,id,scope||jsonb_build_object('mode','CANDIDATE_V511_VISUAL_SOURCE_REFRESH','parent_run_id',id,'reason','AUTH004_TABLET_ARTIFACT_REFRESH_20260819'),universe_snapshot_sha256,family_count,'CURATING',v_cur,curator_component_id,contract_version
  ...
  for a in select * from programacion.input_family_assessments where run_id=v_parent order by family_code loop
    ...
    values(v_new,a.family_code,a.severity,a.applicability,a.coverage_status,a.well_defined_status,a.story_ready_status,a.implementation_ready_status,a.qa_ready_status,a.production_ready_status,a.source_refs,a.rationale||' | Recurado tras actualización canónica del artefacto TABLET B2B-AUTH-004 el 2026-08-19.',a.blockers,a.negative_requirements,a.test_obligations,jsonb_build_object('component_id',46,'execution_id',gen_random_uuid()::text,'execution_mode','INDEPENDENT_CURATOR','contract_revision','5.11','parent_run_id',v_parent,'parent_assessment_id',a.id,'remediation_revision','AUTH004_VISUAL_SOURCE_REFRESH_20260819','direct_source_readback',true),repeat('0',64));
  end loop;

  v_val:='INPUT_VALIDATOR:v0.5r1-auth-54-v511-visual-refresh-'||substr(md5(random()::text||clock_timestamp()::text),1,8);
