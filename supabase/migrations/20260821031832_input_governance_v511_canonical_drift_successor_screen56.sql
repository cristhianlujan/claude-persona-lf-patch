...
declare
  ...
  v_assert jsonb;
begin
  if not exists (select 1 from programacion.input_readiness_runs r where r.id=v_parent and r.version_id=19 and r.pantalla_id=v_screen and r.status='COMPLETED' and r.contract_revision='5.11' and r.invalidated_at is null) then raise exception 'V511_DRIFT_PARENT_NOT_ELIGIBLE screen=% run=%',v_screen,v_parent; end if;
  if exists (select 1 from programacion.input_readiness_runs s where s.supersedes_run_id=v_parent and s.status in ('COMPLETED','BLOCKED')) then raise exception 'V511_DRIFT_PARENT_ALREADY_SUPERSEDED screen=% run=%',v_screen,v_parent; end if;
  ...
  select version_id,pantalla_id,universe_rule_id,id,scope || jsonb_build_object('mode','CANDIDATE_V511_CANONICAL_DRIFT_REVALIDATION','parent_run_id',id,'remediation','PR179_LIVE_SOURCE_DRIFT_RECONCILIATION_20260820'),universe_snapshot_sha256,family_count,'CURATING',v_cur,curator_component_id,contract_version
  ...
  for a in select * from programacion.input_family_assessments where run_id=v_parent order by family_code loop
    ...
    values(v_new,a.family_code,a.severity,a.applicability,a.coverage_status,a.well_defined_status,a.story_ready_status,a.implementation_ready_status,a.qa_ready_status,a.production_ready_status,a.source_refs,a.rationale||' | Recurado contra fuentes canónicas vigentes por reconciliación PR #179 2026-08-20.',a.blockers,a.negative_requirements,a.test_obligations,jsonb_build_object('component_id',46,'execution_id',gen_random_uuid()::text,'execution_mode','INDEPENDENT_CURATOR','contract_revision','5.11','parent_run_id',v_parent,'parent_assessment_id',a.id,'remediation_revision','PR179_LIVE_SOURCE_DRIFT_RECONCILIATION_20260820','direct_source_readback',true),repeat('0',64));
  end loop;
