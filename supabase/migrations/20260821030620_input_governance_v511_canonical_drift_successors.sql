...
begin
  if not exists (
    ...
    raise exception 'V511_DRIFT_PARENT_NOT_ELIGIBLE screen=% run=%',v_screen,v_parent;
  end if;
  ...
  if exists (
    ...
    raise exception 'V511_DRIFT_PARENT_ALREADY_SUPERSEDED screen=% run=%',v_screen,v_parent;
  end if;

  if programacion.fn_input_readiness_run_is_current(v_parent) then
    raise exception 'V511_DRIFT_PARENT_EXPECTED_STALE screen=% run=%',v_screen,v_parent;
  end if;
  ...
  insert into programacion.input_readiness_runs(
    ...
    universe_snapshot_sha256,family_count,status,curator_identity,curator_component_id,contract_version
  )
  ...
  for a in select * from programacion.input_family_assessments where run_id=v_parent order by family_code loop
    insert into programacion.input_family_assessments(
      ...
      jsonb_build_object(
        'component_id',46,'execution_id',gen_random_uuid()::text,
        'execution_mode','INDEPENDENT_CURATOR','contract_revision','5.11',
        'parent_run_id',v_parent,'parent_assessment_id',a.id,
  ...
  update programacion.input_readiness_runs
     set status='VALIDATING',validator_identity=v_val,validator_component_id=47
   where id=v_new;

  select source_snapshot_sha256 into v_sha
  from programacion.input_readiness_runs where id=v_new;
    ...
    update programacion.input_family_assessments
       set validator_outcome='PASS',validator_findings='[]'::jsonb,validator_identity=v_val,
           validator_evidence=jsonb_build_object(
             ...
             'execution_mode','INDEPENDENT_VALIDATOR','direct_source_readback',true,
             'contract_revision','5.11','source_snapshot_sha256',v_sha,
             'curator_sha256',a.curator_sha256,'semantic_depth_sha256',a.semantic_depth_sha256,
             'assertions',v_assert
           )
     where id=a.id;
  end loop;
  ...
  if not programacion.fn_input_readiness_run_is_current(v_new) then
    raise exception 'V511_DRIFT_POSTCHECK_CURRENTNESS_FAILED screen=% run=%',v_screen,v_new;
  end if;
end$$;