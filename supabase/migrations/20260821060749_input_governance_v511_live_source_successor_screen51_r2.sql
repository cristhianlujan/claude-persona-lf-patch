...
begin
  ...
  if v_parent is null then
    raise exception 'V511_R2_PARENT_NOT_FOUND screen=%',v_screen;
  end if;
  if programacion.fn_input_readiness_run_is_current(v_parent) then
    raise exception 'V511_R2_PARENT_EXPECTED_STALE screen=% run=%',v_screen,v_parent;
  end if;
  ...
  select version_id,pantalla_id,universe_rule_id,id,
    scope || jsonb_build_object('mode','CANDIDATE_V511_LIVE_SOURCE_REVALIDATION_R2','parent_run_id',id,'remediation','PR179_RULE_GRAPH_AND_EKB_DRIFT_RECONCILIATION_20260821'),
  ...
  for a in select * from programacion.input_family_assessments where run_id=v_parent order by family_code loop
    ...
    values(v_new,a.family_code,a.severity,a.applicability,a.coverage_status,a.well_defined_status,a.story_ready_status,a.implementation_ready_status,a.qa_ready_status,a.production_ready_status,a.source_refs,
      a.rationale||' | Recurado contra fuentes canónicas vigentes por reconciliación PR #179 2026-08-21.',a.blockers,a.negative_requirements,a.test_obligations,
      jsonb_build_object('component_id',46,'execution_id',gen_random_uuid()::text,'execution_mode','INDEPENDENT_CURATOR','contract_revision','5.11','parent_run_id',v_parent,'parent_assessment_id',a.id,'remediation_revision','PR179_RULE_GRAPH_AND_EKB_DRIFT_RECONCILIATION_20260821','direct_source_readback',true),repeat('0',64));
  end loop;
  ...
  if not programacion.fn_input_readiness_run_is_current(v_new) then
    raise exception 'V511_R2_POSTCHECK_CURRENTNESS_FAILED screen=% run=%',v_screen,v_new;
  end if;
end$$;