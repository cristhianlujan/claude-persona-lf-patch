...
begin
  if btrim(coalesce(p_execution_id,''))='' or btrim(coalesce(p_target_code,''))=''
    ...
    raise exception 'LF_PROFILE_CREATION_BEGIN_INPUT_INVALID';
  end if;
  ...
  select count(*) into route_count from public.lf_router_action_registry
   where asset_type='PERFIL' and action_code='PROFILE_CREATE' and operation_code='CREACION_PERFIL_LF'
     and status='ACTIVE' and write_allowed is true;
  if route_count<>1 then raise exception 'LF_PROFILE_CREATION_ROUTE_NOT_ACTIVE:%',route_count; end if;

  if not exists(select 1 from public.lf_operation_registry
                where operation_code='CREACION_PERFIL_LF' and applies_to_asset_type='PERFIL'
                  and lifecycle_state_code='OP_OPERATIONAL') then
    raise exception 'LF_PROFILE_CREATION_OPERATION_NOT_OPERATIONAL';
  end if;

  select count(*) into primary_count from public.lf_operation_contracts
   where operation_code='CREACION_PERFIL_LF' and status='ACTIVE_ENFORCEMENT'
     and contract_code='CONTRACT-PROFILE-GENERAL-DEPTH-LF-v0.1';
  select count(*) into supplement_count from public.lf_operation_contracts
   where operation_code='CREACION_PERFIL_LF' and status='ACTIVE_ENFORCEMENT'
     and contract_code='CONTRACT-CREACION-PERFIL-LF-NO-CLOSE-IF-REQUIRED-STEP-NOT-CLEAN-PASS-v0.1';
  if primary_count<>1 or supplement_count<>1 then
    raise exception 'LF_PROFILE_CREATION_CONTRACT_ROLES_INVALID:primary=% supplement=%',primary_count,supplement_count;
  end if;
  select * into oc from public.lf_operation_contracts
   where operation_code='CREACION_PERFIL_LF' and status='ACTIVE_ENFORCEMENT'
     and contract_code='CONTRACT-PROFILE-GENERAL-DEPTH-LF-v0.1';

  r:=public.fn_lf_operation_reserve_execution_v1(
    p_execution_id,'CREACION_PERFIL_LF','PERFIL',p_target_code,
    p_idempotency_key,p_request_sha256,p_actor_execution_id,p_target_repo,p_package_root,
    p_manifest || jsonb_build_object(
      'contract_code',oc.contract_code,
      'contract_sha',oc.contract_sha,
      'supplemental_contract_code','CONTRACT-CREACION-PERFIL-LF-NO-CLOSE-IF-REQUIRED-STEP-NOT-CLEAN-PASS-v0.1',
      'parent_execution_id',p_actor_execution_id
    )
  );

  select * into x from public.lf_operation_execution where execution_id=r->>'execution_id' for update;
  if not found or x.operation_code<>'CREACION_PERFIL_LF' or x.target_type<>'PERFIL'
     or x.target_code is distinct from p_target_code or x.target_repo is distinct from p_target_repo
     or x.target_path is distinct from p_package_root or x.status<>'IN_PROGRESS' then
    raise exception 'LF_PROFILE_CREATION_EXECUTION_BINDING_MISMATCH';
  end if;
  ...
  select * into st from public.lf_operation_steps
   where operation_code='CREACION_PERFIL_LF' and step_id='init_execution' and active=true;
  ...
  select * into b from public.lf_operation_step_judge_bindings
   where operation_code='CREACION_PERFIL_LF' and step_id='init_execution'
  ...
  if found then
    if existing.step_id='init_execution' and existing.status=b.clean_result_value then
      return r || jsonb_build_object('init_step','ALREADY_RECORDED_IDEMPOTENT','next_step','router');
    end if;
  ...
  insert into public.lf_operation_execution_steps(
    ...
    'Initialized transactionally by governed Profile Creation begin RPC.',p_actor_execution_id
  );
