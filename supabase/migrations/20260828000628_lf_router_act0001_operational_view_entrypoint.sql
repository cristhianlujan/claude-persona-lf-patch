...
begin
  ...
  -- Canonical operational discovery: only through the active Supabase search view.
  ...
  -- Master registry hydrates the already-resolved active asset identity.
  if v_asset_found then
    select a.* into v_asset
    from public.lf_activos a
    where a.codigo_activo=v_candidate_code and a.archived_at is null
    limit 1;
    if not found then
  ...
  -- CREATE duplicate detection is strict: only exact code/name/alias proves a duplicate.
  ...
  if v_rule.operation_resolution='NONE' then
    ...
    return jsonb_build_object(
      'status','READY_INSPECTION','router','ACT-0001','source','SUPABASE','discovery_source','public.v_lf_fuente_operativa_busqueda',
...
set metadata=coalesce(metadata,'{}'::jsonb) || jsonb_build_object(
      'runtime_discovery_source','public.v_lf_fuente_operativa_busqueda',
      'runtime_identity_hydration','public.lf_activos by codigo_activo after operational-view resolution',
      'runtime_entry_sequence',jsonb_build_array('ACT-0001','v_lf_fuente_operativa_busqueda','lf_activos','lf_router_action_registry','lf_operation_registry','lf_operation_contracts','lf_operation_step_contracts','v_lf_router_adapter_bindings','v_lf_operation_policy_snapshot')
    ),
