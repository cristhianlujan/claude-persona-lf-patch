...
begin
  ...
  -- Base64 for a <=10 MiB payload is bounded by ceil(n/3)*4. Allow only the
  -- exact encoding envelope plus a small fixed margin for padding; reject
  -- pathological staging content before aggregation/decoding.
  ...
  -- Idempotent retry: an exact SOURCE_IMAGE already persisted for this SHA is
  -- reused. This avoids duplicate evidence objects if a caller retries after a
  -- response/readback failure. Staged chunks are still cleaned below.
  ...
  -- Verify durable readback before deleting transient upload chunks.
  perform 1
  from private.lf_p0_review_evidence_objects_v1
...
comment on function public.fn_lf_p0_finalize_source_image_chunks_v1(
  uuid,text,text,text,integer,bigint,text,text,text,jsonb
) is 'Service-role-only fail-closed finalizer for private chunked SOURCE_IMAGE uploads. Requires contiguous chunks, exact bytes+SHA256, durable content readback, idempotent SHA reuse, and deletes staging only after verified persistence.';