-- Materialize the existing EVIDENCE_VERIFIER_V1 trust boundary for Agent Task receipts.
-- No new canonical state: reuse provenance_channels, provenance_receipts and evidence_verifications.

alter table programacion.provenance_channels disable trigger trg_provenance_channels_immutable;
update programacion.provenance_channels
set secret_sha256 = '20721ff49d97a3f2f19ff52270d12668caf04395f2710659bdf83bfe44387b6f'
...
begin
  ...
  order by ex.id desc,ev.id desc
  limit 1;

  if v is null then
    raise exception 'PENDING_WORKER_EVIDENCE_NOT_FOUND: agent-task://%',p_task_id;
  end if;
  return v;
end;
$function$;
...
begin
  if p_verification_payload is null or jsonb_typeof(p_verification_payload)<>'object' then
    raise exception 'EXTERNAL_VERIFICATION_PAYLOAD_REQUIRED';
  end if;
  if p_verification_method <> 'GITHUB_ACTIONS_OIDC_EXACT_EVIDENCE_V1' then
    raise exception 'EXTERNAL_VERIFICATION_METHOD_INVALID';
  end if;
  if p_verifier_identity !~ '^github-actions://cristhianlujan/claude-persona-lf-patch/\.github/workflows/story-agent-evidence-verifier\.yml@refs/heads/main#run-[0-9]+$' then
    raise exception 'EXTERNAL_VERIFIER_IDENTITY_INVALID';
  end if;
  if length(btrim(coalesce(p_verification_ref,'')))=0 then
    raise exception 'EXTERNAL_VERIFICATION_REF_REQUIRED';
  end if;
  ...
  if v_ex.estado<>'RUNNING' or v_ex.request_ref!~'^agent-task://[1-9][0-9]*$' then
    raise exception 'EXTERNAL_VERIFY_AGENT_TASK_RUNNING_REQUIRED';
  end if;
  if v_ex.head_sha is distinct from p_expected_head_sha then
    raise exception 'EXTERNAL_VERIFY_HEAD_MISMATCH';
  end if;
  ...
  if p_verifier_identity=v_ev.source_system or p_verifier_identity=v_ev.source_ref then
    raise exception 'EXTERNAL_VERIFY_SELF_VERIFICATION_FORBIDDEN';
  end if;
  ...
  if v_ev.metadata#>>'{worker_receipt,status}'<>'PASS'
     ...
     or v_ev.metadata#>>'{worker_receipt,context_pack_sha256}'<>v_cp.context_sha256 then
    raise exception 'EXTERNAL_VERIFY_WORKER_RECEIPT_BINDING_INVALID';
  end if;
  ...
  if p_verification_payload->>'execution_id' is distinct from v_ex.id::text
    ...
    raise exception 'EXTERNAL_VERIFY_PAYLOAD_IDENTITY_MISMATCH';
  end if;

  if exists(select 1 from programacion.evidence_verifications where evidence_id=v_ev.id and verification_status='VERIFIED') then
    raise exception 'EXTERNAL_VERIFY_DUPLICATE_VERIFIED_EVIDENCE';
  end if;
  ...
  from programacion.issue_provenance_receipt(
    ...
    'evidence_verification','evidence:'||v_ev.id::text,v_subject_sha,p_verifier_identity,
    p_verification_ref,v_receipt_payload
  );
...
create or replace function public.fn_agent_task_pending_worker_evidence_v1(p_task_id bigint)
returns jsonb
language sql
stable
security definer
set search_path to 'pg_catalog','programacion'
as $function$
  select programacion.fn_agent_task_pending_worker_evidence_v1(p_task_id);
$function$;

create or replace function public.fn_agent_task_external_verify_worker_evidence_v1(
...
set search_path to 'pg_catalog','programacion'
as $function$
  select programacion.fn_external_verify_worker_evidence_v1(
...
revoke all on function programacion.fn_agent_task_pending_worker_evidence_v1(bigint) from public,anon,authenticated;
revoke all on function programacion.fn_external_verify_worker_evidence_v1(bigint,bigint,text,text,text,text,text,text,jsonb,text) from public,anon,authenticated;
revoke all on function public.fn_agent_task_pending_worker_evidence_v1(bigint) from public,anon,authenticated;
revoke all on function public.fn_agent_task_external_verify_worker_evidence_v1(bigint,bigint,text,text,text,text,text,text,jsonb,text) from public,anon,authenticated;
grant execute on function public.fn_agent_task_pending_worker_evidence_v1(bigint) to service_role;
grant execute on function public.fn_agent_task_external_verify_worker_evidence_v1(bigint,bigint,text,text,text,text,text,text,jsonb,text) to service_role;

-- Fail-closed migration self-tests: exact channel hash, no browser/user execution, wrong token rejected.
do $selftest$
declare v_hash text; v_error text;
begin
  select secret_sha256 into v_hash from programacion.provenance_channels where channel_code='EVIDENCE_VERIFIER_V1';
  if v_hash is distinct from '20721ff49d97a3f2f19ff52270d12668caf04395f2710659bdf83bfe44387b6f' then
    raise exception 'SELFTEST_EVIDENCE_VERIFIER_CHANNEL_HASH_MISMATCH';
  end if;
  if has_function_privilege('anon','public.fn_agent_task_external_verify_worker_evidence_v1(bigint,bigint,text,text,text,text,text,text,jsonb,text)','EXECUTE')
     or has_function_privilege('authenticated','public.fn_agent_task_external_verify_worker_evidence_v1(bigint,bigint,text,text,text,text,text,text,jsonb,text)','EXECUTE') then
    raise exception 'SELFTEST_EVIDENCE_VERIFIER_USER_EXECUTE_EXPOSED';
  end if;
  begin
    perform programacion.issue_provenance_receipt(
      'EVIDENCE_VERIFIER_V1',repeat('x',64),'EVIDENCE_VERIFICATION',null,repeat('a',40),
      'evidence_verification','evidence:0',repeat('b',64),'migration-negative-selftest','selftest://wrong-token',
      jsonb_build_object('head_sha',repeat('a',40),'subject_type','evidence_verification','subject_ref','evidence:0','subject_sha256',repeat('b',64),'verification_status','REJECTED','verifier_identity','migration-negative-selftest')
    );
    raise exception 'SELFTEST_WRONG_TOKEN_WAS_ACCEPTED';
  exception when others then
    v_error:=sqlerrm;
    if v_error not like '%channel/token mismatch%' then raise; end if;
  end;
end;
$selftest$;
