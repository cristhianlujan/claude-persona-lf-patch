...
begin
  ...
  where id=p_run_id and version_id=19 and scope->>'analysis_revision'='INPUT_GOV_REMEDIATION_1_3';
  ...
  if v_status='CURATING' then
    ...
    set status='VALIDATING',validator_identity=p_validator_identity,validator_component_id=v_validator_component
    where id=p_run_id;
    ...
    raise exception 'INPUT_REMEDIATION_VALIDATOR_INVALID_RUN_STATUS:%',v_status;
  end if;
  ...
  loop
    ...
    set validator_outcome='PASS',
        ...
        validator_evidence=jsonb_build_object(
          'component_id',v_validator_component,'execution_id',v_exec_id,
          ...
          'bootstrap_classifier_sha256',v_expected->>'classifier_sha256',
          'analysis_revision','INPUT_GOV_REMEDIATION_1_3','assertions',v_assertions
        ),
  ...
  v_payload:=jsonb_build_object(
    'status','COMPLETED','run_id',p_run_id,'pantalla_id',v_pantalla_id,'family_count',v_family_count,
    'validator_pass_count',v_pass,'validator_identity',p_validator_identity,'required_role','DISPATCHER_FINALIZE',
    'analysis_revision','INPUT_GOV_REMEDIATION_1_3','proposal_validation',v_prop,
...
declare
  ...
  v_bootstrap boolean;
begin
  select scope->>'analysis_revision',
         supersedes_run_id is null and scope->>'mode'='GOVERNED_CANONICAL_BOOTSTRAP_V1'
    into v_analysis,v_bootstrap
  from programacion.input_readiness_runs where id=p_run_id and version_id=19;

  if v_analysis='INPUT_GOV_REMEDIATION_1_3' then
    return programacion.fn_input_governance_validate_v2(p_run_id,p_validator_identity);
  end if;
  if coalesce(v_bootstrap,false) then
    return programacion.fn_input_governance_bootstrap_validate_v1(p_run_id,p_validator_identity);
  end if;
