...
returns trigger language plpgsql security definer set search_path=pg_catalog,programacion as $$
...
begin
  ...
  if v_revision not in ('5.10','5.11') then return new; end if;
...
begin
  ...
  if v_revision not in ('5.10','5.11') or new.applicability<>'APPLICABLE' then return new; end if;
