...
create table private.lf_p0_execution_runs_v1 (
  ...
  check (
    ...
    or (
      ...
      and unresolved_medium = 0
      and mutation_escapes = 0
    )
  )
);
...
create table private.lf_p0_execution_elements_v1 (
  ...
  bbox jsonb not null check (
    ...
    and bbox ?& array['x', 'y', 'width', 'height']
    and (bbox->>'x')::integer >= 0
    and (bbox->>'y')::integer >= 0
    and (bbox->>'width')::integer > 0
    and (bbox->>'height')::integer > 0
  ),
  ...
  foreign key (execution_id, parent_element_id)
    references private.lf_p0_execution_elements_v1(execution_id, element_id)
...
create table private.lf_p0_execution_element_evidence_v1 (
  ...
  created_at timestamptz not null default clock_timestamp(),
  primary key (execution_id, evidence_unit_id),
  foreign key (execution_id, element_id)
    references private.lf_p0_execution_elements_v1(execution_id, element_id) on delete restrict,
  foreign key (execution_id, evidence_unit_id)
    references private.lf_p0_execution_evidence_units_v1(execution_id, evidence_unit_id) on delete restrict
);
...
insert into private.lf_p0_invalidated_loop_versions_v1(loop_version, invalidated_at, reason, source_ref)
values
  (
    'p0-v4-r2-pre-atomicity',
    '2026-08-12T00:00:00Z'::timestamptz,
    'Sibling contamination, unjustified fragmentation and zero-assignment visual omissions invalidate prior PASS results.',
    'sandbox/story_creator_p0_visual/v1.1/evals/p0-closed-loop-runtime-config-v4.json'
  ),
  (
    'unversioned-pr137-head-f51ddc0c9b0f854bec4dc9fa68672e2a7ac5d022',
    '2026-08-12T00:00:00Z'::timestamptz,
    'Sibling contamination, unjustified fragmentation and zero-assignment visual omissions invalidate prior PASS results.',
    'sandbox/story_creator_p0_visual/v1.1/evals/p0-closed-loop-runtime-config-v4.json'
  );
...
begin
  foreach relation_name in array array[
    'lf_p0_execution_runs_v1',
    'lf_p0_execution_elements_v1',
    'lf_p0_execution_evidence_units_v1',
    'lf_p0_execution_element_evidence_v1',
    'lf_p0_execution_records_v1',
    'lf_p0_execution_artifacts_v1',
    'lf_p0_execution_transitions_v1',
    'lf_p0_execution_persist_attempts_v1',
    'lf_p0_invalidated_loop_versions_v1'
  ]
  loop
    execute format(
      'create trigger %I before update or delete on private.%I for each row execute function private.fn_lf_p0_forbid_mutation_v1()',
...
begin
  ...
  select request_fingerprint_sha256 into v_existing
  from private.lf_p0_execution_runs_v1 where execution_id = v_execution_id;
  if found then
  ...
  if (v_execution->>'verdict') = 'PASS' and exists (
    select 1 from private.lf_p0_invalidated_loop_versions_v1
  ...
  insert into private.lf_p0_execution_runs_v1(
    ...
    coalesce((v_execution->>'mutation_escapes')::integer, 0), v_fingerprint
  );
  ...
  for v_item in select value from jsonb_array_elements(p_bundle->'artifacts') loop
    insert into private.lf_p0_execution_artifacts_v1(
      ...
      nullif(v_item->>'external_evidence_ref', ''), coalesce(v_item->'payload', '{}'::jsonb)
    );
  end loop;
  ...
  if not exists (
       select 1 from private.lf_p0_execution_records_v1
...
as $$
  ...
  from (select * from private.lf_p0_execution_runs_v1 where execution_id = p_execution_id) r;
$$;
...
comment on function private.fn_persist_lf_p0_execution_v1(jsonb) is
  'Atomically persists one immutable P0 execution graph; identical retries are idempotent and conflicting retries fail.';
comment on function private.fn_reconstruct_lf_p0_execution_v1(text) is
  'Reconstructs the normalized P0 execution graph by execution_id without exposing private tables.';
