...
begin
  ...
  if v_pantalla_id is null then
    raise exception 'V58_ASSERTION_NEW_RUN_NOT_FOUND:%',p_new_run_id;
  end if;
  ...
  for v_old in
    ...
    where a.run_id=p_parent_run_id
      and a.family_code=p_family_code
  loop
    if v_pantalla_id=51
       and p_family_code='VISUAL_EVIDENCE'
        ...
        'expected',jsonb_build_array(
          jsonb_build_object(
            'artifact',jsonb_build_object(
              ...
              'storage_metadata',jsonb_build_object(
                'variant_code','B2B-AUTH-001-DESKTOP-LIGHT',
          ...
          jsonb_build_object(
            'artifact',jsonb_build_object(
              ...
              'storage_provider','GOOGLE_DRIVE',
              'storage_metadata',jsonb_build_object(
                'variant_code','B2B-AUTH-001-TABLET-LIGHT',
                'canonical_canvas',true
              )
            )
          ),
          jsonb_build_object(
            'artifact',jsonb_build_object(
              ...
              'storage_provider','GOOGLE_DRIVE',
              'storage_metadata',jsonb_build_object(
                'variant_code','B2B-AUTH-001-MOBILE-LIGHT',
                'canonical_canvas',true
              )
            )
          )
    ...
    if v_rebound->>'result'<>'PASS' then
      raise exception 'V58_REBOUND_ASSERTION_FAILED screen=% family=% source=% path=%',
        v_pantalla_id,p_family_code,v_rebound->'source_ref',v_rebound->'path';
    end if;
    v_out:=v_out || jsonb_build_array(v_rebound);
  end loop;

  if jsonb_array_length(v_out)=0 then
    raise exception 'V58_ASSERTION_SET_EMPTY:%:%',v_pantalla_id,p_family_code;
  end if;
