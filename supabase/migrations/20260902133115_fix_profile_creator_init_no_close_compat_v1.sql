-- Profiles LF — reconcile the governed init result with the canonical no-close gate.
-- Source-first, fail-closed: init_execution is the only required step allowed to use
-- STEP_CLEAN_PASS; every post-init required step still requires STEP_PASS_WITH_EVIDENCE.
...
begin
  ...
  select count(*) into v_missing_count from public.lf_operation_steps s
  ...
  from public.lf_operation_steps s left join public.lf_operation_execution_steps es on es.execution_id=p_execution_id and es.step_id=s.step_id
  where s.operation_code='CREACION_PERFIL_LF' and s.required is true and s.active is true
  ...
  select count(*) into v_final_bad_pass_count from public.lf_operation_execution_steps
  ...
  return jsonb_build_object(
    ...
    'missing_required_steps',v_missing_count,'required_steps_not_clean_pass',v_not_clean_count,
    'pass_with_restrictions_count',v_restricted_count,'blocked_count',v_blocked_count,'return_to_worker_count',v_return_count,
    'closure_allowed_false',v_closure_allowed_false,'blocked_from_closure_true',v_blocked_from_closure_true,
    'security_hold_active',v_security_hold_true,'final_steps_invalid_pass_count',v_final_bad_pass_count,'final_steps_block_required',v_block,
    'init_status_contract','STEP_CLEAN_PASS','post_init_status_contract','STEP_PASS_WITH_EVIDENCE');
end;
$function$;
