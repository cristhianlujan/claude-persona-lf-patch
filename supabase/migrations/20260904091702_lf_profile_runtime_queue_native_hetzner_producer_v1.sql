...
as $function$
begin
  -- HETZNER is the primary transport. Text-only queue-native work may omit the
  -- governed artifact envelope. Any image/screen-bound work must carry it.
  ...
  -- GitHub Actions is a backup only. Every new backup request must say why it
  -- was selected; legacy rows remain readable/updatable without rewriting history.
