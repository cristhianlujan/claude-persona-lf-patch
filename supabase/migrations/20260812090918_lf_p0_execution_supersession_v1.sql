-- Add explicit supersession lineage without mutating prior execution rows.

alter table private.lf_p0_execution_runs_v1
  add column supersedes_execution_id text,
  add column supersession_reason text,
  add constraint lf_p0_execution_runs_v1_supersedes_fkey
    foreign key (supersedes_execution_id)
    references private.lf_p0_execution_runs_v1(execution_id)
    on delete restrict,
  add constraint lf_p0_execution_runs_v1_supersession_check
    check (
      (supersedes_execution_id is null and supersession_reason is null)
      or (
        supersedes_execution_id is not null
        and supersession_reason is not null
        and length(supersession_reason) > 0
        and supersedes_execution_id <> execution_id
      )
    );

create index lf_p0_execution_runs_v1_supersedes_idx
  on private.lf_p0_execution_runs_v1(supersedes_execution_id)
  where supersedes_execution_id is not null;

-- Historical note: the applied migration also replaced the persistence function so
-- supersedes_execution_id/supersession_reason were accepted immediately. The final,
-- stricter definition is source-controlled in 20260812100613 and supersedes that body.
...
begin
  ...
  insert into private.lf_p0_execution_runs_v1(
    ...
    p0_5_authorized,production_authorized,unresolved_critical,unresolved_high,
    unresolved_medium,mutation_escapes,request_fingerprint_sha256,
    ...
    v_execution->>'configuration_sha256',v_execution->'languages',v_execution->'dependencies',
    coalesce((v_execution->>'acceptance_declared')::boolean,false),
  ...
  if exists(select 1 from private.lf_p0_execution_evidence_units_v1 u where u.execution_id=v_execution_id and not exists(select 1 from private.lf_p0_execution_element_evidence_v1 l where l.execution_id=u.execution_id and l.evidence_unit_id=u.evidence_unit_id)) then
