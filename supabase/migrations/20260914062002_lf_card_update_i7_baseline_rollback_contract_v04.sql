...
-- Repairs IR-F05 and keeps rollback as a no-provider-call intent only.
...
create or replace function public.lf_validate_card_reversible_baseline_v1(
  p_execution_id text,
...
begin
  ...
  if p_evidence_payload->'baseline_reversible' is distinct from 'true'::jsonb
     or nullif(p_evidence_payload->>'baseline_canonical_content','') is null
     or (p_evidence_payload->>'baseline_content_sha256') is distinct from v_hash
     or (p_evidence_payload->>'baseline_content_sha256') is distinct from (v_binding->>'content_sha256')
     or (p_evidence_payload->>'baseline_block_excerpt_hash') is distinct from (v_binding->>'content_sha256')
     or (p_evidence_payload->>'baseline_content_chars')::integer is distinct from length(p_evidence_payload->>'baseline_canonical_content')
     ...
     or (p_evidence_payload->>'baseline_block_anchor_or_path') is distinct from v_expected_anchor
     or (p_evidence_payload->>'baseline_carrier_revision') is distinct from v_expected_revision
     or (p_evidence_payload->>'baseline_anchor_start_index')::integer is distinct from (v_binding->>'anchor_start_index')::integer
     or (p_evidence_payload->>'baseline_last_bound_paragraph_end_index')::integer is distinct from (v_binding->>'last_bound_paragraph_end_index')::integer then
    return jsonb_build_object('valid',false,'code','BASELINE_REVERSIBILITY_OR_IDENTITY_MISMATCH');
  end if;
...
begin
  select * into v_execution from public.lf_operation_execution where execution_id=p_execution_id;
  if not found or v_execution.operation_code is distinct from 'ACTUALIZACION_CARD_LF' or v_execution.target_type is distinct from 'CARD' then
    return jsonb_build_object('outcome','BLOCKED','code','ROLLBACK_EXECUTION_IDENTITY_INVALID');
  end if;
  select * into v_card from public.lf_activos where codigo_activo=v_execution.target_code and tipo_activo='CARD' and archived_at is null;
  ...
  if (v_binding->>'carrier_type') is distinct from 'GOOGLE_DOC_EMBEDDED_BLOCK' then
    return jsonb_build_object('outcome','BLOCKED','code','ROLLBACK_PROVIDER_NOT_IMPLEMENTED','scope','GOOGLE_DOC_ONLY');
  end if;
  ...
  if not found or v_baseline.status is distinct from 'STEP_PASS_WITH_EVIDENCE' then
    return jsonb_build_object('outcome','BLOCKED','code','ROLLBACK_REVERSIBLE_BASELINE_NOT_AVAILABLE');
  end if;
  v_baseline_check:=public.lf_validate_card_reversible_baseline_v1(p_execution_id,v_baseline.evidence_payload);
  if (v_baseline_check->'valid') is distinct from 'true'::jsonb then
    return jsonb_build_object('outcome','BLOCKED','code','ROLLBACK_BASELINE_NOT_EXACT','baseline_validation',v_baseline_check);
  end if;
  ...
  if not found or v_readback.status is distinct from 'STEP_PASS_WITH_EVIDENCE' then
    return jsonb_build_object('outcome','BLOCKED','code','ROLLBACK_POST_WRITE_READBACK_NOT_AVAILABLE');
  end if;
  v_current_revision:=v_readback.evidence_payload->>'readback_carrier_revision';
  if nullif(v_current_revision,'') is null then
    return jsonb_build_object('outcome','BLOCKED','code','ROLLBACK_CURRENT_REVISION_MISSING');
  end if;

  v_intent:=jsonb_build_object(
    'intent_schema','LF_CARD_ROLLBACK_INTENT_V1',
    'execution_id',p_execution_id,
    'card_code',v_execution.target_code,
    'carrier_type','GOOGLE_DOC_EMBEDDED_BLOCK',
    ...
    'bounded_target',jsonb_build_object(
      'document_id',v_binding->>'document_id',
      'tab_id',v_binding->>'tab_id',
      'block_anchor',v_binding->>'block_anchor',
      'anchor_start_index',v_baseline.evidence_payload->>'baseline_anchor_start_index',
      'last_bound_paragraph_end_index',v_baseline.evidence_payload->>'baseline_last_bound_paragraph_end_index'
    ),
    'restore_content',v_baseline.evidence_payload->>'baseline_canonical_content',
    'restore_content_sha256',v_baseline.evidence_payload->>'baseline_content_sha256',
    'baseline_evidence_ref',v_baseline.evidence_ref,
    'post_write_readback_ref',v_readback.evidence_ref,
    'provider_call_authorized',false,
    'rollback_write_executed',false,
