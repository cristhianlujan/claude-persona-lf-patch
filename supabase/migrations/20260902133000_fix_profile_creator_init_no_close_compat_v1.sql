...
begin
  ...
  where s.operation_code='CREACION_PERFIL_LF'
  ...
  where s.operation_code='CREACION_PERFIL_LF'
    ...
    and s.active is true
    and (
      (s.step_id = 'init_execution' and coalesce(es.status,'MISSING') <> 'STEP_CLEAN_PASS')
      or
      (s.step_id <> 'init_execution' and coalesce(es.status,'MISSING') <> 'STEP_PASS_WITH_EVIDENCE')
    );

  select count(*) into v_restricted_count
  from public.lf_operation_execution_steps
  where execution_id=p_execution_id
    and status in ('PASS_WITH_RESTRICTIONS','STEP_PASS_WITH_RESTRICTIONS');

  select count(*) into v_blocked_count
  from public.lf_operation_execution_steps
  where execution_id=p_execution_id
    and status in ('BLOCKED','STEP_BLOCKED','MISSING');

  select count(*) into v_return_count
  from public.lf_operation_execution_steps
  where execution_id=p_execution_id
    and status in ('RETURN_TO_WORKER','STEP_RETURN_TO_WORKER');

  select count(*) into v_final_bad_pass_count
  from public.lf_operation_execution_steps
  where execution_id=p_execution_id
    and step_id in ('contract_judge','close','report_output')
    and status in ('PASS','OK','DONE','VALIDATED','STEP_PASS_WITH_EVIDENCE')
    and (
      v_missing_count > 0
      or v_not_clean_count > 0
      or v_restricted_count > 0
      or v_blocked_count > 0
      or v_return_count > 0
      or coalesce(v_manifest->>'closure_allowed','') = 'false'
      or coalesce(v_manifest->>'blocked_from_closure','') = 'true'
      or coalesce(v_manifest->>'security_hold_continues','') = 'true'
  ...
  v_block := v_missing_count > 0
    or v_not_clean_count > 0
    or v_restricted_count > 0
    or v_blocked_count > 0
    or v_return_count > 0
    or v_closure_allowed_false
    or v_blocked_from_closure_true
    or v_security_hold_true
    or coalesce(v_manifest->>'mini_judge_verdict','') = 'PASS_WITH_RESTRICTIONS';
