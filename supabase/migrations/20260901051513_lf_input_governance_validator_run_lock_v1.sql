-- Serialize concurrent Validator chunk attempts for the same readiness run.
-- This preserves terminal receipt immutability and prevents overlapping PENDING snapshots.
...
begin
  ...
  -- One transaction at a time may validate a given readiness run. Concurrent
  -- retries wait here and then re-read PENDING rows after the prior chunk commits.
