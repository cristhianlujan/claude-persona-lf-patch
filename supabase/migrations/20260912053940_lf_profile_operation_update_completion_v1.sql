-- LF_PROFILE_OPERATION_UPDATE_COMPLETION_V1
-- ACTUALIZACION_PERFIL_LF must close the parent execution transactionally when report_output is clean.
-- Clean report_output replays also repair legacy IN_PROGRESS parent rows, but only while every required step remains clean.
