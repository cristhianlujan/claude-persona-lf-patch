...
-- Paired with 20260907023000.
-- Supports exactly two valid states:
--   A) forward objects still active -> restore exact baseline + drop canary-only objects;
--   B) safety finalizer already restored exact baseline -> functional no-op, ledger close only.
-- Any third state fails closed.
...
begin
  ...
  if v_count=4 then
    ...
    if encode(extensions.digest(convert_to(v_rebind,'UTF8'),'sha256'),'hex') <> '1bbf57bb5f51fff22e540664a3fe4e07a547a5e62b855c69abb2f981e5d92c79' then
      raise exception 'S28_IG006_ROLLBACK_SAFETY_FINALIZED_REBIND_SHA_MISMATCH';
    end if;
    if encode(extensions.digest(convert_to(v_assertions,'UTF8'),'sha256'),'hex') <> 'd02e3d07523debdb2acf38f46a7ebfd4c0b7d8d5238ed4b5d13de3581e9d4822' then
      raise exception 'S28_IG006_ROLLBACK_SAFETY_FINALIZED_ASSERTIONS_SHA_MISMATCH';
    end if;
    if encode(extensions.digest(convert_to(v_rebind_assertion,'UTF8'),'sha256'),'hex') <> 'b3ab572553e592fa8e8c64b9031ec741936995be67c2c469c2a0834ce11d6f9e' then
      raise exception 'S28_IG006_ROLLBACK_SAFETY_FINALIZED_REBIND_ASSERTION_SHA_MISMATCH';
    end if;
    if encode(extensions.digest(convert_to(v_guard,'UTF8'),'sha256'),'hex') <> 'd000493dce8f7d25139c20c75025a0a0c6dd5041547f6469aecf6d373acad574' then
      raise exception 'S28_IG007_ROLLBACK_SAFETY_FINALIZED_GUARD_SHA_MISMATCH';
    end if;
    if encode(extensions.digest(convert_to(v_exec,'UTF8'),'sha256'),'hex') <> '3290e752c27a46a089ed93d9a15769b7b9ca416f00d389c681431fde977da588' then
      raise exception 'S28_IG006_ROLLBACK_SAFETY_FINALIZED_EXECUTE_SHA_MISMATCH';
    end if;
...
begin
  ...
  if encode(extensions.digest(convert_to(v_exec,'UTF8'),'sha256'),'hex') <> '3290e752c27a46a089ed93d9a15769b7b9ca416f00d389c681431fde977da588' then
    raise exception 'S28_IG006_ROLLBACK_EXECUTE_SHA_MISMATCH';
  end if;
