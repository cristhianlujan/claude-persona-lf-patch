...
-- Purpose:
--   1) allow an exact-head candidate execution to be materialized while the only
--      unresolved task blocker is the independent F03 audit requirement;
--   2) keep that requirement visible in readiness.blockers;
--   3) allow Worker v10 Acceptance/Delivery authority to consume only a guarded
--      F03 receipt contract v5 bound to the same execution/repo/candidate HEAD.
-- No new tables. No PASS inference. No relaxation for business/dependency blockers.
...
BEGIN
  SELECT pg_get_functiondef('programacion.fn_task_readiness(bigint)'::regprocedure)
    INTO v_def;

  IF strpos(v_def,v_old)=0 THEN
    RAISE EXCEPTION 'PROG017_READINESS_PATCH_SOURCE_MISMATCH';
  END IF;
  IF strpos(replace(v_def,v_old,v_new),v_old)>0 THEN
    RAISE EXCEPTION 'PROG017_READINESS_PATCH_NOT_APPLIED';
  END IF;
