-- Effective PASS and promotion require service-role nonce evidence V6.
...
begin
  ...
  if not found or not a.is_current or a.validation_status<>'PASS_WITH_EVIDENCE' then return false; end if;
  ...
  if jsonb_typeof(e)<>'object'
     ...
     or e->>'writer_authentication'<>'SERVICE_ROLE_NONCE_V6'
  ...
  if not found
     ...
     or not private.fn_reconciliation_nonce_v6_valid(g.id)
  then return false; end if;
  ...
  if not found
     ...
     or v_event.payload->>'writer_authentication'<>'SERVICE_ROLE_NONCE_V6'
  ...
  loop
    ...
    if not exists(
      ...
      where t.id=v_test_id
        ...
        and private.fn_gate_nonce_v6_valid(t.id)
    ) then return false; end if;
  end loop;
...
begin
  ...
  then
    raise exception using errcode='23514',message='reconciliation run is not natively protected, nonce-authenticated, current and promotable';
  end if;
  ...
  then
    raise exception using errcode='23514',message='nonce-authenticated gate test set is not current and promotable';
  end if;
  ...
  if jsonb_array_length(private.fn_lf_dependency_array(a.dependencies))=0 then
    v_dependency_declaration:=jsonb_build_object(
      ...
      'rationale','No internal artifact dependencies are declared; repository content, commit, workflow and nonce-authenticated gate evidence were independently verified.',
