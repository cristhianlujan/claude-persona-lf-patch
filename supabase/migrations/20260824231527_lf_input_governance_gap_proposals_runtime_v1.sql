  ...
  loop
    ...
    loop
      ...
      values(
        ...
        jsonb_build_array('GOV-015_CLASSIFICATION_APPLIED','NO_PROPOSAL_AS_CANONICAL_SOURCE'),
        'PROPOSED',
...
create or replace function programacion.fn_input_governance_validate_gap_proposals_v1(
  p_run_id bigint,
...
begin
  ...
  if p_validator_identity is distinct from v_run.validator_identity or p_validator_identity=v_run.curator_identity then
    raise exception 'INPUT_REMEDIATION_VALIDATOR_IDENTITY_INVALID:%',p_run_id;
  end if;
  ...
  loop
    v_readbacks:='[]'::jsonb;
    for r in select value from jsonb_array_elements(p.source_refs)
    loop
      v_obs:=programacion.fn_input_resolve_source_ref(r,v_run.pantalla_id,v_run.version_id);
      v_readbacks:=v_readbacks||jsonb_build_array(
        jsonb_build_object(
          'source_ref',r,
          'source_observed_sha256',programacion.fn_v09_sha256_jsonb(v_obs)
        )
      );
    end loop;

    update programacion.input_gap_proposals
    set validator_identity=p_validator_identity,
        validator_outcome='PASS',
        status=case when proposal_kind='HUMAN_DECISION_REQUIRED' then 'HUMAN_DECISION_REQUIRED' else 'VALIDATED' end,
        validator_evidence=jsonb_build_object(
          'direct_source_readback',true,
          'source_snapshot_sha256',v_run.source_snapshot_sha256,
          'source_readbacks',v_readbacks,
          'analysis_revision','INPUT_GOV_REMEDIATION_1_3',
          'proposal_is_canonical_source',false,
          'automatic_canonicalization','DENY'
        ),
  ...
  return jsonb_build_object(
    'run_id',p_run_id,
    'validated_proposal_count',v_count,
    'human_decision_required_count',v_human,
    'analysis_revision','INPUT_GOV_REMEDIATION_1_3'
  );
end;
$function$;
...
as $function$
  select jsonb_build_object(
    ...
    'human_decision_count',count(*) filter(where status='HUMAN_DECISION_REQUIRED' and validator_outcome='PASS'),
    'internal_validated_remediation_count',count(*) filter(where status='VALIDATED' and validator_outcome='PASS'),
    'owner_visible_count',count(*) filter(where status='HUMAN_DECISION_REQUIRED' and validator_outcome='PASS'),
    'proposals',coalesce(
      jsonb_agg(
        jsonb_build_object(
          'id',id,'family_code',family_code,'gap_code',gap_code,'proposal_kind',proposal_kind,
          'status',status,'validator_outcome',validator_outcome,
          'gap_classification',proposed_payload->>'gap_classification',
          'owner_visible',(status='HUMAN_DECISION_REQUIRED' and validator_outcome='PASS')
        ) order by id
      ),
      '[]'::jsonb
    )
