-- S30 R15 v2: make the ACTUALIZACION_ESTRATEGIA_LF pre-write sequence physically non-skippable.
-- Recovery successor to the unapplied 20260915013230 source; fixes fail-closed post-verification quoting only.
...
BEGIN
  ...
  IF NOT FOUND
     ...
     OR v_exec.target_path IS DISTINCT FROM 'supabase/migrations/20260915182804_s30_strategy_update_required_step_sequence_guard_v2.sql' THEN
...
BEGIN
  ...
  END IF;
  IF strpos(v_def,$q$st.required is true$q$)=0
     OR strpos(v_def,$q$st.active is true$q$)=0
     OR strpos(v_def,$q$es.status=b.clean_result_value$q$)=0
     OR strpos(v_def,$q$recorded_by_rpc'='lf_strategy_update_begin_v1$q$)=0
     OR strpos(v_def,$q$core_recorder'='lf_record_operation_step_core_v1$q$)=0 THEN
...
SET manifest=manifest || jsonb_build_object(
      ...
      'recovery_source_version','20260915182804',
      'recovery_of_unapplied_source_version','20260915013230'
    ),
    status='COMPLETED',
