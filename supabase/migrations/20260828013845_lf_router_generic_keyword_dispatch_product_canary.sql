...
set metadata = jsonb_set(
      ...
      '{keywords}',
      '["mvp","primera version","alcance de producto","prioridad de producto","criterios de aceptacion","intencion de negocio","brief operativo","trade-off","decision de producto","scope de producto"]'::jsonb,
      true
    ),
    updated_at = now(),
    updated_by_execution_id = 'EXEC-ROUTER-CAPABILITY-PRODUCT-CANARY-20260827-001'
where codigo_activo='PERFIL-PRODUCT-DIRECTOR-LF';
...
begin
  ...
  if v_action is null then
    if v_req ~ '(^| )(consulta|consultar|version|estado|metadata|existe)( |$)' then
      v_action:='ASSET_INSPECTION';
    elsif v_type_hint='PERFIL'
      and v_req ~ '(^| )(crea|crear|creame|nuevo|nueva)( |$)'
      and v_req ~ '(^| )(perfil|profile)( |$)' then
      v_action:='PROFILE_CREATE';
    elsif v_type_hint='PERFIL'
      and v_req ~ '(^| )(corrige|corregir|mejora|mejorar|actualiza|actualizar|remedia|remediar|repara|reparar|modifica|modificar)( |$)'
      and v_req ~ '(^| )(perfil|profile)( |$)' then
      v_action:='PROFILE_UPDATE';
    elsif v_type_hint='PERFIL' and v_asset_found then
