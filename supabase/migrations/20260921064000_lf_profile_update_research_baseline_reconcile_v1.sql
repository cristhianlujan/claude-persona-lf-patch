...
'Governed post-merge profile reconciliation. Preserves operational/runtime/document/automatic-impact state and reconciles source identity plus optional PROFILE_RESEARCH_BASELINE_BINDING_V1 policy from the execution manifest. Default remains NOT_REQUIRED.';

do $post$
begin
  if position('research_baseline_policy_reconciled' in pg_get_functiondef('public.lf_profile_update_post_merge_reconcile_v1(text,text,text,text,text,integer,text)'::regprocedure))=0
     or position('PROFILE_RESEARCH_BASELINE_BINDING_V1' in pg_get_functiondef('public.lf_profile_update_post_merge_reconcile_v1(text,text,text,text,text,integer,text)'::regprocedure))=0 then
    raise exception 'LF_PROFILE_UPDATE_RESEARCH_BASELINE_RECONCILE_POSTCHECK_FAILED';
  end if;
end
$post$;
