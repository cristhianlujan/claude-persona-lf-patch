DO $pre$ DECLARE v text; BEGIN SELECT max(version) INTO v FROM supabase_migrations.schema_migrations; IF v IS DISTINCT FROM '20260914065046' THEN RAISE EXCEPTION 'S30_MATRIX_ENGINE_LEDGER_DRIFT:%',v; END IF; END $pre$;
...
BEGIN
 ...
 WHEN 'ROUTER_NATURAL_ACTION' THEN
   rp:=public.lf_router_resolve_v1(p_probe->>'request',NULL,NULL,'STRATEGY','ROUTER');
   ok:=((rp->>'status'='READY_TO_EXECUTE' AND rp->>'action_code'=p_probe->>'expected_action' AND rp->>'operation_code'=p_subject_code)
       OR (rp->>'status'='BLOCKED' AND rp->>'blocking_code'='BLOCK_OPERATION_QUALIFICATION_REQUIRED' AND rp->>'operation_code'=p_subject_code AND EXISTS(SELECT 1 FROM public.lf_router_action_registry a WHERE a.operation_code=p_subject_code AND a.action_code=p_probe->>'expected_action' AND a.status='ACTIVE')));
   actual:=rp;
...
CREATE OR REPLACE FUNCTION public.lf_strategy_matrix_engine_sha256_v1()
RETURNS text LANGUAGE plpgsql STABLE SET search_path TO 'pg_catalog','public','extensions' AS $fn$
DECLARE payload text;
BEGIN
 SELECT string_agg(pg_get_functiondef(p.oid),E'\n---ENGINE---\n' ORDER BY p.proname,p.oid::regprocedure::text) INTO payload
 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
 WHERE n.nspname='public' AND p.oid::regprocedure::text IN (
  'lf_eval_strategy_matrix_probe_v1(jsonb,text,text,text)',
  'lf_run_strategy_matrix_suite_v1(text,text,text,text,text)',
  'lf_canary_strategy_unqualified_execution_block_v1(bigint)',
  'lf_canary_strategy_material_change_invalidates_qualification_v1(bigint)',
  'lf_strategy_execution_qualification_guard_v1(bigint,timestamp with time zone)'
 );
 RETURN encode(extensions.digest(convert_to(coalesce(payload,''),'UTF8'),'sha256'),'hex');
END $fn$;
...
BEGIN
 SELECT jsonb_build_object(
   ...
   'matrix_engine_sha256',public.lf_strategy_matrix_engine_sha256_v1()
...
DO $post$ DECLARE h text; BEGIN h:=public.lf_strategy_matrix_engine_sha256_v1(); IF h !~ '^[0-9a-f]{64}$' THEN RAISE EXCEPTION 'S30_MATRIX_ENGINE_SHA_INVALID:%',h; END IF; END $post$;