...
declare
  ...
  v_validator_identity text; v_source_sha text; v_assert jsonb; v_specs jsonb; v_transition_id bigint; v_decision_number bigint; v_assessment record;
begin
  ...
  select id into v_screen from lf_ops.pantallas where codigo='B2B-AUTH-002';
  ...
  select transition_id into v_transition_id from lf_ops.estados_transiciones where transition_code='TR_RECOVERY_REQUEST_ACCEPTED';
  if v_version_id is null or v_screen is null or v_decision_number is null or v_curator_component is null or v_validator_component is null or v_parent is null or v_transition_id is null then raise exception 'AUTH002_OWNER_DECISION_PREFLIGHT_UNRESOLVED'; end if;
  if programacion.fn_input_readiness_run_is_current(v_parent) then raise exception 'AUTH002_PARENT_EXPECTED_STALE:%',v_parent; end if;
  ...
  select v_new,src.family_code,
    ...
    case when src.family_code='FEATURE_FLAGS' then 'NOT_APPLICABLE por DEC-INPUT-GOV-512-HUMAN-001: FEATURE_FLAGS excluido del alcance actual; ausencia solo complementaria.'
         ...
         when src.family_code='STATES' then 'APPLICABLE y COMPLETE: AUTH-002 materializa B2B_RECOVERY_REQUEST_READY y B2B_RECOVERY_REQUEST_ACCEPTED, preservando respuesta anti-enumeración según AUTH-027/AUTH-028 y decisión 123.'
         when src.family_code='TRANSITIONS' then 'APPLICABLE y COMPLETE: TR_RECOVERY_REQUEST_ACCEPTED modela RECOVERY_SUBMIT sin exponer existencia de cuenta, gobernada por AUTH-027/AUTH-028 y decisión 123.'
    ...
    case when src.family_code='FEATURE_FLAGS' then src.test_obligations||jsonb_build_array('Reopen FEATURE_FLAGS exclusion if canonical feature-flag authority appears')
         ...
         when src.family_code='TRANSITIONS' then src.test_obligations||jsonb_build_array('TR_RECOVERY_REQUEST_ACCEPTED must remain resolvable and consistent with state set')
  ...
  if (select count(*) from programacion.input_family_assessments where run_id=v_new)<>47 then raise exception 'AUTH002_CURATOR_CARDINALITY_MISMATCH'; end if;
  ...
  if not programacion.fn_input_readiness_run_is_current(v_new) then raise exception 'AUTH002_SUCCESSOR_NOT_CURRENT:%',v_new; end if;
  if (select count(*) from programacion.input_family_assessments where run_id=v_new and validator_outcome='PASS')<>47 then raise exception 'AUTH002_VALIDATOR_CARDINALITY_MISMATCH'; end if;
  if (select count(*) from programacion.input_family_assessments where run_id=v_new and applicability='UNRESOLVED')<>0 then raise exception 'AUTH002_UNRESOLVED_REMAINS'; end if;
end;
$do$;