...
BEGIN
  ...
  -- The full screenshot is the sole source canvas/background. Crops are detail-only.
...
BEGIN
  ...
  -- From this migration forward, every P0 browser review is canonical V4.2.
...
BEGIN
  ...
  END IF;
  IF EXISTS (
    SELECT 1 FROM private.lf_p0_human_review_challenges_v1 c
    WHERE c.challenge_id = p_challenge_id OR c.review_id = p_review_id
  ) THEN
  ...
  INSERT INTO private.lf_p0_review_evidence_objects_v1(
    evidence_object_id,review_id,execution_id,object_role,object_name,mime_type,
    content_bytes,content_sha256,content,data_classification,source_head_sha,metadata
  ) VALUES (
    v_manifest_id,p_review_id,p_execution_id,'PACKET_MANIFEST',
    'review-manifest-canonical-v42-'||substr(v_old.source_head_sha,1,12)||'.json',
  ...
  INSERT INTO private.lf_p0_review_evidence_objects_v1(
    ...
    v_html_id,p_review_id,p_execution_id,'BROWSER_REVIEW',
    'human-review-canonical-v42-'||substr(v_old.source_head_sha,1,12)||'.html',
  ...
  INSERT INTO private.lf_p0_human_review_challenges_v1(
    challenge_id,review_id,execution_id,source_head_sha,visual_output_sha256,
    packet_manifest_sha256,required_reviewer_role,reviewer_actions,evidence_store_ref,
    issued_at,expires_at,data_classification,dual_review_required,review_lane,review_subject,
    source_evidence_object_id,source_sha256,visual_output_object_id,packet_manifest_object_id,
    browser_review_object_id,browser_review_sha256,semantic_fingerprint,element_count,
    uncertain_count,inferred_count,changed_count,pending_human_count,delta,review_mode,
    carry_forward_safe,supersedes_challenge_id,convergence_justification
  ) VALUES (
    p_challenge_id,p_review_id,p_execution_id,v_old.source_head_sha,v_old.visual_output_sha256,
    v_manifest_sha,v_old.required_reviewer_role,v_old.reviewer_actions,v_old.evidence_store_ref,
    p_issued_at,p_expires_at,v_old.data_classification,v_old.dual_review_required,v_old.review_lane,
    v_old.review_subject,v_old.source_evidence_object_id,v_old.source_sha256,
    v_old.visual_output_object_id,v_manifest_id,v_html_id,v_html_sha,v_old.semantic_fingerprint,
    v_old.element_count,v_old.uncertain_count,v_old.inferred_count,v_old.changed_count,
    v_old.pending_human_count,v_old.delta,v_old.review_mode,v_old.carry_forward_safe,
    v_old.challenge_id,'CANONICAL_V42_SINGLE_RENDERER_REFRESH_NO_HUMAN_DEBT_CHANGE'
  );

  RETURN jsonb_build_object(
    'outcome','CANONICAL_V42_REVIEW_READY',
    ...
    'source_sha256',v_old.source_sha256,
    'semantic_fingerprint',v_old.semantic_fingerprint,
    'element_count',v_old.element_count,
...
REVOKE ALL ON FUNCTION public.fn_lf_p0_publish_canonical_review_v42_v1(
  text,text,text,text,timestamptz,timestamptz,text,text,text
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.fn_lf_p0_publish_canonical_review_v42_v1(
  text,text,text,text,timestamptz,timestamptz,text,text,text
) TO service_role;

-- Retire the copy-and-replace refresh path. It was the structural drift source.
