...
BEGIN
  ...
  v_fast_def := replace(v_manifest_def,'fn_input_context_manifest(p_run_id bigint)','fn_input_context_manifest_known_current_v1(p_run_id bigint, p_run_current boolean)');
  v_fast_def := replace(v_fast_def,'programacion.fn_input_readiness_run_is_current(p_run_id)','p_run_current');
  v_fast_def := replace(v_fast_def,'programacion.fn_input_stage_gate_summary(p_run_id)','programacion.fn_input_stage_gate_summary_known_current_v1(p_run_id,true)');
  EXECUTE v_fast_def;

  EXECUTE 'REVOKE ALL ON FUNCTION programacion.fn_input_context_manifest_known_current_v1(bigint,boolean) FROM PUBLIC';
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='anon') THEN EXECUTE 'REVOKE ALL ON FUNCTION programacion.fn_input_context_manifest_known_current_v1(bigint,boolean) FROM anon'; END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='authenticated') THEN EXECUTE 'REVOKE ALL ON FUNCTION programacion.fn_input_context_manifest_known_current_v1(bigint,boolean) FROM authenticated'; END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='service_role') THEN EXECUTE 'REVOKE ALL ON FUNCTION programacion.fn_input_context_manifest_known_current_v1(bigint,boolean) FROM service_role'; END IF;
  ...
  v_execute_def := replace(v_execute_def,'programacion.fn_input_context_manifest(v_run)','programacion.fn_input_context_manifest_known_current_v1(v_run,true)');
  EXECUTE v_execute_def;

  IF encode(digest(pg_get_functiondef('programacion.fn_input_context_manifest(bigint)'::regprocedure),'sha256'),'hex') <> '3163c77328166aa29fc81ecaafc58abcdb21a290371997bad2767e13187234fa' THEN
    RAISE EXCEPTION 'INPUT_CONTEXT_MANIFEST_STRONG_ENTRYPOINT_CHANGED';
  END IF;

  SELECT pg_get_functiondef('programacion.fn_input_context_manifest_known_current_v1(bigint,boolean)'::regprocedure) INTO v_verify;
  IF position('programacion.fn_input_readiness_run_is_current(p_run_id)' in v_verify)>0 THEN RAISE EXCEPTION 'KNOWN_CURRENT_MANIFEST_RECOMPUTES_CURRENTNESS'; END IF;
  IF position('programacion.fn_input_stage_gate_summary(p_run_id)' in v_verify)>0 THEN RAISE EXCEPTION 'KNOWN_CURRENT_MANIFEST_RECOMPUTES_STAGE_CURRENTNESS'; END IF;
  IF position('programacion.fn_input_stage_gate_summary_known_current_v1(p_run_id, true)' in v_verify)=0 AND position('programacion.fn_input_stage_gate_summary_known_current_v1(p_run_id,true)' in v_verify)=0 THEN
    RAISE EXCEPTION 'KNOWN_CURRENT_MANIFEST_STAGE_FASTPATH_MISSING';
  END IF;

  IF EXISTS (SELECT 1 FROM pg_proc p CROSS JOIN LATERAL aclexplode(p.proacl) a WHERE p.oid='programacion.fn_input_context_manifest_known_current_v1(bigint,boolean)'::regprocedure AND a.grantee=0 AND a.privilege_type='EXECUTE') THEN
    RAISE EXCEPTION 'KNOWN_CURRENT_MANIFEST_PUBLIC_EXECUTE_NOT_REVOKED';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='service_role') AND has_function_privilege('service_role','programacion.fn_input_context_manifest_known_current_v1(bigint,boolean)','EXECUTE') THEN
  ...
  END IF;
  IF position('programacion.fn_input_context_manifest(v_run)' in v_verify)>0 THEN RAISE EXCEPTION 'INPUT_GOVERNANCE_EXECUTE_STRONG_MANIFEST_DUPLICATE_REMAINS'; END IF;
END;
$migration$;
