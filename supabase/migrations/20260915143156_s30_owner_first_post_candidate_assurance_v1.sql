...
BEGIN
  ...
  END IF;
  IF x.status<>'IN_PROGRESS' THEN RAISE EXCEPTION 'LF_STRATEGY_EXECUTION_BEGIN_EXECUTION_NOT_IN_PROGRESS:%',x.status; END IF;
  ...
  IF FOUND THEN
    IF existing.step_id='init_execution' AND existing.status=b.clean_result_value THEN
      RETURN r || jsonb_build_object('init_step','ALREADY_RECORDED_IDEMPOTENT','snapshot_id',s.id,'target_path',target_path,'target_strategy_ref',target_ref,'target_revision_sha256',target_rev,'next_step','router','snapshot_assurance_timing','POST_CANDIDATE_TERMINAL_EFFECT');
    END IF;
    RAISE EXCEPTION 'LF_STRATEGY_EXECUTION_BEGIN_INIT_STEP_CONFLICT:%:%',existing.step_id,existing.status;
  END IF;
  ep:=jsonb_build_object(
    'execution_id',x.execution_id,'idempotency_key',x.idempotency_key,'request_sha256',x.request_sha256,
    'target_strategy_ref',target_ref,'target_strategy_revision_sha256',target_rev,'target_path',target_path,
    'runtime_mode','GOVERNED_OPERATIONAL_OR_START_PINNED','source_contract',c.contract_code,
    ...
    'assertions_checked',jsonb_build_array('r16_quality_bound','c05_reliability_bound','required_evidence_present','authority_currentness_pass','runtime_activation_authorized','business_effect_dispatch_allowed','owner_execution_not_pre_gated_by_snapshot_independent_assurance'),
    'hard_fails_checked','[]'::jsonb,'blocking_findings','[]'::jsonb,'blocking_codes','[]'::jsonb,'return_to_worker_reasons','[]'::jsonb,
    'step_result',b.clean_result_value,'mini_judge_code',b.judge_code,'mini_judge_result',b.clean_result_value,'attempt_history','[]'::jsonb,
  ...
  VALUES(x.execution_id,st.step_order,'init_execution',b.clean_result_value,target_path,ep,'Initialized transactionally by governed Strategy Execution begin RPC under owner-first/post-candidate-assurance policy.',p_actor_execution_id);
  ...
  IF NOT FOUND OR existing.status<>b.clean_result_value OR existing.evidence_payload->>'derived_result'<>b.clean_result_value THEN
    RAISE EXCEPTION 'LF_STRATEGY_EXECUTION_BEGIN_READBACK_FAILED';
  END IF;
  RETURN r || jsonb_build_object('init_step','RECORDED','snapshot_id',s.id,'target_path',target_path,'target_strategy_ref',target_ref,'target_revision_sha256',target_rev,'next_step','router','snapshot_assurance_timing','POST_CANDIDATE_TERMINAL_EFFECT');
END
$fn$;
...
CREATE OR REPLACE FUNCTION public.lf_strategy_close_write_v1(
  p_execution_id text,p_snapshot_id bigint,p_expected_revision_sha256 text,p_reason text,p_disposition text,p_evidence jsonb
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  IF NOT FOUND OR x.operation_code<>'CIERRE_ESTRATEGIA_LF' OR x.status<>'IN_PROGRESS' OR x.target_type<>'STRATEGY' THEN RAISE EXCEPTION 'LF_STRATEGY_CLOSE_EXECUTION_BINDING_MISMATCH'; END IF;
  ...
  IF public.lf_lifecycle_state_is_terminal_v1('STRATEGY_LIFECYCLE',s.lifecycle_state_code) OR coalesce(s.metadata->'strategy_close'->>'status','')='CLOSED' THEN
    IF s.metadata->'strategy_close'->>'execution_id'=p_execution_id THEN RETURN jsonb_build_object('result','ALREADY_CLOSED_IDEMPOTENT','snapshot_id',s.id,'close_receipt',s.metadata->'strategy_close'); END IF;
  ...
  IF coalesce(p_evidence->>'safe_work_remaining_count','') !~ '^[0-9]+$' OR coalesce(p_evidence->>'open_executable_causal_chains','') !~ '^[0-9]+$' THEN RAISE EXCEPTION 'LF_STRATEGY_CLOSE_ELIGIBILITY_COUNTS_REQUIRED'; END IF;
  sw:=(p_evidence->>'safe_work_remaining_count')::int; oc:=(p_evidence->>'open_executable_causal_chains')::int;
  ...
  IF a.lifecycle_state_code<>target_state OR a.status IS DISTINCT FROM s.status OR a.archived_at IS DISTINCT FROM s.archived_at OR a.archived_reason IS DISTINCT FROM s.archived_reason OR a.runtime_state<>'NO_HABILITADO' OR a.impact_policy<>'BLOQUEADO' OR a.visibility<>'READ_ONLY_INTERNAL' THEN RAISE EXCEPTION 'LF_STRATEGY_CLOSE_POSTWRITE_INVARIANT'; END IF;
...
SET search_path TO 'pg_catalog','public'
AS $fn$
DECLARE s public.lf_strategy_snapshots%rowtype; qstate text; terminal_blocked boolean:=false; owner_begin_pre_gate_absent boolean:=false; owner_lifecycle_pre_gate_absent boolean:=false; terminal_gate_bound boolean:=false; msg text; begin_def text; lifecycle_def text; close_def text;
BEGIN
  SELECT * INTO s FROM public.lf_strategy_snapshots WHERE id=p_snapshot_id;
  ...
  BEGIN
    UPDATE public.lf_qualification_receipts q SET lifecycle_state_code=public.lf_lifecycle_resolve_transition_v1('QUALIFICATION_LIFECYCLE',q.lifecycle_state_code,'INVALIDATE_QUALIFICATION'),invalidated_at=clock_timestamp() WHERE q.subject_type='STRATEGY' AND q.subject_code=s.snapshot_code AND q.lifecycle_state_code=qstate AND q.invalidated_at IS NULL;
    BEGIN
      PERFORM public.lf_strategy_terminal_qualification_guard_v1(s.id,'CLOSE_STRATEGY',clock_timestamp());
    EXCEPTION WHEN OTHERS THEN
      GET STACKED DIAGNOSTICS msg=MESSAGE_TEXT;
      IF msg LIKE 'LF_STRATEGY_TERMINAL_QUALIFICATION_REQUIRED:%' THEN terminal_blocked:=true; ELSE RAISE; END IF;
    END;
    RAISE EXCEPTION USING ERRCODE='P0Q21',MESSAGE='S30_R21_CANARY_ROLLBACK';
  EXCEPTION WHEN SQLSTATE 'P0Q21' THEN NULL;
  END;
  RETURN jsonb_build_object('passed',owner_begin_pre_gate_absent AND owner_lifecycle_pre_gate_absent AND terminal_gate_bound AND terminal_blocked,'snapshot_id',s.id,'snapshot_code',s.snapshot_code,'policy_code','S30-OWNER-FIRST-POST-CANDIDATE-ASSURANCE-v1','owner_begin_pre_gate_absent',owner_begin_pre_gate_absent,'owner_lifecycle_pre_gate_absent',owner_lifecycle_pre_gate_absent,'terminal_gate_bound',terminal_gate_bound,'terminal_unqualified_close_blocked',terminal_blocked,'expected_terminal_block','LF_STRATEGY_TERMINAL_QUALIFICATION_REQUIRED','persistent_mutation',false);
END
$fn$;
...
SET title='Owner Strategy execution is not pre-gated; terminal close requires current qualification',
    input_payload=jsonb_build_object('probe_code','STRATEGY_UNQUALIFIED_EXECUTION_BLOCK','policy_code','S30-OWNER-FIRST-POST-CANDIDATE-ASSURANCE-v1','owner_execution_pre_assurance_permission_gate_allowed',false,'terminal_close_requires_current_qualification',true),
    metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('semantic_revision','R21_OWNER_FIRST_POST_CANDIDATE_ASSURANCE','legacy_probe_name_preserved',true),
    updated_at=clock_timestamp(),updated_by_execution_id='EXEC-S30-R21-POST-CANDIDATE-ASSURANCE-POLICY-20260915-001'
WHERE suite_code='TS-STRATEGY-BASE-V1' AND test_code='S11';

DO $verify$
DECLARE begin_def text; lifecycle_def text; close_def text; s11_count int;
BEGIN
  SELECT pg_get_functiondef('public.lf_strategy_execution_begin_v1(text,bigint,text,text,text,jsonb)'::regprocedure) INTO begin_def;
  SELECT pg_get_functiondef('public.lf_strategy_lifecycle_from_execution_step_v1()'::regprocedure) INTO lifecycle_def;
  SELECT pg_get_functiondef('public.lf_strategy_close_write_v1(text,bigint,text,text,text,jsonb)'::regprocedure) INTO close_def;
  IF strpos(begin_def,'lf_strategy_execution_qualification_guard_v1')>0 THEN RAISE EXCEPTION 'S30_R21_VERIFY_PRE_EXECUTION_QUALIFICATION_GATE_STILL_BOUND'; END IF;
  IF strpos(begin_def,'lf_operation_execution_qualification_guard_v1')=0 THEN RAISE EXCEPTION 'S30_R21_VERIFY_OPERATION_QUALIFICATION_GUARD_MISSING'; END IF;
  IF strpos(lifecycle_def,'lf_strategy_execution_qualification_guard_v1')>0 THEN RAISE EXCEPTION 'S30_R21_VERIFY_LIFECYCLE_PRE_ASSURANCE_GATE_STILL_BOUND'; END IF;
  IF strpos(close_def,'lf_strategy_terminal_qualification_guard_v1')=0 THEN RAISE EXCEPTION 'S30_R21_VERIFY_TERMINAL_QUALIFICATION_GATE_MISSING'; END IF;
  SELECT count(*) INTO s11_count FROM public.lf_test_suite_cases WHERE suite_code='TS-STRATEGY-BASE-V1' AND test_code='S11' AND input_payload->>'policy_code'='S30-OWNER-FIRST-POST-CANDIDATE-ASSURANCE-v1' AND coalesce((input_payload->>'owner_execution_pre_assurance_permission_gate_allowed')::boolean,true)=false AND coalesce((input_payload->>'terminal_close_requires_current_qualification')::boolean,false)=true;
