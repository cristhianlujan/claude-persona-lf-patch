...
begin
  ...
  if coalesce(v_observed->>'observed_sha256','')!~'^[0-9a-f]{64}$'
     or v_observed->>'observed_sha256' is distinct from programacion.fn_v09_sha256_jsonb(v_observed-'observed_sha256') then raise exception 'EXTERNAL_VERIFY_V2_OBSERVED_SHA_MISMATCH'; end if;
  ...
  if p_verification_payload->>'github_repository' is distinct from 'cristhianlujan/libertad-financiera'
     ...
     or coalesce(p_verification_payload->>'github_run_id','')!~'^[1-9][0-9]*$' then raise exception 'EXTERNAL_VERIFY_V2_VERIFIER_IDENTITY_PAYLOAD_MISMATCH'; end if;
  ...
  if v_observed->>'remote_readback_status' is distinct from 'PASS'
     ...
     or v_observed->>'github_repository_id' is distinct from '1301234955' then raise exception 'EXTERNAL_VERIFY_V2_REMOTE_READBACK_INVALID'; end if;
  if v_observed->>'producer_run_id' is distinct from v_receipt->>'github_actions_run_id'
     or v_observed->>'producer_run_id' is distinct from regexp_replace(v_ev.source_ref,'^.*/actions/runs/([0-9]+)$','\1') then raise exception 'EXTERNAL_VERIFY_V2_PRODUCER_RUN_MISMATCH'; end if;
  ...
  if v_observed->>'producer_workflow_path' is distinct from '.github/workflows/hmo-001-e2e-evidence.yml'
     ...
     or v_observed->>'producer_tested_head_sha' is distinct from v_ex.head_sha then raise exception 'EXTERNAL_VERIFY_V2_PRODUCER_BINDING_MISMATCH'; end if;
  ...
  if v_expected_count<>v_observed_count
     ...
     or exists(select 1 from jsonb_array_elements_text(v_observed_files) o(x) where not exists(select 1 from jsonb_array_elements_text(v_expected_files) e(x) where e.x=o.x)) then raise exception 'EXTERNAL_VERIFY_V2_CHANGED_FILES_MISMATCH'; end if;
  if coalesce(v_observed->>'remote_changed_files_sha256','')!~'^[0-9a-f]{64}$'
     or v_observed->>'remote_changed_files_sha256' is distinct from programacion.fn_v09_sha256_jsonb(v_observed_files) then raise exception 'EXTERNAL_VERIFY_V2_CHANGED_FILES_SHA_MISMATCH'; end if;
  if coalesce(jsonb_typeof(v_observed->'remote_file_blobs'),'')<>'object'
     or coalesce(v_observed->>'remote_file_blobs_sha256','')!~'^[0-9a-f]{64}$'
     or v_observed->>'remote_file_blobs_sha256' is distinct from programacion.fn_v09_sha256_jsonb(v_observed->'remote_file_blobs') then raise exception 'EXTERNAL_VERIFY_V2_REMOTE_BLOBS_SHA_MISMATCH'; end if;
  if exists(select 1 from jsonb_array_elements_text(v_expected_files) e(x) where not ((v_observed->'remote_file_blobs') ? e.x))
     or exists(select 1 from jsonb_object_keys(v_observed->'remote_file_blobs') o(x) where not exists(select 1 from jsonb_array_elements_text(v_expected_files) e(x) where e.x=o.x)) then raise exception 'EXTERNAL_VERIFY_V2_REMOTE_BLOBS_SET_MISMATCH'; end if;
  ...
  if coalesce(jsonb_typeof(v_independent),'')<>'object'
     ...
     or v_independent->>'lint' is distinct from 'PASS' then raise exception 'EXTERNAL_VERIFY_V2_INDEPENDENT_EXECUTION_INVALID'; end if;
  if coalesce(v_independent->>'execution_sha256','')!~'^[0-9a-f]{64}$'
     or v_independent->>'execution_sha256' is distinct from programacion.fn_v09_sha256_jsonb(v_independent-'execution_sha256') then raise exception 'EXTERNAL_VERIFY_V2_EXECUTION_SHA_MISMATCH'; end if;
  ...
  if v_semantic is distinct from v_receipt#>>'{tests,semantic}'
     ...
     or v_auth is distinct from v_receipt#>>'{tests,auth_regression}' then raise exception 'EXTERNAL_VERIFY_V2_INDEPENDENT_SUMMARY_MISMATCH'; end if;
end;
$function$;
...
begin
  ...
  if v_worker_receipt->>'status'<>'PASS' or v_worker_receipt->>'execution_id'<>v_ex.id::text
     or v_worker_receipt->>'context_pack_id'<>v_cp.id::text or v_worker_receipt->>'context_pack_sha256'<>v_cp.context_sha256 then raise exception 'EXTERNAL_VERIFY_WORKER_RECEIPT_BINDING_INVALID'; end if;
  ...
  v_subject_payload:=jsonb_build_object('evidence_id',v_ev.id,'evidence_sha256',v_ev.sha256,'source_system',v_ev.source_system,
    'source_ref',v_ev.source_ref,'verification_status','VERIFIED','verification_method',p_verification_method,'verifier_identity',p_verifier_identity,'verification_payload',p_verification_payload);
  ...
  v_receipt_payload:=p_verification_payload||jsonb_build_object('execution_id',v_ex.id,'head_sha',v_ex.head_sha,
    'subject_type','evidence_verification','subject_ref','evidence:'||v_ev.id::text,'subject_sha256',v_subject_sha,'verification_status','VERIFIED','verifier_identity',p_verifier_identity);
  ...
  if v_proof_version='2' then
    update programacion.evaluaciones set resultado='PASS',finished_at=now(),
...
returns boolean language sql stable security definer set search_path to 'pg_catalog','programacion'
as $function$
  select exists(
    select 1 from programacion.ejecuciones ex
    ...
    where ex.id=p_execution_id and ex.request_ref~'^agent-task://[1-9][0-9]*$'
      ...
      and ev.metadata#>>'{worker_receipt,execution_id}'=ex.id::text and ev.metadata#>>'{worker_receipt,context_pack_id}'=cp.id::text
      and ev.metadata#>>'{worker_receipt,context_pack_sha256}'=cp.context_sha256 and programacion.fn_evidence_pass_compatible_v1(ev.id)
      and (coalesce(ev.metadata#>>'{worker_receipt,proof_contract_version}','1')<>'2' or exists(
        select 1 from programacion.evidence_verifications vv where vv.evidence_id=ev.id and vv.verification_status='VERIFIED'
          ...
          and vv.verification_payload#>>'{observed,independent_execution,status}'='PASS' and vv.authority_receipt_id is not null))
  );
$function$;

-- Deterministically amend the existing materializer rather than adding a parallel consumer.
do $patch$
declare v_ddl text;
begin
  select pg_get_functiondef(p.oid) into v_ddl from pg_proc p join pg_namespace n on n.oid=p.pronamespace
