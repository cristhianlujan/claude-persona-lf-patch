...
declare v_def text;
begin
  if to_regprocedure('programacion.fn_input_resolve_source_ref_v510(jsonb,integer,bigint)') is null then
    select pg_get_functiondef(p.oid) into v_def from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='programacion' and p.proname='fn_input_resolve_source_ref' and pg_get_function_identity_arguments(p.oid)='p_ref jsonb, p_pantalla_id integer, p_version_id bigint';
    v_def:=replace(v_def,'FUNCTION programacion.fn_input_resolve_source_ref(p_ref jsonb, p_pantalla_id integer, p_version_id bigint)','FUNCTION programacion.fn_input_resolve_source_ref_v510(p_ref jsonb, p_pantalla_id integer, p_version_id bigint)');
    execute v_def;
  end if;
end$$;
...
declare v_kind text:=p_ref->>'kind'; v_ids bigint[]; v_expected integer; v_actual integer; v_observed jsonb;
begin
  if v_kind='ERROR_SET' then
...
returns text language sql immutable set search_path=pg_catalog as $$
  select case coalesce(p_ref->>'kind','')
    ...
    when 'ERROR_SET' then 'CANONICAL_DOMAIN_SOURCE'
    when 'MESSAGE_SET' then 'CANONICAL_DOMAIN_SOURCE'
