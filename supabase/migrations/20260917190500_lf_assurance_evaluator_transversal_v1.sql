...
-- Single transversal evaluator for the existing LF Assurance Method.
-- Reuses canonical claim/obligation/defeater catalogs, subject bindings,
    ...
    if v_assertion_fail > 0 then
      ...
      if v_nonpass_judge='FAIL' then
        ...
        where j.test_run_id=v_run.test_run_id
          ...
          and (v_run.operation_code is null or reviewer.operation_code is distinct from v_run.operation_code)
          and (j.metadata->>'reviewer_operation_code' is null or j.metadata->>'reviewer_operation_code'=reviewer.operation_code)
...
create or replace function public.lf_assurance_obligation_evidence_v1(
  p_suite_code text,
  p_subject_revision text,
  p_obligation_code text,
  p_obligation_version integer
)
returns jsonb
language plpgsql
stable
...
declare
  ...
  v_zero_effect_required boolean := false;
...
begin
  ...
  where obligation_code=p_obligation_code
    and version=p_obligation_version
    and required=true
    and status in ('ACTIVE','CANDIDATO')
  limit 1;
  ...
  if not found then
    return jsonb_build_object(
      'obligation_code',p_obligation_code,'obligation_version',p_obligation_version,
      'result','UNPROVEN','reason','REQUIRED_OBLIGATION_NOT_REGISTERED',
      'evaluator_mode','DETERMINISTIC','evidence_refs','[]'::jsonb
    );
  end if;

  if p_suite_code is null then
    return jsonb_build_object(
      'obligation_code',v_obl.obligation_code,'obligation_version',v_obl.version,
      'result','UNPROVEN','reason','OBLIGATION_SUITE_BINDING_MISSING',
      'evaluator_mode','DETERMINISTIC','evidence_refs','[]'::jsonb
    );
  end if;

  v_zero_effect_required :=
    coalesce(case when jsonb_typeof(v_obl.evidence_contract->'zero_effect_required')='boolean' then (v_obl.evidence_contract->>'zero_effect_required')::boolean end,false)
    or coalesce(case when jsonb_typeof(v_obl.evidence_contract->'zero_effect_on_fail')='boolean' then (v_obl.evidence_contract->>'zero_effect_on_fail')::boolean end,false)
    or coalesce(case when jsonb_typeof(v_obl.evidence_contract->'zero_effect_on_failure')='boolean' then (v_obl.evidence_contract->>'zero_effect_on_failure')::boolean end,false)
    or coalesce(case when jsonb_typeof(v_obl.evidence_contract->'zero_effect_on_unproven')='boolean' then (v_obl.evidence_contract->>'zero_effect_on_unproven')::boolean end,false);

  for v_kind,v_ref in
    with raw_refs(kind,raw_ref) as (
      values
        ('POSITIVE',v_obl.positive_test_ref),
        ('NEGATIVE',v_obl.negative_test_ref),
        ('ADVERSARIAL',v_obl.adversarial_test_ref)
    )
    select r.kind,btrim(parts.piece)
    from raw_refs r
    cross join lateral regexp_split_to_table(coalesce(r.raw_ref,''),',') as parts(piece)
    where nullif(btrim(parts.piece),'') is not null
    order by r.kind,btrim(parts.piece)
  loop
    ...
    if not exists (
      ...
      v_has_unproven := true;
      v_case_results := v_case_results || jsonb_build_array(jsonb_build_object(
        'kind',v_kind,'test_code',v_ref,'result','UNPROVEN','reason','OBLIGATION_TEST_REF_UNMAPPED'
      ));
    else
      v_mapped := v_mapped + 1;
      v_eval := public.lf_assurance_case_evidence_v1(p_suite_code,v_ref,p_subject_revision);
      v_case_results := v_case_results || jsonb_build_array(jsonb_build_object('kind',v_kind,'evidence',v_eval));
      v_evidence_refs := v_evidence_refs || coalesce(v_eval->'evidence_refs','[]'::jsonb);

      if v_eval->>'evaluator_mode'='INDEPENDENT_REVIEW' then v_has_independent := true;
      else v_has_deterministic := true;
      end if;

      if v_kind in ('NEGATIVE','ADVERSARIAL')
         and coalesce((v_eval->>'zero_effect_proven')::boolean,false) then
        v_zero_effect_seen := true;
      end if;

      case coalesce(v_eval->>'result','UNPROVEN')
  ...
  if v_obl.verification_method='INDEPENDENT_REVIEW' and not v_has_independent then
    v_has_unproven := true;
  end if;

  if v_zero_effect_required and not v_zero_effect_seen then
    v_has_unproven := true;
  end if;
  ...
  elsif v_has_unproven or v_unmapped>0 or v_mapped<>v_declared or v_passed<>v_mapped then v_result := 'UNPROVEN';
  else v_result := 'PASS';
  end if;

  return jsonb_build_object(
    'obligation_code',v_obl.obligation_code,'obligation_version',v_obl.version,
    'verification_method',v_obl.verification_method,'result',v_result,'evaluator_mode',v_mode,
    'declared_test_refs',v_declared,'mapped_test_refs',v_mapped,'passed_test_refs',v_passed,
    'unmapped_test_refs',v_unmapped,'zero_effect_required',v_zero_effect_required,
    'zero_effect_proven',v_zero_effect_seen,'case_results',v_case_results,'evidence_refs',v_evidence_refs
  );
end;
$function$;
...
begin
  ...
  -- A defeater is countered only by negative/adversarial proof, never by happy-path evidence alone.
  for v_ref in
    with raw_refs(raw_ref) as (
      values (v_obl.negative_test_ref),(v_obl.adversarial_test_ref)
    )
    select distinct btrim(parts.piece)
    from raw_refs r
    cross join lateral regexp_split_to_table(coalesce(r.raw_ref,''),',') as parts(piece)
    where nullif(btrim(parts.piece),'') is not null
    order by btrim(parts.piece)
  loop
    v_declared := v_declared + 1;
    if not exists (
      select 1 from public.lf_test_suite_cases tc
      ...
      v_evidence_refs := v_evidence_refs || coalesce(v_eval->'evidence_refs','[]'::jsonb);
      if v_eval->>'result'='PASS' then v_passed := v_passed + 1; end if;
      if coalesce((v_eval->>'zero_effect_proven')::boolean,false) then v_zero_effect_seen := true; end if;
  ...
  if v_unknown_contract_keys>0 then v_required_ok := false; end if;
  ...
  for v_required_label in
    select distinct x.value from jsonb_array_elements_text(v_required_labels) as x(value)
  loop
    if not exists (
  ...
  return jsonb_build_object(
    ...
    'closed',(
      v_declared>0 and v_unmapped=0 and v_mapped=v_declared and v_passed=v_mapped
      and v_required_ok and (not v_def.zero_effect_required or v_zero_effect_seen)
    ),
    'reason',case
      when v_declared=0 then 'DEFEATER_NEGATIVE_ADVERSARIAL_CASE_MISSING'
      when v_unmapped>0 or v_mapped<>v_declared then 'DEFEATER_NEGATIVE_ADVERSARIAL_CASE_UNMAPPED'
    ...
    'declared_countertests',v_declared,'mapped_countertests',v_mapped,'passed_countertests',v_passed,
    'unmapped_countertests',v_unmapped,'required_counterevidence',v_def.required_counterevidence,
...
declare
  ...
  v_required_obligations integer := 0;
  ...
  v_has_deterministic boolean := false;
  v_has_independent boolean := false;
  v_unknown_closure_keys integer := 0;
  v_case_details jsonb := '[]'::jsonb;
  v_child_details jsonb := '[]'::jsonb;
  v_obligation_details jsonb := '[]'::jsonb;
...
begin
  ...
  select count(*)::integer into v_unknown_closure_keys
  from jsonb_object_keys(coalesce(v_claim.closure_rule,'{}'::jsonb)) as x(key)
  where x.key not in (
    'required_cases','pass_requires','open_defeater_blocks_pass','zero_effect_required',
    'independent_review_required','self_assessment_forbidden','exact_head_readback_required'
  );
  if v_unknown_closure_keys>0 then
    v_has_unproven := true;
    v_reasons := v_reasons || jsonb_build_array('CLOSURE_RULE_HAS_UNSUPPORTED_KEYS');
  end if;
  ...
  -- Obligations are a first-class closure layer; claims do not jump directly from catalog to PASS.
  for v_obl in
    select distinct on (o.obligation_code) o.obligation_code,o.version
    from public.lf_assurance_obligation_catalog o
    where o.claim_code=p_claim_code
      and o.claim_version=p_claim_version
      and o.required=true
      and o.status in ('ACTIVE','CANDIDATO')
    order by o.obligation_code,o.version desc
  loop
    v_required_obligations := v_required_obligations + 1;
    v_machine_resolved := true;
    v_obl_eval := public.lf_assurance_obligation_evidence_v1(v_suite_code,p_subject_revision,v_obl.obligation_code,v_obl.version);
    v_obligation_details := v_obligation_details || jsonb_build_array(v_obl_eval);
    v_evidence_refs := v_evidence_refs || coalesce(v_obl_eval->'evidence_refs','[]'::jsonb);
    if v_obl_eval->>'evaluator_mode' in ('INDEPENDENT_REVIEW','HYBRID') then v_has_independent := true; end if;
    if v_obl_eval->>'evaluator_mode' in ('DETERMINISTIC','HYBRID') then v_has_deterministic := true; end if;
    case coalesce(v_obl_eval->>'result','UNPROVEN')
      when 'FAIL' then v_has_fail := true;
      when 'FALSE_PASS_RISK' then v_has_false_pass := true;
      when 'OPEN' then v_has_open := true;
      when 'PASS' then null;
  ...
  return jsonb_build_object(
    ...
    'required_case_results',v_case_details,'required_subclaim_results',v_child_details,
    'required_obligation_results',v_obligation_details,'defeater_results',v_defeater_details
  );
end;
$function$;
...
as $function$
  select public.lf_assurance_claim_resolve_core_v1(
    p_subject_type,p_subject_code,p_subject_revision,p_claim_code,p_claim_version,0
  );
$function$;
...
revoke all on function public.lf_assurance_obligation_evidence_v1(text,text,text,integer) from public,anon,authenticated;
...
grant execute on function public.lf_assurance_obligation_evidence_v1(text,text,text,integer) to service_role;
...
'Exact-revision LF Test Matrix evidence resolver. Normalizes PASS/PASSED and FAIL/FAILED, rejects assertion contradictions, requires governed execution/durable evidence, and validates canonical independent-review lineage.';
comment on function public.lf_assurance_obligation_evidence_v1(text,text,text,integer) is
'Resolves one required assurance obligation through its canonical positive/negative/adversarial LF Test Matrix refs. Unmapped or unproven refs remain UNPROVEN.';
comment on function public.lf_assurance_defeater_evidence_v1(text,text,text,integer) is
'Closes a defeater only from mapped negative/adversarial PASS evidence, declared counterevidence labels and zero-effect proof when required.';
comment on function public.lf_assurance_claim_evaluate_v1(text,text,text,text,integer) is
'Transversal LF assurance evaluator. Resolves supported machine-readable closure rules, required obligations, child claims and defeaters from exact-revision canonical evidence; uncertainty fails closed.';
comment on function public.lf_assurance_claim_evaluate_and_record_v1(text,text,text,text,integer,text,text) is
'Append-only assurance recorder with governed execution provenance and transaction advisory locking. The caller cannot declare PASS.';